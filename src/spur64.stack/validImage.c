/* Automatically generated by
   ...
   bec3b39b-863c-43da-aa70-36c06b3b38bf (Compiler-eem.520) from ImageLeakChecker
   VMMaker.oscog-eem.3664 uuid: bec3b39b-863c-43da-aa70-36c06b3b38bf
 */
static char __buildInfo[] = "ImageLeakChecker VMMaker.oscog-eem.3664 uuid: "
...
static NoDbgRegParms void updateFormatOfShortenedObjectto(sqInt objOop,
                                                          sqInt indexableSize);
...
newSpaceStartnewSpaceBytessurvivorBytes(sqInt startAddress, sqInt totalBytes,
                                        sqInt requestedSurvivorBytes);
...
static NoDbgRegParms sqInt addToFreeListbytes(sqInt freeChunk,
                                              sqInt chunkBytes);
static NoDbgRegParms sqInt addToFreeTreebytes(sqInt freeChunk,
                                              sqInt chunkBytes);
...
static NoDbgRegParms void checkFreeSpaceignoring(sqInt gcModes,
                                                 sqInt anOopToIgnore);
...
static NoDbgRegParms sqInt fetchPointerofFreeChunk(sqInt fieldIndex,
                                                   sqInt objOop);
static NoDbgRegParms sqInt fetchPointerofMaybeForwardedObject(sqInt fieldIndex,
                                                              sqInt objOop);
...
static NoDbgRegParms void initFreeTreeChunkbytes(sqInt freeChunk,
                                                 sqInt chunkBytes);
...
static NoDbgRegParms void
setNextFreeChunkOfwithValuechunkBytes(sqInt freeChunk, sqInt nextFreeChunk,
                                      sqInt chunkBytes);
...
extern sqInt storePointerUncheckedofObjectwithValue(sqInt fieldIndex,
                                                    sqInt objOop,
                                                    sqInt valuePointer);
static NoDbgRegParms sqInt storePointerofForwarderwithValue(sqInt fieldIndex,
                                                            sqInt objOop,
                                                            sqInt valuePointer);
static NoDbgRegParms sqInt storePointerofFreeChunkwithValue(sqInt fieldIndex,
                                                            sqInt objOop,
                                                            sqInt valuePointer);
extern sqInt storePointerofObjectwithValue(sqInt fieldIndex, sqInt objOop,
                                           sqInt valuePointer);
static NoDbgRegParms sqInt storeUncheckedNoAssertofAnywithValue(
    sqInt fieldIndex, sqInt objOop, sqInt valuePointer);
...
static NoDbgRegParms sqInt unlinkFreeChunkatIndexchunkBytes(sqInt chunk,
                                                            sqInt index,
                                                            sqInt chunkBytes);
...
static NoDbgRegParms sqInt updateRootOfObjStackAtwith(sqInt objStackRootIndex,
                                                      sqInt newRootPage);
...
static NoDbgRegParms sqInt followLiteralofMethod(sqInt offset,
                                                 sqInt methodPointer);
static NoDbgRegParms sqInt getLongFromFileswap(sqImageFile aFile,
                                               sqInt swapFlag);
static NoDbgRegParms usqInt getShortFromFileswap(sqImageFile aFile,
                                                 sqInt swapFlag);
static NoDbgRegParms sqInt getWord32FromFileswap(sqImageFile aFile,
                                                 sqInt swapFlag);
...
static NoDbgRegParms sqInt includesBehaviorThatOf(sqInt aClass,
                                                  sqInt aSuperclass);
...
static NoDbgRegParms char *nameOfClasslengthInto(sqInt classOop,
                                                 sqInt *lengthPtr);
static NoDbgRegParms sqInt objectbeginsWithStringofSize(sqInt anOop,
                                                        char *aCString,
                                                        sqInt aCStringStrlen);
static NoDbgRegParms sqInt objectequalsString(sqInt anOop, char *aCString);
static NoDbgRegParms sqInt objectequalsStringofSize(sqInt anOop, char *aCString,
                                                    sqInt aCStringStrlen);
...
const char *interpreterVersion =
    "Open Smalltalk ImageChecker VM [ImageLeakChecker VMMaker.oscog-eem.3664]";
...
void *ioLoadFunctionFrom(char *functionName, char *moduleName) { return null; }
...
int main(int argc, char *argv[]) {
  ...
  if ((argc == argi) && (forVersion)) {
    return 0;
  }
  if ((argc != (argi + 1)) ||
      (((ifh = sqImageFileOpen((GIV(imageFilename) = argv[argi]), "rb"))) ==
       0)) {
...
void printHex(usqInt n) {
  DECL_MAYBE_SQ_GLOBAL_STRUCT
  if (GIV(verbose)) {
    fprintf(GIV(transcript), "%16p" /* pst: */, ((void *)n));
  }
}
...
static NoDbgRegParms void printNum(sqInt n) {
  DECL_MAYBE_SQ_GLOBAL_STRUCT
  if (GIV(verbose)) {
    fprintf(GIV(transcript), "%" PRIdSQINT "", ((sqInt)n));
  }
}
...
static NoDbgRegParms int isDirectAlien(sqInt oop) {
  return (longAt((void *)(oop + BaseHeaderSize))) > 0;
}
...
static NoDbgRegParms sqInt isPositiveMachineIntegerObject(sqInt oop) {
  ...
  classOop =
      fetchPointerofObject(ClassLargePositiveInteger, GIV(specialObjectsOop));
...
usqInt sizeOfAlienData(sqInt oop) {
  DECL_MAYBE_SQ_GLOBAL_STRUCT
  if (!(isKindOfClass(
          oop, fetchPointerofObject(ClassAlien, GIV(specialObjectsOop))))) {
    /* primitiveFailFor: */
...
allocateSlotsForPinningInOldSpacebytesformatclassIndex(sqInt numSlots,
          ...
          while (node != 0) {
            ...
            if (((segmentContainingObj(node))->containsPinned)) {
              if (prev) {
                ...
                storePointerofFreeChunkwithValue(0 /* freeChunkNextIndex */,
                                                 prev, next);
                if (next) {
                  storePointerofFreeChunkwithValue(1 /* freeChunkPrevIndex */,
                                                   next, prev);
                }
              } else {
                ...
                GIV(freeLists)
                [index] = ((nextSqInt = fetchPointerofFreeChunk(
                                0 /* freeChunkNextIndex */, node)));
                if (nextSqInt) {
                  storePointerofFreeChunkwithValue(1 /* freeChunkPrevIndex */,
                                                   nextSqInt, 0);
                }
              }
              freeChunkWithBytesat((index * (allocationUnit())) - totalBytes,
                                   (startOfObject(node)) + totalBytes);
              chunk = node;
              goto l1;
            }
            prev = node;
    ...
    if (childBytes == totalBytes) {
      ...
      while (1) {
        ...
        if (((segmentContainingObj(node))->containsPinned)) {
          ...
          assert(assertInnerValidFreeObject(node));
          nextFreeChunk =
              fetchPointerofFreeChunk(0 /* freeChunkNextIndex */, node);

          /* begin setNextFreeChunkOf:withValue:isLilliputianSize: */
          storePointerofFreeChunkwithValue(0 /* freeChunkNextIndex */, prev,
                                           nextFreeChunk);
          if (nextFreeChunk) {
            storePointerofFreeChunkwithValue(1 /* freeChunkPrevIndex */,
                                             nextFreeChunk, prev);
    ...
    if (child) {
      if (childBytes <= (totalBytes + (allocationUnit()))) {
        ...
        if (!acceptedNode) {
          ...
          do {
            acceptedChunk = fetchPointerofFreeChunk(0 /* freeChunkNextIndex */,
                                                    acceptedChunk);
            if ((acceptedChunk != 0) &&
  ...
  if (acceptedNode) {
    if (acceptedChunk) {
      ...
      while (1) {
        next =
            fetchPointerofFreeChunk(0 /* freeChunkNextIndex */, acceptedNode);
        if (!(next != acceptedChunk))
          break;
        acceptedNode = next;
      }
      nextFreeChunk =
          fetchPointerofFreeChunk(0 /* freeChunkNextIndex */, acceptedChunk);

      /* begin setNextFreeChunkOf:withValue:isLilliputianSize: */
      storePointerofFreeChunkwithValue(0 /* freeChunkNextIndex */, acceptedNode,
                                       nextFreeChunk);
      if (nextFreeChunk) {
        storePointerofFreeChunkwithValue(1 /* freeChunkPrevIndex */,
                                         nextFreeChunk, acceptedNode);
      }
      freeChunkWithBytesat((bytesInBody(acceptedChunk)) - totalBytes,
                           (startOfObject(acceptedChunk)) + totalBytes);
    ...
    freeChunkWithBytesat((bytesInBody(acceptedNode)) - totalBytes,
                         (startOfObject(acceptedNode)) + totalBytes);
...
sqInt fetchClassTagOf(sqInt oop) {
  ...
  return ((tagBits = oop & (tagMask())) ? tagBits : classIndexOf(oop));
}
...
static NoDbgRegParms void initSegmentBridgeWithBytesat(usqLong numBytes,
  ...
  if (numSlots) {
    longAtput((void *)(address),
              ((((usqInt)((numSlotsMask())) << (numSlotsFullShift())))) +
                  numSlots);
    longAtput((void *)(address + BaseHeaderSize),
              (((((((usqInt)((numSlotsMask())) << (numSlotsFullShift())))) +
                 (1U << (pinnedBitShift()))) +
                (1ULL << (markedBitFullShift()))) +
               ((((usqInt)((wordIndexableFormat())) << (formatShift()))))) +
                  (segmentBridgePun()));
  } else {
    longAtput((void *)(address),
...
static sqInt logBytesPerOop(void) { return 3; }
...
l1:
  return ((format == (firstLongFormat())) && (((indexableSize & 1) != 0))
              ? format + 1
              : format);
}
...
static NoDbgRegParms usqInt objectBytesForSlots(sqInt numSlots) {
  return (numSlots ? ((((usqInt)(numSlots) << (shiftForWord())))) +
...
static NoDbgRegParms void setClassIndexOfto(sqInt objOop, sqInt classIndex) {
  ...
  longAtput((void *)(objOop),
            ((longAt((void *)(objOop))) & (~(usqIntptr_t)(classIndexMask()))) +
                classIndex);
}
...
static NoDbgRegParms void setIsImmutableOfto(sqInt objOop, sqInt aBoolean) {
  longAtput((void *)(objOop),
            (aBoolean
                 ? (longAt((void *)(objOop))) | (1U << (immutableBitShift()))
                 : (longAt((void *)(objOop))) &
                       (~(usqIntptr_t)(1U << (immutableBitShift())))));
}

/*	Answer the ieee 754 double precision floating point bits of the
   argument, a SmallFloat. See section 61-bit Immediate Floats in the
   SpurMemoryManager class comment. msb lsb Decode:
   [8expsubset][52mantissa][1s][3tags] shift away tags:		[ 000
   ][8expsubset][52mantissa][1s] add exponent offset:	[ 11 exponent
   ][52mantissa][1s] rot sign:				[1s][ 11 exponent
   ][52mantissa] */
...
usqInt smallObjectBytesForSlots(sqInt numSlots) {
  return BaseHeaderSize +
         ((numSlots < 1 ? allocationUnit() : numSlots * BytesPerOop));
}
  ...
  if (!(objOop == (GIV(rememberedSet)[GIV(rememberedSetSize) - 1]))) {
    ...
    while (index < GIV(rememberedSetSize)) {
      if (objOop == (GIV(rememberedSet)[index])) {
        GIV(rememberedSet)
        [index] = (GIV(rememberedSet)[GIV(rememberedSetSize) - 1]);
...
static void growRememberedSet(void) {
  ...
  GIV(rememberedSetRedZone) = ((((GIV(rememberedSetLimit) * 3) / 4) < fudge)
                                   ? fudge
                                   : ((GIV(rememberedSetLimit) * 3) / 4));
}
...
newSpaceStartnewSpaceBytessurvivorBytes(sqInt startAddress, sqInt totalBytes,
  ...
  assert((((((GIV(eden)).start)) % (allocationUnit())) +
          ((((GIV(eden)).limit)) % (allocationUnit()))) == 0);
  assert((((((GIV(pastSpace)).start)) % (allocationUnit())) +
          ((((GIV(pastSpace)).limit)) % (allocationUnit()))) == 0);
  assert((((((GIV(futureSpace)).start)) % (allocationUnit())) +
          ((((GIV(futureSpace)).limit)) % (allocationUnit()))) == 0);
  initFutureSpaceStart();
  GIV(tenuringProportion) = 0.9;
}

/* SpurGenerationScavenger>>#openScavengeLog */
void openScavengeLog(void) {
  DECL_MAYBE_SQ_GLOBAL_STRUCT
...
static NoDbgRegParms void noteValidHeapExtent(sqInt bytesToShift) {
  ...
  GIV(oldHeapLimit) =
      ((((lastSegment()).segStart)) + (((lastSegment()).segSize))) -
      (((lastSegment()).swizzle));
}
...
static NoDbgRegParms sqInt swizzleObjin(sqInt objOop, sqInt containerOopOrNil) {
  ...
  if (!(oopisGreaterThanOrEqualToandLessThan(objOop, GIV(oldHeapBase),
                                             GIV(oldHeapLimit)))) {
    if (GIV(verbose)) {
      ...
      if (containerOopOrNil) {
        printf("Oop %p is out of bounds at/in %p\n", ((void *)objOop),
               ((void *)(containerOopOrNil - GIV(nilObj))));
      } else {
...
activeAndDeferredScan(sqInt anEphemeron) {
  ...
  if ((isImmediate((key = keyOfEphemeron(anEphemeron)))) || (isMarked(key))) {
    return 0;
  }
...
static NoDbgRegParms void addFreeSubTree(sqInt freeTree) {
  ...
  while (1) {
    ...
    if (bytesInNode > bytesInArg) {
      ...
      if (!subNode) {
        storePointerofFreeChunkwithValue(freeChunkSmallerIndex(), treeNode,
                                         freeTree);
        storePointerofFreeChunkwithValue(2 /* freeChunkParentIndex */, freeTree,
                                         treeNode);
        return;
      }
    } else {
      ...
      if (!subNode) {
        storePointerofFreeChunkwithValue(freeChunkLargerIndex(), treeNode,
                                         freeTree);
        storePointerofFreeChunkwithValue(2 /* freeChunkParentIndex */, freeTree,
                                         treeNode);
        return;
         ...
         ((oopisGreaterThanOrEqualTo(address, GIV(oldSpaceStart))
               ...
               : (isInEden(address)) ||
                     ((isInPastSpace(address)) ||
                      ((scavengeInProgress()) && (isInFutureSpace(address))))));
}

/* SpurMemoryManager>>#addressCouldBeOldObj: */
static NoDbgRegParms sqInt addressCouldBeOldObj(sqInt address) {
  return ((address & (BaseHeaderSize - 1)) == 0) && (isInOldSpace(address));
}
...
sqInt addressCouldBeOop(sqInt address) {
  return (isImmediate(address)) || (addressCouldBeObj(address));
}
...
static NoDbgRegParms sqInt addToFreeListbytes(sqInt freeChunk,
  ...
  if (index < (numFreeLists())) {
    setNextFreeChunkOfwithValuechunkBytes(freeChunk, GIV(freeLists)[index],
                                          chunkBytes);
    if (!(isLilliputianSize(chunkBytes))) {
      storePointerofFreeChunkwithValue(1 /* freeChunkPrevIndex */, freeChunk,
                                       0);
    }
...
/*	Add freeChunk to the large free chunk tree.
        ...
        to, if it is added to the next list of a freeTreeNode, otherwise answer
   0. */
...
static NoDbgRegParms sqInt addToFreeTreebytes(sqInt freeChunk,
  ...
  while (child != 0) {
    ...
    if (childBytes == chunkBytes) {
      nextFreeChunk =
          fetchPointerofFreeChunk(0 /* freeChunkNextIndex */, child);

      /* begin setNextFreeChunkOf:withValue:isLilliputianSize: */
      storePointerofFreeChunkwithValue(0 /* freeChunkNextIndex */, freeChunk,
                                       nextFreeChunk);
      if (nextFreeChunk) {
        storePointerofFreeChunkwithValue(1 /* freeChunkPrevIndex */,
      ...
      /* begin setNextFreeChunkOf:withValue:isLilliputianSize: */
      storePointerofFreeChunkwithValue(0 /* freeChunkNextIndex */, child,
                                       freeChunk);
      if (freeChunk) {
        storePointerofFreeChunkwithValue(1 /* freeChunkPrevIndex */, freeChunk,
                                         child);
  ...
  /* insert in tree */
  storePointerofFreeChunkwithValue(2 /* freeChunkParentIndex */, freeChunk,
                                   parent);
...
static NoDbgRegParms void adjustAllOopsBy(sqInt bytesToShift) {
  ...
  if ((bytesToShift != 0) || (GIV(numSegments) > 1)) {
      ...
      if (classIndex >= (isForwardedObjectClassIndexPun())) {
        ...
        while (oopisGreaterThanOrEqualTo(fieldAddr, obj + BaseHeaderSize)) {
          ...
          if (isNonImmediate(fieldOop)) {
            longAtput((void *)(fieldAddr), swizzleObjin(fieldOop, obj));
          }
        ...
        if (classIndex == (isFreeObjectClassIndexPun())) {
          ...
          if (field) {
            storeUncheckedNoAssertofAnywithValue(0 /* freeChunkNextIndex */,
                                                 obj, swizzleObjin(field, obj));
          }
          ...
          if (chunkBytes >= ((numFreeLists()) * (allocationUnit()))) {
            for (index = 2 /* freeChunkParentIndex */;
                 index <= (freeChunkLargerIndex()); index += 1) {
              field = fetchPointerofFreeChunk(index, obj);
              if (field) {
                storeUncheckedNoAssertofAnywithValue(index, obj,
                                                     swizzleObjin(field, obj));
              }
            }
          }
        }
...
static NoDbgRegParms sqInt allocateOldSpaceChunkOfBytes(usqInt chunkBytes) {
    ...
    while ((((index += index)) < (numFreeLists())) &&
      ...
      if (((GIV(freeListsMask) & (1ULL << index)) != 0)) {
        if ((chunk = GIV(freeLists)[index])) {
          ...
          /* For some reason the assertion is not compiled correctly */
          GIV(freeLists)
          [index] = ((next = fetchPointerofFreeChunk(0 /* freeChunkNextIndex */,
                                                     chunk)));
          if (next) {
            storePointerofFreeChunkwithValue(1 /* freeChunkPrevIndex */, next,
                                             0);
          }
          assert((bytesInBody(chunk)) == (index * (allocationUnit())));
          freeChunkWithBytesat((index * (allocationUnit())) - chunkBytes,
                               (startOfObject(chunk)) + chunkBytes);
          return chunk;
        }
        GIV(freeListsMask) -= 1ULL << index;
      }
  ...
  while (child != 0) {
    ...
    if (childBytes == chunkBytes) {
      ...
      if (chunk) {
        ...
        assert(assertInnerValidFreeObject(chunk));
        nextFreeChunk =
            fetchPointerofFreeChunk(0 /* freeChunkNextIndex */, chunk);
  ...
  if (chunk) {
    ...
    /* begin setNextFreeChunkOf:withValue:isLilliputianSize: */
    storePointerofFreeChunkwithValue(0 /* freeChunkNextIndex */, parent,
                                     nextFreeChunk);
    if (nextFreeChunk) {
      storePointerofFreeChunkwithValue(1 /* freeChunkPrevIndex */,
                                       nextFreeChunk, parent);
    }
    if (chunkBytes != nodeBytes) {
      freeChunkWithBytesat(nodeBytes - chunkBytes,
                           (startOfObject(chunk)) + chunkBytes);
    }
    return startOfObject(chunk);
  }
...
sqInt allocatePinnedSlots(sqInt nSlots) {
  ...
  obj = allocateSlotsForPinningInOldSpacebytesformatclassIndex(
      nSlots, objectBytesForSlots(nSlots), wordIndexableFormat(),
...
assertInnerValidFreeObject(sqInt objOop) {
  ...
  if (!(isLilliputianSize(bytesInBody(objOop)))) {
    ...
    if ((index < (numFreeLists())) && ((1ULL << index) <= GIV(freeListsMask))) {
      ...
      do {
        ...
        if (((smallChild == 0) && (largeChild == 0)) ||
          ...
          cameFrom = treeNode;
          treeNode =
              fetchPointerofFreeChunk(2 /* freeChunkParentIndex */, treeNode);
        } else {
...
static NoDbgRegParms sqInt becomeEffectFlagsFor(sqInt objOop) {
  ...
  if (isPointersNonImm(objOop)) {
    ...
    return (isCompiledMethod(objOop) ? BecameCompiledMethodFlag : 0);
  }
}
...
static sqInt bitsSetInFreeSpaceMaskForAllFreeLists(void) {
  ...
  for (i = 0; i < (numFreeLists()); i += 1) {
    if (((GIV(freeLists)[i]) != 0) && ((!((1ULL << i) & GIV(freeListsMask))))) {
      return 0;
    }
  }
  return 1;
...
static sqInt bridgeSize(void) { return 2 * BaseHeaderSize; }
...
static NoDbgRegParms void checkFreeSpace(sqInt gcModes) {
  ...
  if ((gcModes > 0) && (((checkForLeaks & (GCCheckFreeSpace | gcModes)) ==
                         (GCCheckFreeSpace | gcModes)))) {
  }
}
...
static NoDbgRegParms sqInt checkHeapIntegrityclassIndicesShouldBeValid(
    ...
    if ((freeListOop = GIV(freeLists)[i])) {
      if (isInOldSpace(freeListOop)) {
        if (heapMapAtWord(pointerForOop(freeListOop))) {
          fprintf(GIV(transcript), "leak in free list %d to non-free %p\n",
                  ((int)i), ((void *)freeListOop));
          ok = 0;
        }
      } else {
        fprintf(GIV(transcript), "leak in free list %d to out-of-bounds %p\n",
                ((int)i), ((void *)freeListOop));
        ok = 0;
      }
    }
  ...
  if (GIV(pastSpaceStart) < (((GIV(eden)).start))) {
    if ((GIV(pastSpaceStart) + BaseHeaderSize) == (((GIV(eden)).start))) {
      hackSlimBridgeToat(objectStartingAt(((GIV(eden)).start)),
                         GIV(pastSpaceStart));

      /* And carefully check the assumption */
      assert((objectAfterMaybeSlimBridgelimit(
                 objectInPastSpaceBefore(GIV(pastSpaceStart)), GIV(nilObj))) ==
             (objectStartingAt(((GIV(eden)).start))));
    } else {
      initSegmentBridgeWithBytesat((((GIV(eden)).start)) - GIV(pastSpaceStart),
                                   GIV(pastSpaceStart));
    }
  }
  ...
  while (oopisLessThan(objOop, GIV(freeStart))) {
    if (isFreeObject(objOop)) {
      fprintf(GIV(transcript), "young object %p is free\n", ((void *)objOop));
      ok = 0;
    } else {
      if (!((!(isMarked(objOop))) && (excludeUnmarkedObjs))) {
        if (isRemembered(objOop)) {
          fprintf(GIV(transcript), "young object %p is remembered\n",
                  ((void *)objOop));
          ok = 0;
        }
      }
      if ((!((longAt((void *)(objOop))) &
        ...
        if (classIndicesShouldBeValid &&
            ((classOop == GIV(nilObj)) && (!(isHiddenObj(objOop))))) {
          fprintf(GIV(transcript),
        ...
        for (fi = 0; fi <= toDoLimit; fi += 1) {
          ...
          if (isNonImmediate(fieldOop)) {
            if (isInHeapBounds(fieldOop)) {
              if (!(heapMapAtWord(pointerForOop(fieldOop)))) {
                fprintf(GIV(transcript), "object leak in %p @ %d = %p\n",
...
l3:
    ...
    if (isFreeObject(objOop)) {
      ...
      if (!(isLilliputianSize(bytesInBody(objOop)))) {
        ...
        if ((fieldOop != 0) &&
          ...
          fprintf(GIV(transcript), "leak in free chunk %p @ 1 = %p is mapped\n",
                  ((void *)objOop), ((void *)fieldOop));
          ok = 0;
        }
      }
      if (isLargeFreeObject(objOop)) {
        for (fi = 2 /* freeChunkParentIndex */; fi <= (freeChunkLargerIndex());
             fi += 1) {
      ...
      if (!(excludeUnmarkedObjs && (!(isMarked(objOop))))) {
        containsYoung = 0;
        if (isRemembered(objOop)) {
  ...
  if (numRememberedObjectsInHeap != GIV(rememberedSetSize)) {
    fprintf(GIV(transcript), "root count mismatch. #heap roots %d; #roots %d\n",
            ((int)numRememberedObjectsInHeap), ((int)GIV(rememberedSetSize)));
  ...
  for (iSqInt = 0; iSqInt < GIV(rememberedSetSize); iSqInt += 1) {
    ...
    if (obj & (BytesPerWord - 1)) {
      fprintf(GIV(transcript), "misaligned oop in remembered set @ %d = %p\n",
              ((int)iSqInt), ((void *)obj));
      ok = 0;
    } else {
      if (heapMapAtWord(pointerForOop(obj))) {
        if (isYoung(obj)) {
          fprintf(GIV(transcript), "non-root in remembered set @ %d = %p\n",
                  ((int)iSqInt), ((void *)obj));
          ok = 0;
        }
      } else {
        fprintf(GIV(transcript), "object leak in remembered set @ %d = %p\n",
                ((int)iSqInt), ((void *)obj));
        ok = 0;
      }
    }
    ...
    for (iSqInt = ((numOnThisPage + ObjStackFixedSlots) - 1);
      ...
      if (obj & (BytesPerWord - 1)) {
        fprintf(GIV(transcript),
                ...
                ((void *)objStackPage), ((void *)obj));
        ok = 0;
      } else {
        if (!(excludeUnmarkedObjs && (!(isMarked(obj))))) {
          if (!(heapMapAtWord(pointerForOop(obj)))) {
            fprintf(GIV(transcript),
...
l1:
  for (ri = 1; ri <= GIV(remapBufferCount); ri += 1) {
    ...
    if (obj & (BytesPerWord - 1)) {
      fprintf(GIV(transcript), "misaligned remapRoot @ %d = %p\n", ((int)ri),
              ((void *)obj));
      ok = 0;
    } else {
      if (!(heapMapAtWord(pointerForOop(obj)))) {
        fprintf(GIV(transcript), "object leak in remapRoots @ %d = %p\n",
                ((int)ri), ((void *)obj));
        ok = 0;
      }
    }
  ...
  for (ri = 1; ri <= GIV(extraRootCount); ri += 1) {
    ...
    if (obj & (BytesPerWord - 1)) {
      fprintf(GIV(transcript), "misaligned extraRoot @ %d => %p\n", ((int)ri),
              ((void *)obj));
      ok = 0;
    } else {
      if (!(heapMapAtWord(pointerForOop(obj)))) {
        fprintf(GIV(transcript), "object leak in extraRoots @ %d => %p\n",
                ((int)ri), ((void *)obj));
        ok = 0;
      }
    }
...
sqInt checkOkayOop(usqInt oop) {
  ...
  if (!(addressCouldBeObj(oop))) {
    fprintf(GIV(transcript), "oop %p is not a valid address", ((void *)oop));
    return 0;
  }
  ...
  if (!(((classIndex = classIndexOf(oop))) >= (firstClassIndexPun()))) {
    fprintf(GIV(transcript), "oop %p is a free chunk, or bridge, not an object",
            ((void *)oop));
    return 0;
  }
  ...
  fmt = (byteAt((void *)(oop + (formatFieldByteOffset())))) & (formatMask());
  if ((fmt == 6) || (fmt == 8)) {
    fprintf(GIV(transcript), "oop %p has an unknown format type",
            ((void *)oop));
    return 0;
  }
  ...
  unusedBits = (1U << (classIndexFieldWidth())) |
               (1ULL << ((identityHashFieldWidth()) + 32));
...
sqInt checkOopHasOkayClass(usqInt obj) {
  ...
  if (!(okayOop(objClass))) {
    fprintf(GIV(transcript), "obj %p class obj is not ok\n", ((void *)obj));
    return 0;
  }
  if (!((isPointersNonImm(objClass)) && ((numSlotsOf(objClass)) >= 3))) {
    fprintf(
        GIV(transcript),
  ...
  objFormat =
      (isBytes(obj) ? ((((byteAt((void *)(obj + (formatFieldByteOffset())))) &
                         (formatMask())) |
                        7) -
                       7)
  ...
  if ((instSpecOfClass(objClass)) != objFormat) {
    fprintf(GIV(transcript), "obj %p and its class (behavior) formats differ\n",
            ((void *)obj));
    return 0;
  }
  return 1;
...
sqInt classAtIndex(sqInt classIndex) {
  ...
  classTablePage = fetchPointerofObject(((usqInt)(classIndex)) >>
                                            (classTableMajorIndexShift()),
                                        GIV(hiddenRootsObj));
  if (classTablePage == GIV(nilObj)) {
    return null;
  }
  return fetchPointerofObject(classIndex & (classTableMinorIndexMask()),
                              classTablePage);
}
...
static sqInt classDoubleByteArray(void) {
  ...
  if ((isImmediate(classOop)) || ((((hash = rawHashBitsOf(classOop))) == 0) ||
                                  ((classAtIndex(hash)) != classOop))) {
    return GIV(nilObj);
  }
  GIV(validatedIntegerClassFlags) =
      GIV(validatedIntegerClassFlags) | ValidatedClassDoubleByteArrayFlag;
  return classOop;
}
...
sqInt classDoubleWordArray(void) {
  ...
  GIV(validatedIntegerClassFlags) =
      GIV(validatedIntegerClassFlags) | ValidatedClassDoubleWordArrayFlag;
  return classOop;
}
...
sqInt classWordArray(void) {
  ...
  GIV(validatedIntegerClassFlags) =
      GIV(validatedIntegerClassFlags) | ValidatedClassWordArrayFlag;
  return classOop;
}
...
static NoDbgRegParms sqInt cloneInOldSpaceforPinning(sqInt objOop,
  ...
  if (forPinning) {
    newObj = allocateSlotsForPinningInOldSpacebytesformatclassIndex(
        numSlots, objectBytesForSlots(numSlots), fmt, classIndexOf(objOop));
  } else {
  ...
  if (isPointersFormat(fmt)) {
    ...
    for (i = 0; i < numSlots; i += 1) {
      ...
      if ((isNonImmediate(oop)) && (isYoungObject(oop))) {
        hasYoung = 1;
      }
    ...
    if (fmt >= (firstCompiledMethodFormat())) {
      if ((isYoungObject(objOop)) || (isRemembered(objOop))) {
        remember(newObj);
      }
    }
  }
...
static NoDbgRegParms sqInt cloneObject(sqInt objOop) {
  ...
  if (numSlots > (maxSlotsForNewSpaceAlloc())) {
    ...
    newObj = allocateSlotsInOldSpacebytesformatclassIndex(
        numSlots, objectBytesForSlots(numSlots), fmt, classIndex);
  } else {
    classIndex = classIndexOf(objOop);
  ...
  if (newObj) {
    if (isPointersFormat(fmt)) {
      ...
      if (hasYoung && (!(isYoungObject(newObj)))) {
        remember(newObj);
      }
    } else {
      for (i = 0; i < numSlots; i += 1) {
      ...
      if (fmt >= (firstCompiledMethodFormat())) {
        if ((isOldObject(newObj)) &&
            ((isYoungObject(objOop)) || (isRemembered(objOop)))) {
          remember(newObj);
        }
      }
    }
...
/*	Compute the used size of the class table before swizzling. Needed to
        initialize the classTableBitmap which is populated during
   adjustAllOopsBy: */
...
static NoDbgRegParms void countNumClassPagesPreSwizzle(sqInt bytesToShift) {
  ...
  /* a.k.a. nilObj */
  /* first five objects are nilObj, falseObj, trueObj, freeListsObj,
   * hiddenRootsObj */
  ...
  assert(GIV(numClassTablePages) ==
         ((classTableRootSlots()) + (hiddenRootSlots())));
...
static NoDbgRegParms sqInt doShortentoIndexableSize(sqInt objOop,
  ...
  if ((delta <= (allocationUnit())) && (isOldObject(objOop))) {
  ...
  setFormatOfto(objOop,
                normalisedFormatForindexableSize(objOop, indexableSize));
  return delta;
}
...
usqInt eeInstantiateClassIndexformatnumSlots(sqInt knownClassIndex,
  ...
  if (numSlots >= (numSlotsMask())) {
    ...
    longAtput((void *)(GIV(freeStart) + 4),
...
static NoDbgRegParms void enableObjectEnumerationFrom(sqInt initialObject) {
  ...
  if (oopisLessThan(initialObject, GIV(oldSpaceStart))) {
    if (GIV(freeStart) > (((GIV(eden)).start))) {
      ...
      initSegmentBridgeWithBytesat(GIV(oldSpaceStart) - GIV(freeStart),
                                   GIV(freeStart));
      if (GIV(pastSpaceStart) < (((GIV(eden)).start))) {
        if ((GIV(pastSpaceStart) + BaseHeaderSize) == (((GIV(eden)).start))) {
          hackSlimBridgeToat(objectStartingAt(((GIV(eden)).start)),
          ...
          /* And carefully check the assumption */
          assert((objectAfterMaybeSlimBridgelimit(
                     objectInPastSpaceBefore(GIV(pastSpaceStart)),
                     GIV(nilObj))) == (objectStartingAt(((GIV(eden)).start))));
        } else {
          initSegmentBridgeWithBytesat(
              (((GIV(eden)).start)) - GIV(pastSpaceStart), GIV(pastSpaceStart));
        }
      }
    } else {
      /* begin bridgePastSpaceAndOldSpace */
      initSegmentBridgeWithBytesat(GIV(oldSpaceStart) - GIV(pastSpaceStart),
                                   GIV(pastSpaceStart));
    }
  }
}
  ...
  if ((stackOrNil == GIV(nilObj)) ||
    ...
    if (freeOrNewPage) {
      ...
      longAtput((void *)((stackOrNil + BaseHeaderSize) +
                ...
                0);
      assert((!GIV(marking)) || (isMarked(freeOrNewPage)));
    } else {
      ...
      freeOrNewPage = allocateSlotsInOldSpacebytesformatclassIndex(
          ObjStackPageSlots, objectBytesForSlots(ObjStackPageSlots),
          formatField, classIndex);
      if (!freeOrNewPage) {
    ...
    longAtput((void *)((freeOrNewPage + BaseHeaderSize) +
              ...
              (stackOrNil == GIV(nilObj) ? 0 : stackOrNil));
    ...
    longAtput((void *)((freeOrNewPage + BaseHeaderSize) +
              ...
              0);
    storePointerofObjectwithValue(objStackRootIndex, GIV(hiddenRootsObj),
                                  freeOrNewPage);
...
static NoDbgRegParms sqInt enterIntoClassTable(sqInt aBehavior) {
  ...
  majorIndex =
      ((usqInt)(GIV(classTableIndex))) >> (classTableMajorIndexShift());
  ...
  while (1) {
    ...
    if (page == GIV(nilObj)) {
      ...
      page = allocateSlotsInOldSpacebytesformatclassIndex(
          numSlots, objectBytesForSlots(numSlots), arrayFormat(),
          arrayClassIndexPun());
      if (!page) {
      ...
      assert(oopisLessThan(
          ((page + BaseHeaderSize) + (numSlotsSqInt * BytesPerOop)) - 1,
          addressAfter(page)));
      toDoLimitUsqInt =
          ((usqInt)(((page + BaseHeaderSize) + (numSlotsSqInt * BytesPerOop)) -
                    1));
...
static NoDbgRegParms sqLong fetchLong64ofObject(sqInt longIndex, sqInt objOop) {
  return long64At(
      (void *)((objOop + BaseHeaderSize) + ((((usqInt)(longIndex) << 3)))));
}
...
static NoDbgRegParms sqInt fetchShort16ofObject(sqInt shortIndex,
                                                sqInt objOop) {
  return shortAt(
      (void *)((objOop + BaseHeaderSize) + ((((usqInt)(shortIndex) << 1)))));
}
...
void findStringBeginningWith(char *aCString) {
  ...
  while (1) {
    ...
    if (!(oopisLessThan(objSqInt, GIV(endOfMemory))))
      break;
    ...
    if (fmt >= (indexablePointersFormat())) {
      ...
      classFormat =
          ((fetchPointerofObject(InstanceSpecificationIndex, classPointer)) >>
           3);
      return pointerForOop((objOop + BaseHeaderSize) +
                           ((((usqInt)((fixedFieldsOfClassFormat(classFormat)))
                              << (shiftForWord())))));
    }
...
static NoDbgRegParms sqInt fixedFieldsOfClass(sqInt objOop) {
  return fixedFieldsOfClassFormat(
      ((fetchPointerofObject(InstanceSpecificationIndex, objOop)) >> 3));
}
...
l2:
  for (i = 0; i < numSlots; i += 1) {
    ...
    if (isNonImmediate(oop)) {
      ...
      if ((depth > 0) &&
          ((/* hasPointerFields: */
            (isNonImmediate(oop)) && (hasPointerFieldsNonImm(oop))) &&
...
static void followSpecialObjectsOop(void) {
  DECL_MAYBE_SQ_GLOBAL_STRUCT
  if ((!((longAt((void *)(GIV(specialObjectsOop)))) &
         ((classIndexMask()) - (isForwardedObjectClassIndexPun()))))) {
...
static sqInt freeChunkLargerIndex(void) { return 4; }
...
static sqInt freeChunkSmallerIndex(void) { return 3; }
...
static sqInt identityHashFieldWidth(void) { return 22; }
...
static NoDbgRegParms void inFreeTreeReplacewith(sqInt treeNode, sqInt newNode) {
  ...
  for (i = 2 /* freeChunkParentIndex */; i <= (freeChunkLargerIndex());
       i += 1) {
    relative = fetchPointerofFreeChunk(i, treeNode);
    if (i == 2 /* freeChunkParentIndex */) {
      if (relative) {
        storePointerofFreeChunkwithValue(
            (treeNode == (fetchPointerofFreeChunk(freeChunkSmallerIndex(),
                                                  relative))
                 ? freeChunkSmallerIndex()
                 : freeChunkLargerIndex()),
            relative, newNode);
      ...
      if (relative) {
        assert((fetchPointerofFreeChunk(freeChunkParentIndex(), relative)) ==
               treeNode);
        storePointerofFreeChunkwithValue(2 /* freeChunkParentIndex */, relative,
                                         newNode);
      }
    }
...
/*	Reinitialize the free list info. The freeLists object needs to be
   swizzled because its neither a free, nor a pointer object. Free objects have
...
static NeverInline void initializeNewSpaceVariables(void) {
  ...
  GIV(scavengeThreshold) = ((((GIV(eden)).limit)) -
                            ((((GIV(eden).limit)) - ((GIV(eden).start))) / 64));
  GIV(newSpaceStart) =
      (((((GIV(pastSpace)).start)) < (((GIV(futureSpace)).start)))
           ? (((GIV(pastSpace)).start))
           : (((GIV(futureSpace)).start)));
...
static NoDbgRegParms void initializeObjectMemory(sqInt bytesToShift) {
  ...
  GIV(heapSizeAtPreviousGC) =
      (totalOldSpaceCapacity()) - GIV(totalFreeOldSpace);
  GIV(oldSpaceUsePriorToScavenge) =
      (totalOldSpaceCapacity()) - GIV(totalFreeOldSpace);
...
void inOrderPrintFreeTreeprintList(sqInt freeChunk, sqInt printNextList) {
  ...
  if (printNextList) {
    next = freeChunk;
    while (((next = fetchPointerofFreeChunk(0 /* freeChunkNextIndex */,
                                            next))) != 0) {
      /* begin tab */
      printChar('\t');
...
static NoDbgRegParms int isArrayNonImm(sqInt oop) {
  return ((byteAt((void *)(oop + (formatFieldByteOffset())))) &
...
static NoDbgRegParms sqInt isArray(sqInt oop) {
  return (isNonImmediate(oop)) && (isArrayNonImm(oop));
}
...
static NoDbgRegParms sqInt isBytes(sqInt oop) {
  return (isNonImmediate(oop)) && (isBytesNonImm(oop));
}
...
static NoDbgRegParms sqInt isInEden(sqInt objOop) {
  DECL_MAYBE_SQ_GLOBAL_STRUCT
  return oopisGreaterThanOrEqualToandLessThan(objOop, ((GIV(eden)).start),
                                              GIV(freeStart));
}
...
static NoDbgRegParms sqInt isInFutureSpace(sqInt address) {
  ...
  return oopisGreaterThanOrEqualToandLessThan(
      address, ((GIV(futureSpace)).start), GIV(futureSurvivorStart));
}
...
sqInt isInOldSpace(sqInt address) {
  DECL_MAYBE_SQ_GLOBAL_STRUCT
  return oopisGreaterThanOrEqualToandLessThan(address, GIV(oldSpaceStart),
                                              GIV(endOfMemory));
}
...
static NoDbgRegParms sqInt isInPastSpace(sqInt address) {
  DECL_MAYBE_SQ_GLOBAL_STRUCT
  return oopisGreaterThanOrEqualToandLessThan(address, ((GIV(pastSpace)).start),
                                              GIV(pastSpaceStart));
}
...
sqInt isLong64s(sqInt oop) {
  return (isNonImmediate(oop)) && (isLong64sNonImm(oop));
}
...
sqInt isOopImmutable(sqInt oop) {
  return (isImmediate(oop)) || (isImmutable(oop));
}

/* SpurMemoryManager>>#isOopMutable: */
sqInt isOopMutable(sqInt oop) {
  return (isNonImmediate(oop)) && (!(isImmutable(oop)));
}
...
static NoDbgRegParms sqInt isPointers(sqInt oop) {
  return (isNonImmediate(oop)) && (isPointersNonImm(oop));
}
...
int isReallyYoungObject(sqInt obj) { return isYoungObject(obj); }

/*	Answer if oop is young. */

/* SpurMemoryManager>>#isReallyYoung: */
static NoDbgRegParms sqInt isReallyYoung(sqInt oop) {
  return (isNonImmediate(oop)) && (isReallyYoungObject(oop));
}
...
sqInt isShorts(sqInt oop) {
  return (isNonImmediate(oop)) && (isShortsNonImm(oop));
}
...
int isUnmarked(sqInt objOop) { return !(isMarked(objOop)); }
...
static NoDbgRegParms sqInt isValidObjStackAt(sqInt objStackRootIndex) {
  ...
  stackOrNil = fetchPointerofObject(objStackRootIndex, GIV(hiddenRootsObj));
  return (stackOrNil == GIV(nilObj)) || (isValidObjStackPagemyIndexfirstPage(
...
static NoDbgRegParms sqInt isValidObjStackPagemyIndex(sqInt objStackPage,
  ...
  if (!(((byteAt((void *)(objStackPage + (formatFieldByteOffset())))) &
         (formatMask())) == (wordIndexableFormat()))) {
  ...
  if (GIV(marking) && (!(isMarked(objStackPage)))) {
...
static NoDbgRegParms sqInt isValidObjStackPagemyIndexfirstPage(
  ...
  while (freeOrNextPage != 0) {
    ...
    if (!(isValidObjStackPagemyIndex(freeOrNextPage, myx))) {
      GIV(objStackInvalidBecause) =
          stretchcat(GIV(objStackInvalidBecause), ", on next page");
      return 0;
    }
  ...
  if (isFirstPage) {
    if (!(((myx >= (classTableRootSlots())) &&
           (myx <= (((classTableRootSlots()) + (hiddenRootSlots())) - 1))))) {
...
static NoDbgRegParms sqInt isWeak(sqInt oop) {
  return (isNonImmediate(oop)) && (isWeakNonImm(oop));
}
...
static NoDbgRegParms sqInt isWordsOrBytes(sqInt oop) {
  return (isNonImmediate(oop)) && (isWordsOrBytesNonImm(oop));
}

/*	Answer if the argument contains only indexable 16-bit half words or
   32-bit words (no oops). See comment in formatOf: */
...
sqInt isWordsOrShorts(sqInt oop) {
  return (isNonImmediate(oop)) && (isWordsOrShortsNonImm(oop));
}
...
static NoDbgRegParms sqInt isWords(sqInt oop) {
  return (isNonImmediate(oop)) && (isWordsNonImm(oop));
}
...
sqInt isYoung(sqInt oop) {
  DECL_MAYBE_SQ_GLOBAL_STRUCT
  return (isNonImmediate(oop)) && (oopisLessThan(oop, GIV(oldSpaceStart)));
}
    ...
    assert(
        ...
        ((classIndex == (isForwardedObjectClassIndexPun())) ||
         (((long64At((void *)(objSqInt))) != 0) &&
          (classIndex < (GIV(numClassTablePages) * (classTablePageSize()))))));
    if (classIndex >= (isForwardedObjectClassIndexPun())) {
      if ((isPointersNonImm(objSqInt)) || (isCompiledMethod(objSqInt))) {
...
static NoDbgRegParms void markAndTraceClassOf(sqInt objOop) {
  ...
  if (!(isMarked(classObj))) {
    ...
    if (isImmediate(classObj)) {
      ...
      assert(!((objStack == GIV(markStack)) && (isWeakNonImm(classObj))));

      /* There should only be weaklings on the weaklingStack */
      assert((objStack != GIV(weaklingStack)) || (isWeakNonImm(classObj)));
    }
...
NeverInline void markAndTrace(sqInt objOop) {
  ...
  if (format == (weakArrayFormat())) {
    ...
    if (isImmediate(objOop)) {
      ...
      assert(!((objStack == GIV(markStack)) && (isWeakNonImm(objOop))));

      /* There should only be weaklings on the weaklingStack */
      assert((objStack != GIV(weaklingStack)) || (isWeakNonImm(objOop)));
    }
        ...
        if (isNonImmediate(field)) {
          ...
          if (format == (weakArrayFormat())) {
            ...
            if (isImmediate(field)) {
              ...
              assert(!((objStack1 == GIV(markStack)) && (isWeakNonImm(field))));
      ...
      while (index > 0) {
        ...
        if (isNonImmediate(field)) {
          ...
          if (isImmediate(field)) {
            ...
            /* There should only be weaklings on the weaklingStack */
            assert((objStack1 != GIV(weaklingStack)) || (isWeakNonImm(field)));
          }
          ...
          if (((byteAt((void *)(field + (numSlotsFieldByteOffset())))) >
            ...
            if (isImmediate(objOop1)) {
              ...
              assert(
                  !((objStack1 == GIV(markStack)) && (isWeakNonImm(objOop1))));
...
sqInt maxSlotsForNewSpaceAlloc(void) { return fixedFieldsOfClassFormatMask(); }
...
/*	The three bit field { UNUSED, isPinned, isRemembered } is for bits
...
static NoDbgRegParms sqInt noCheckPushonObjStack(sqInt objOop, sqInt objStack) {
  ...
  if (topx >= ObjStackLimit) {
    noCheckPushonObjStack(objOop, ensureRoomOnObjStackAt(fetchPointerofObject(
                                      ObjStackMyx, objStack)));
  } else {
    /* begin storePointer:ofObjStack:withValue: */
...
sqInt objectAfter(sqInt objOop) {
  ...
  if (oopisLessThan(objOop, GIV(oldSpaceStart))) {
    ...
    if (isInPastSpace(objOop)) {
      /* begin objectAfter:limit: */
      followingWordAddress = addressAfter(objOop);
      if (oopisGreaterThanOrEqualTo(followingWordAddress,
                                    GIV(pastSpaceStart))) {
...
sqInt objectBefore(sqInt objOop) {
  ...
  while (1) {
    ...
    if (!(oopisLessThan(objOopSqInt, GIV(endOfMemory))))
      break;
...
static NoDbgRegParms void printBridgeon(sqInt oop, FILE *aStream) {
  fprintf(aStream, "%p is a bridge hdr%d slot size %" PRIuSQINT "\n",
          ((void *)oop), (hasOverflowHeader(oop) ? 16 : 8), numSlotsOfAny(oop));
}
...
static NoDbgRegParms void printCantBeObjecton(sqInt oop, FILE *aStream) {
  ...
  fprintf(aStream, "%p%s\n", ((void *)oop),
          (oop & ((allocationUnit()) - 1) ? " is misaligned" : whereIs(oop)));
}
...
void printEntity(sqInt oop) {
  ...
  printChar(' ');
  if (!(addressCouldBeObj(oop))) {
    print((isImmediate(oop) ? "immediate" : "unknown"));
    return;
  }
  ...
  /* begin space */
  printChar(' ');
  printHexnpnp(byteAt((void *)(oop + (numSlotsFieldByteOffset()))));
  print("/");
  ...
  if (printFlags) {
    ...
    print((((byteAt((void *)(oop + (formatFieldByteOffset())))) &
            (formatMask())) <= 15
               ? "f:0"
               : "f:"));
    printHexnpnp((byteAt((void *)(oop + (formatFieldByteOffset())))) &
                 (formatMask()));
    print((isGrey(oop) ? " g" : " ."));
    print((isImmutable(oop) ? "i" : "."));
    print((isMarked(oop) ? "m" : "."));
    print((isPinned(oop) ? "p" : "."));
    print((isRemembered(oop) ? "r" : "."));
  }
  cr();
}
...
static NoDbgRegParms void printFreeChunkprintAsTreeNode(sqInt freeChunk,
  ...
  if ((numBytes >= ((numFreeLists()) * (allocationUnit()))) &&
    ...
    print(" ^ ");
    printHexnp(
        fetchPointerofFreeChunk(2 /* freeChunkParentIndex */, freeChunk));
    print(" < ");
...
sqInt printFreeList(sqInt chunkOrIndex) {
  ...
  if ((chunkOrIndex >= 0) && (chunkOrIndex < (numFreeLists()))) {
...
static NoDbgRegParms void printFreeTreeChunk(sqInt chunkOrZero) {
  if (chunkOrZero > 0) {
    printFreeTreeChunk(
        fetchPointerofFreeChunk(freeChunkSmallerIndex(), chunkOrZero));
    printFreeChunk(chunkOrZero);
    printFreeTreeChunk(
        fetchPointerofFreeChunk(freeChunkLargerIndex(), chunkOrZero));
  }
}
...
void printHeaderOf(sqInt objOop) {
  ...
  if (hasOverflowHeader(objOop)) {
    print(" hdr16 slotf ");
    printHexnp(byteAt((void *)(objOop + (numSlotsFieldByteOffset()))));
    print(" slotc ");
    printHexnp(((((usqInt)((
                   (sqInt)((usqInt)((longAt((void *)(objOop - BaseHeaderSize))))
                           << 8)))))) >>
  ...
  /* begin space */
  printChar(' ');
  printChar((isMarked(objOop) ? 'M' : 'm'));
  printChar((isGrey(objOop) ? 'G' : 'g'));
  printChar((isPinned(objOop) ? 'P' : 'p'));
  printChar((isRemembered(objOop) ? 'R' : 'r'));
  printChar((isImmutable(objOop) ? 'I' : 'i'));
  print(" hash ");
  printHexnp(rawHashBitsOf(objOop));
  print(" fmt ");
  printHexnp((byteAt((void *)(objOop + (formatFieldByteOffset())))) &
             (formatMask()));
  print(" cidx ");
...
l1:
  ...
  elementsPerLine = 16;
  for (index = 1; index <= lastIndex; index += 1) {
    fprintf(aStream, "%4p%c", ((void *)(fetchByteofObject(index - 1, oop))),
            (((index % elementsPerLine) == 0) || (index == lastIndex) ? '\n'
                                                                      : ' '));
  }
}
...
void printObjectsFromto(sqInt startAddress, sqInt endAddress) {
  ...
  while (oopisLessThan(oop, endAddress)) {
    if (!((isFreeObject(oop)) || (isSegmentBridge(oop)))) {
      printOop(oop);
    }
    oop = objectAfter(oop);
  }
...
printObjStackPagemyIndextag(sqInt objStackPage, sqInt myx, char *pageType) {
  ...
  fprintf(GIV(transcript),
          ...
          ((void *)objStackPage), ((int)(classIndexOf(objStackPage))),
          ((int)(wordSizeClassIndexPun())),
          ((int)((byteAt((void *)(objStackPage + (formatFieldByteOffset())))) &
                 (formatMask()))),
          ((int)(wordIndexableFormat())), ((int)(numSlotsOfAny(objStackPage))),
          ObjStackPageSlots,
          ((int)(fetchPointerofObject(ObjStackMyx, objStackPage))), ((int)myx),
          (isMarked(objStackPage) ? ") mkd" : ") unmkd"));
}
...
void printOopsFromto(sqInt startAddress, sqInt endAddress) {
  ...
  firstNonEntity = 0;
  oop = objectBefore(startAddress);
  limit = (((((usqIntptr_t)endAddress)) < GIV(endOfMemory))
...
void printReferencesTo(sqInt anOop) {
  ...
  if ((isRemembered(anOop)) && (isInRememberedSet(anOop))) {
...
static NoDbgRegParms sqInt pushOnUnscannedEphemeronsStack(sqInt anEphemeron) {
  ...
  longAtput((void *)((GIV(unscannedEphemerons).top =
                          ((GIV(unscannedEphemerons).top)) + BytesPerOop)),
            anEphemeron);
  return 1;
}
...
static NoDbgRegParms void rememberedSetObj(sqInt anObj) {
  ...
  assert(validStorePointerUncheckedArgs(RememberedSetRootIndex,
                                        GIV(hiddenRootsObj), anObj));
  longAtput(
      (void *)((GIV(hiddenRootsObj) + BaseHeaderSize) +
...
static NoDbgRegParms void reverseBytesFromto(sqInt startAddr, sqInt stopAddr) {
  ...
  while (oopisLessThan(addr, stopAddr)) {
    longAtput((void *)(addr), byteSwapped(longAt((void *)(addr))));
    addr += BytesPerWord;
  }
}
...
static NoDbgRegParms usqInt roundUpHeapSize(usqInt heapSize) {
  ...
  return (((heapSize & ((1ULL << bit) - 1)) != 0)
              ? (((heapSize | ((1ULL << bit) - 1)) - ((1ULL << bit) - 1))) +
                    (1ULL << bit)
              : heapSize);
}
...
static sqInt scavengerDenominator(void) { return 7; }
...
/* SpurMemoryManager>>#setFormatOf:to: */
static NoDbgRegParms void setFormatOfto(sqInt objOop, sqInt format) {
...
static NoDbgRegParms void setHashBitsOfto(sqInt objOop, sqInt hash) {
  long32Atput((void *)(objOop + 4), ((((long32At((void *)(objOop + 4))) |
...
static NoDbgRegParms NeverInline sqInt setHeapBasememoryLimitendOfMemory(
  ...
  newSpaceStartnewSpaceBytessurvivorBytes(
      GIV(newSpaceStart), GIV(oldSpaceStart) - GIV(newSpaceStart),
      ((GIV(oldSpaceStart) - GIV(newSpaceStart)) - reserve) /
...
static NoDbgRegParms void setHiddenRootsObj(sqInt anOop) {
  ...
  assert(((numSlotsOf(GIV(classTableFirstPage))) - 1) ==
  ...
  /* Set classTableIndex to the start of the last used page (excepting first
     ...
     max as a sentinel. */
  GIV(classTableIndex) = ((sqInt)((usqInt)(GIV(numClassTablePages))
                                  << (classTableMajorIndexShift())));
  for (i = 1; i < GIV(numClassTablePages); i += 1) {
    if (((page = fetchPointerofObject(i, GIV(hiddenRootsObj)))) ==
        GIV(nilObj)) {
      if ((((usqInt)(GIV(classTableIndex))) >> (classTableMajorIndexShift())) >
          i) {
        GIV(classTableIndex) =
            ((sqInt)((usqInt)(((((i - 1) < 1) ? 1 : (i - 1))))
                     << (classTableMajorIndexShift())));
      }
      GIV(numClassTablePages) = i;
...
setNextFreeChunkOfwithValuechunkBytes(sqInt freeChunk, sqInt nextFreeChunk,
  ...
  if ((nextFreeChunk != 0) && (!lilliputian)) {
    storePointerofFreeChunkwithValue(1 /* freeChunkPrevIndex */, nextFreeChunk,
                                     freeChunk);
  }
}
...
sqInt shouldRemapOop(sqInt oop) {
  return (isNonImmediate(oop)) && (shouldRemapObj(oop));
}

/*	Answer the number of bytes in the given object, including its base
   header, rounded up to an integral number of words. Hence, were it not for the
   fact that zero-sized objects have at least room for a forwarding pointer,
...
static NoDbgRegParms usqInt startOfObject(sqInt objOop) {
  return (hasOverflowHeader(objOop) ? objOop - BaseHeaderSize : objOop);
}
...
static NoDbgRegParms sqInt storePointerofForwarderwithValue(
    sqInt fieldIndex, sqInt objOop, sqInt valuePointer) {
  assert(isForwarded(objOop));
...
static NoDbgRegParms sqInt storePointerofFreeChunkwithValue(
    sqInt fieldIndex, sqInt objOop, sqInt valuePointer) {
  assert(isFreeObject(objOop));
  assert((valuePointer == 0) || (isFreeObject(valuePointer)));
  return longAtput((void *)((objOop + BaseHeaderSize) +
                            ((((usqInt)(fieldIndex) << (shiftForWord()))))),
                   valuePointer);
}
...
/* SpurMemoryManager>>#storeUncheckedNoAssert:ofAny:withValue: */
static NoDbgRegParms sqInt storeUncheckedNoAssertofAnywithValue(
    sqInt fieldIndex, sqInt objOop, sqInt valuePointer) {
  return longAtput((void *)((objOop + BaseHeaderSize) +
                            ((((usqInt)(fieldIndex) << (shiftForWord()))))),
...
static NoDbgRegParms sqInt swizzleObjStackAt(sqInt objStackRootIndex) {
  ...
  firstPage = (stackOrNil = fetchPointerofObject(objStackRootIndex,
                                                 GIV(hiddenRootsObj)));
  if (stackOrNil == GIV(nilObj)) {
    return stackOrNil;
  }
...
static sqInt totalFreeListBytes(void) {
  ...
  do {
    assert((bytesInBody(treeNodeSqInt)) >=
           ((numFreeLists()) * (allocationUnit())));
    smallChild =
        fetchPointerofFreeChunk(freeChunkSmallerIndex(), treeNodeSqInt);
    ...
    if (((smallChild == 0) && (largeChild == 0)) ||
      ...
      while (listNode != 0) {
        ...
        assert(bytesInChunk == (bytesInBody(listNode)));
        nextNode =
            fetchPointerofFreeChunk(0 /* freeChunkNextIndex */, listNode);
        assert(nextNode != listNode);
        listNode = nextNode;
      }
      ...
      treeNodeSqInt =
          fetchPointerofFreeChunk(2 /* freeChunkParentIndex */, treeNodeSqInt);
    } else {
      if ((smallChild != 0) && (cameFrom != smallChild)) {
...
static sqInt traceImmediatelySlotLimit(void) { return 64; }
...
static NoDbgRegParms sqInt unlinkFreeChunkatIndexchunkBytes(sqInt chunk,
  ...
  GIV(freeLists)
  [index] =
      ((next = fetchPointerofFreeChunk(0 /* freeChunkNextIndex */, chunk)));
...
static NoDbgRegParms sqInt unlinkFreeChunkchunkBytes(sqInt freeChunk,
  ...
  if (prev) {
    setNextFreeChunkOfwithValuechunkBytes(
        prev, fetchPointerofFreeChunk(0 /* freeChunkNextIndex */, freeChunk),
        chunkBytes);
    return freeChunk;
  }
  ...
  if ((index < (numFreeLists())) && ((1ULL << index) <= GIV(freeListsMask))) {
    ...
    /* For some reason the assertion is not compiled correctly */
    GIV(freeLists)
    [index] = ((nextSqInt = fetchPointerofFreeChunk(0 /* freeChunkNextIndex */,
                                                    freeChunk)));
    if (nextSqInt) {
      storePointerofFreeChunkwithValue(1 /* freeChunkPrevIndex */, nextSqInt,
                                       0);
    }
    return freeChunk;
  }
  ...
  while (node != 0) {
    ...
    if (node == freeChunk) {
      if (prev) {
        ...
        GIV(freeLists)
        [index] = ((nextSqInt = fetchPointerofFreeChunk(
                        0 /* freeChunkNextIndex */, freeChunk)));
      }
      return freeChunk;
    }
    prev = node;
...
static NoDbgRegParms void unlinkSolitaryFreeTreeNode(sqInt freeTreeNode) {
  ...
  if (parent) {
    if (smaller) {
      storePointerofFreeChunkwithValue(
          (freeTreeNode ==
                   (fetchPointerofFreeChunk(freeChunkSmallerIndex(), parent))
               ? freeChunkSmallerIndex()
               : freeChunkLargerIndex()),
          parent, smaller);
      storePointerofFreeChunkwithValue(2 /* freeChunkParentIndex */, smaller,
                                       parent);
      if (larger) {
      ...
      storePointerofFreeChunkwithValue(
          ...
          parent, larger);
      if (larger) {
        storePointerofFreeChunkwithValue(2 /* freeChunkParentIndex */, larger,
                                         parent);
      }
    }
  } else {
...
/*	Snapshot did not guarantee the state of the freelist prevLink, so we
   need to update it. Effectively transforms the freechunk single linked list in
   double linked list.
 */
/*	Small chunks */
...
static sqInt validClassTableRootPages(void) {
  ...
  if (!((numSlotsOf(GIV(hiddenRootsObj))) ==
        ((classTableRootSlots()) + (hiddenRootSlots())))) {
    return 0;
  }
...
static NoDbgRegParms const char *validFreeTreeChunkparent(sqInt chunk,
  ...
  if (parent !=
      (fetchPointerofFreeChunk(2 /* freeChunkParentIndex */, chunk))) {
    return "bad parent";
  }
  ...
  if ((reason = validFreeTreeChunkparent(
           fetchPointerofFreeChunk(freeChunkSmallerIndex(), chunk), chunk))) {
    return reason;
  }
  if ((reason = validFreeTreeChunkparent(
           fetchPointerofFreeChunk(freeChunkLargerIndex(), chunk), chunk))) {
    return reason;
  }
  return null;
}
...
static NoDbgRegParms sqInt validStorePointerUncheckedArgs(sqInt fieldIndex,
  ...
  DECL_MAYBE_SQ_GLOBAL_STRUCT
  return (fieldIndex >= 0) &&
         (((fieldIndex == 0) || (fieldIndex < (numSlotsOf(objOop)))) &&
          ((isImmediate(valuePointer)) ||
...
sqInt shouldRemapObj(sqInt objOop) {
  ...
  return /* slidingCompactionShouldRemapObj: */
      ...
      ((GIV(gcPhaseInProgress) > 0) &&
       ((scavengeInProgress()
             ? (isReallyYoungObject(objOop)) && (!(isInFutureSpace(objOop)))
             : /* isMobile: */
...
static void allocateOrExtendSegmentInfos(void) {
  ...
  GIV(segments) =
      realloc(GIV(segments), newNumSegs * (sizeof(SpurSegmentInfo)));
  if (!GIV(segments)) {
    error("out of memory; cannot allocate more segments");
  }
  memset(GIV(segments) + GIV(numSegInfos), 0,
         (newNumSegs - GIV(numSegInfos)) * (sizeof(SpurSegmentInfo)));
...
static void checkSegments(void) {
  ...
  assert(((segLimit(&GIV(segments)[GIV(numSegments) - 1])) - (bridgeSize())) ==
         (GIV(endOfMemory)));
}
...
static NoDbgRegParms sqInt isValidSegmentBridge(sqInt objOop) {
  DECL_MAYBE_SQ_GLOBAL_STRUCT
  return ((addressCouldBeObj(objOop)) || (objOop == GIV(endOfMemory))) &&
         ((isSegmentBridge(objOop)) &&
          ((hasOverflowHeader(objOop)) || ((numSlotsOfAny(objOop)) == 0)));
}
...
static NoDbgRegParms sqInt readHeapFromImageFiledataBytes(sqImageFile f,
   ...
   * eliminated. */
  assert((newBase - (GIV(oldSpaceStart))) ==
         (totalBytesRead - (GIV(numSegments) * (bridgeSize()))));
...
SpurSegmentInfo *segmentContainingObj(sqInt objOop) {
  ...
  do {
    ...
    if (oopisGreaterThanOrEqualTo(objOop, (seg->segStart))) {
      if (mid == high) {
        return (oopisLessThan(objOop, segLimit(seg)) ? seg : 0);
      } else {
        low = mid;
        mid = ((mid + high) + 1) / 2;
      }
...
static sqInt totalOldSpaceCapacity(void) {
  ...
  return GIV(totalHeapSizeIncludingBridges) -
         (GIV(numSegments) * (bridgeSize()));
}
...
static NoDbgRegParms sqInt bailOutOfImageLoad(sqInt rawVersion) {
  DECL_MAYBE_SQ_GLOBAL_STRUCT
  fprintf(
      stderr, "This ver %d vm cannot read ver %" PRIdSQINT " image file %s\n",
      ((int)68021 /* imageFormatVersion */), rawVersion, GIV(imageFilename));
  return 0;
}

/*	Check that the classes the VM assumes are compact have the right
 * indices. */
...
static void checkAssumedCompactClasses(void) {
  ...
  if ((fetchPointerofObject(ClassArray, GIV(specialObjectsOop))) !=
      (knownClassAtIndex(ClassArrayCompactIndex))) {
    invalidCompactClassError("Array");
  }

  /* begin checkCompactIndex:isClass:named: */
  if ((fetchPointerofObject(ClassLargeNegativeInteger,
                            GIV(specialObjectsOop))) !=
      (knownClassAtIndex(ClassLargeNegativeIntegerCompactIndex))) {
    invalidCompactClassError("LargeNegativeInteger");
  }

  /* begin checkCompactIndex:isClass:named: */
  if ((fetchPointerofObject(ClassLargePositiveInteger,
                            GIV(specialObjectsOop))) !=
  ...
  /* begin checkCompactIndex:isClass:named: */
  if ((fetchPointerofObject(ClassFloat, GIV(specialObjectsOop))) !=
      (knownClassAtIndex(ClassFloatCompactIndex))) {
    invalidCompactClassError("Float");
  }

  /* begin checkCompactIndex:isClass:named: */
  if ((fetchPointerofObject(ClassBlockClosure, GIV(specialObjectsOop))) !=
      (knownClassAtIndex(ClassBlockClosureCompactIndex))) {
    invalidCompactClassError("BlockClosure");
  }

  /* begin checkCompactIndex:isClass:named: */
  if ((fetchPointerofObject(ClassMethodContext, GIV(specialObjectsOop))) !=
      (knownClassAtIndex(ClassMethodContextCompactIndex))) {
    invalidCompactClassError("MethodContext");
  }
  GIV(classByteArrayCompactIndex) = compactIndexOfClass(
      fetchPointerofObject(ClassByteArray, GIV(specialObjectsOop)));
}
...
static sqInt elementsPerPrintOopLine(void) { return 5; }
...
static NoDbgRegParms usqInt
firstBytecodeOfAlternateHeadermethod(sqInt methodHeader, sqInt theMethod) {
  return (theMethod + ((LiteralStart + (((methodHeader >> 3)) &
...
static NoDbgRegParms sqInt getLongFromFileswap(sqImageFile aFile,
  ...
  return (swapFlag ? byteSwapped(w) : w);
}

/*	Answer the next 16 bit word read from aFile, byte-swapped according to
   the swapFlag.
 */
...
static NoDbgRegParms usqInt getShortFromFileswap(sqImageFile aFile,
  ...
  return (swapFlag ? ((((usqInt)(w)) >> 8) & 0xFF) | (((w & 0xFF) << 8)) : w);
}

/*	Answer the next 32 bit word read from aFile, byte-swapped according to
   the swapFlag.
 */
...
static NoDbgRegParms sqInt getWord32FromFileswap(sqImageFile aFile,
  ...
  return (swapFlag ? SQ_SWAP_4_BYTES(w) : w);
}
...
static NoDbgRegParms sqInt isBooleanObject(sqInt oop) {
  DECL_MAYBE_SQ_GLOBAL_STRUCT
  return (oop == GIV(trueObj)) || (oop == GIV(falseObj));
}
...
sqInt isKindOfInteger(sqInt oop) {
  return ((((oop) & 7) == 1)) || (isLargeIntegerInstance(oop));
}

/* StackInterpreter>>#isLargeIntegerObject: */
sqInt isLargeIntegerObject(sqInt oop) { return isLargeIntegerInstance(oop); }
...
static NoDbgRegParms sqInt lengthOfNameOfClass(sqInt classOop) {
  ...
  if (numSlots == GIV(metaclassNumSlots)) {
    return lengthOfNameOfClass(
        fetchPointerofObject(GIV(thisClassIndex), classOop));
  }
...
void longPrintOop(sqInt oop) {
  ...
  if ((cls = fetchClassOfNonImm(oop))) {
    ...
    fprintf(GIV(transcript), "%p: a(n) %.*s", ((void *)oop), ((int)length),
            className);
    fprintf(GIV(transcript), "(%" PRIxSQINT "=>%p)", classIndexOf(oop),
            ((void *)cls));
  } else {
    fprintf(GIV(transcript), "%p has a nil class!!\n", ((void *)oop));
  }
  fmt = (byteAt((void *)(oop + (formatFieldByteOffset())))) & (formatMask());
  fprintf(GIV(transcript), " format %" PRIxSQINT "", fmt);
  if (fmt > (lastPointerFormat())) {
  ...
  fprintf(GIV(transcript), " hdr%d %c%c%c%c%c",
          (hasOverflowHeader(oop) ? 16 : 8), (isImmutable(oop) ? 'i' : '.'),
          (isRemembered(oop) ? 'r' : '.'), (isPinned(oop) ? 'p' : '.'),
          (isMarked(oop) ? 'm' : '.'), (isGrey(oop) ? 'g' : '.'));
  fprintf(GIV(transcript), " hash %" PRIxSQINT "\n", rawHashBitsOf(oop));
  if (isPureBitsFormat(fmt)) {
    if (isKindOfClass(
            oop, fetchPointerofObject(ClassAlien, GIV(specialObjectsOop)))) {
      fprintf(GIV(transcript), " datasize %" PRIdSQINT " %s @ %p\n",
              longAt((void *)(oop + BaseHeaderSize)),
              (isIndirectAlien(oop)
                   ? "indirect"
                   : (isPointerAlien(oop) ? "pointer" : "direct")),
              startOfAlienData(oop));
      return;
    }
    if (isKindOfClass(oop, superclassOf(fetchPointerofObject(
                               ClassByteString, GIV(specialObjectsOop))))) {
  ...
  fmtSqInt =
      (byteAt((void *)(oop + (formatFieldByteOffset())))) & (formatMask());
  if (fmtSqInt <= (lastPointerFormat())) {
    if ((fmtSqInt == (indexablePointersFormat())) &&
        ((classIndexOf(oop)) == ClassMethodContextCompactIndex)) {
      /* begin fetchStackPointerOf: */
...
sqInt maybeSelectorOfMethod(sqInt methodObj) {
  ...
  if (isWordsOrBytes(penultimateLiteral)) {
    return ((fetchClassTagOfNonImm(penultimateLiteral)) ==
                    (fetchClassTagOfNonImm(fetchPointerofObject(
                        SelectorDoesNotUnderstand, GIV(specialObjectsOop))))
                ? penultimateLiteral
                : 0);
  }
  return (
      (isPointers(penultimateLiteral)) &&
               ...
               (((fetchPointerofObject(0, penultimateLiteral)) == methodObj) &&
                (((maybeSelector = fetchPointerofObject(1, penultimateLiteral)),
                  (isWordsOrBytes(maybeSelector)) &&
                      ((fetchClassTagOfNonImm(maybeSelector)) ==
                       (fetchClassTagOfNonImm(
                           fetchPointerofObject(SelectorDoesNotUnderstand,
                                                GIV(specialObjectsOop)))))))))
          ? maybeSelector
          : 0);
}
...
sqInt methodClassOf(sqInt methodPointer) {
  ...
  literal =
      followLiteralofMethod((literalCountOf(methodPointer)) - 1, methodPointer);
  return ((literal != GIV(nilObj)) && (isPointers(literal))
  ...
  if (numSlots == GIV(metaclassNumSlots)) {
    ...
    if (addressCouldBeClassObj(maybeThisClassOop)) {
      return nameOfClasslengthInto(
          fetchPointerofObject(GIV(thisClassIndex), classOop), lengthPtr);
    }
    lengthPtr[0] = 9;
    return "bad class";
  }
...
static NoDbgRegParms sqInt objectbeginsWithStringofSize(sqInt anOop,
                                                        char *aCString,
                                                        sqInt aCStringStrlen) {
  return (isBytes(anOop)) && ((!(isCompiledMethod(anOop))) &&
                              (((numBytesOfBytes(anOop)) >= aCStringStrlen) &&
                               ((strncmp(aCString, firstIndexableField(anOop),
                                         aCStringStrlen)) == 0)));
}
...
sqInt primitiveIndexOfMethodheader(sqInt theMethod, sqInt methodHeader) {
  ...
  return (((methodHeader & AlternateHeaderHasPrimFlag) != 0)
              ? ((firstBytecode = firstBytecodeOfAlternateHeadermethod(
                      methodHeader, theMethod)),
...
static NoDbgRegParms void printDecodeMethodHeaderOop(sqInt methodHeaderOop) {
  ...
  print(" nLits ");
  printNum((/* begin literalCountOfMethodHeader: */
...
static NoDbgRegParms sqInt printHexnpnp(usqInt n) {
  DECL_MAYBE_SQ_GLOBAL_STRUCT
  return fprintf(GIV(transcript), "%" PRIxSQINT "", n);
}
...
static NoDbgRegParms sqInt printHexnp(usqInt n) {
  DECL_MAYBE_SQ_GLOBAL_STRUCT
  return fprintf(GIV(transcript), "%p", ((void *)n));
}
  ...
  while (1) {
    ...
    if (classIndex >= (isForwardedObjectClassIndexPun())) {
      if ((isCompiledMethod(objSqInt)) &&
        ...
        printChar(' ');
        printOopShortInner(
            ((isPointers(methodClassAssociation)) &&
...
void printMethodCacheFor(sqInt thing) {
  ...
  for (i = 0; i < MethodCacheSize; i += MethodCacheEntrySize) {
    ...
    if (((thing == -1) ||
      ...
      n += 1;
      fprintf(GIV(transcript), "%d %x\n\t", ((int)i), ((int)i));
      if (isBytesNonImm(s)) {
        fprintf(GIV(transcript), "%p %.*s\n", ((void *)s),
...
static NoDbgRegParms void printNameOfClasscount(sqInt classOop, sqInt cnt) {
  ...
  if ((numSlots == GIV(metaclassNumSlots)) &&
    ...
    printNameOfClasscount(fetchPointerofObject(GIV(thisClassIndex), classOop),
                          cnt - 1);
    print(" class");
  } else {
    if (numSlots <= GIV(classNameIndex)) {
...
static NoDbgRegParms void printOopShortInner(sqInt oop) {
  ...
  if (isImmediate(oop)) {
    if (((oop & (characterTag())) != 0)) {
      fprintf(GIV(transcript), "$%c(%" PRIxSQINT ")",
              ((int)(characterValueOf(oop))), characterValueOf(oop));
      return;
    }
    if ((((oop) & 7) == 1)) {
      fprintf(GIV(transcript), "%" PRIdSQINT "(0x%" PRIxSQINT ")", (oop >> 3),
              (oop >> 3));
      return;
    }
    if (((oop & (smallFloatTag())) != 0)) {
      fprintf(GIV(transcript), "%g(0x%" PRIxSQINT ")", noFailFloatValueOf(oop),
              oop);
      return;
    }
    fprintf(GIV(transcript), "unknown immediate %p", ((void *)oop));
    return;
  }
  if (!(addressCouldBeObj(oop))) {
    print((oop & ((allocationUnit()) - 1) ? " is misaligned" : whereIs(oop)));
    return;
  }
  if (isFreeObject(oop)) {
  ...
  if ((!((longAt((void *)(oop))) &
    ...
    fprintf(GIV(transcript), " is a forwarder to %p", ((void *)target));
    return;
  }
  if (/* isFloatInstance: */
      ((tagBits = oop & (tagMask()))
  ...
  fprintf(GIV(transcript), "a(n) %.*s", ((int)nameLen), name);
...
void printOop(sqInt oop) {
  ...
  l1:
    ...
    for (index = startIP; index <= lastIndex; index += 1) {
      ...
      fprintf(GIV(transcript), " %02x/%-+3d", ((int)byte), ((int)byte));
      column += 1;
...
static NoDbgRegParms void printStringOf(sqInt oop) {
  ...
  if ((isinstanceOfcompactClassIndex(
          oop, fetchPointerofObject(ClassByteArray, GIV(specialObjectsOop)),
          GIV(classByteArrayCompactIndex))) ||
...
sqInt readableFormat(sqInt imageVersion) {
  return (imageFormatVersionFromSnapshot(imageVersion)) ==
...
size_t readImageFromFileHeapSizeStartingAt(sqImageFile f,
  ...
  GIV(multipleBytecodeSetsActive) =
      ((version & MultipleBytecodeSetsBitmask) != 0);
  ...
  if (!((GIV(lastHash) = seed & (identityHashHalfWordMask())))) {
    while (((GIV(lastHash) = (((usqInt)1 /* ioUTCMicrosecondsNow */)) &
  ...
  GIV(numStackPages) =
      (desiredNumStackPages
           ? desiredNumStackPages
           : (hdrNumStackPages ? hdrNumStackPages
  ...
  headroomSqInt =
      (extraVMMemory ? extraVMMemory
                     : (!(GIV(growHeadroom)) ? 0x1000000 : GIV(growHeadroom)));
...
l1:
  heapSize =
      roundUpHeapSize(((dataSize + headroom) + GIV(edenBytes)) +
...
/*	Byte-swap all words in memory after reading in the entire image file
   with bulk read. Contributed by Tim Rowledge.
 */
...
static NoDbgRegParms void setImageHeaderFlagsFrom(sqInt headerFlags) {
  ...
  GIV(imageFloatsBigEndian) = ((!(headerFlags & 2)) ? 1 : 0);
...
static NoDbgRegParms void shortPrintOop(sqInt oop) {
  ...
  fprintf(GIV(transcript), "%p: a(n) %.*s\n", ((void *)oop), ((int)length),
          className);
}
...
/*	Byte-swap the words of all bytes objects in the image, including
   Strings, ByteArrays, and CompiledMethods. This returns these objects to their
        original byte ordering
        after blindly byte-swapping the entire image. For compiled methods,
        byte-swap only their bytecodes part. Ensure floats are in
   platform-order. */
...
static void updateObjectsPostByteSwap(void) {
  ...
  while (1) {
    ...
    if (classIndex >= (isForwardedObjectClassIndexPun())) {
      ...
      /* oop contains bytes */
      if (fmt == (firstLongFormat())) {
        if (swapFloatWords && ((classIndexOf(obj)) == ClassFloatCompactIndex)) {
...
static char _m[] = "";
void *vm_exports[][3] = {{NULL, NULL, NULL}};
