...
unlinkSolitaryFreeTreeNode(sqInt freeTreeNode)
{
	...
	/* case 1. interior node has one child, P = parent, N = node, S = subtree (mirrored for large vs small)
	   ...
	   _/_  _\_		    _/_
	   | L | | R |		    | L | */
	smaller = longAt((void *)((freeTreeNode + BaseHeaderSize) + (3U << (shiftForWord()))));
	larger = longAt((void *)((freeTreeNode + BaseHeaderSize) + (4U << (shiftForWord()))));
	parent = longAt((void *)((freeTreeNode + BaseHeaderSize) + (2U << (shiftForWord()))));
	if (parent) {
		if (smaller) {
			fieldIndex = (freeTreeNode == (longAt((void *)((parent + BaseHeaderSize) + (3U << (shiftForWord())))))
			...
			longAtput((void *)((parent + BaseHeaderSize) + ((((usqInt)(fieldIndex) << (shiftForWord()))))),smaller);

			/* begin storePointer:ofFreeChunk:withValue: */
			assert(isFreeObject(smaller));
			assert((parent == 0)
			 || (isFreeObject(parent)));
			longAtput((void *)((smaller + BaseHeaderSize) + (2U << (shiftForWord()))),parent);
			if (larger) {
			...
			assert((larger == 0)
			 || (isFreeObject(larger)));
			longAtput((void *)((parent + BaseHeaderSize) + ((((usqInt)(fieldIndex) << (shiftForWord()))))),larger);
			if (larger) {
				/* begin storePointer:ofFreeChunk:withValue: */
				assert(isFreeObject(larger));
				assert((parent == 0)
				 || (isFreeObject(parent)));
				longAtput((void *)((larger + BaseHeaderSize) + (2U << (shiftForWord()))),parent);
			}
		}
	}
	else {
