...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	/* begin storePointer:ofFreeChunk:withValue: */
	assert(isFreeObject(freeChunk));
	longAtput((void *)((freeChunk + BaseHeaderSize) + (0U << (shiftForWord()))),0);
	...
	/* begin storePointer:ofFreeChunk:withValue: */
	assert(isFreeObject(freeChunk));
	longAtput((void *)((freeChunk + BaseHeaderSize) + (2U << (shiftForWord()))),0);

	/* begin storePointer:ofFreeChunk:withValue: */
	assert(isFreeObject(freeChunk));
	longAtput((void *)((freeChunk + BaseHeaderSize) + (3U << (shiftForWord()))),0);

	/* begin storePointer:ofFreeChunk:withValue: */
	assert(isFreeObject(freeChunk));
	longAtput((void *)((freeChunk + BaseHeaderSize) + (4U << (shiftForWord()))),0);
	...
	while (child != 0) {
		...
		if (childBytes == chunkBytes) {
			nextFreeChunk = longAt((void *)((child + BaseHeaderSize) + (0U << (shiftForWord()))));
			...
			assert((freeChunk == 0)
			 || (isFreeObject(freeChunk)));
			longAtput((void *)((child + BaseHeaderSize) + (0U << (shiftForWord()))),freeChunk);
			if (freeChunk) {
				/* begin storePointer:ofFreeChunk:withValue: */
				assert(isFreeObject(freeChunk));
				assert((child == 0)
				 || (isFreeObject(child)));
				longAtput((void *)((freeChunk + BaseHeaderSize) + (1U << (shiftForWord()))),child);
			}
			return child;
		}
		...
		/* size match; add to list at node.
		   walk down the tree */
		parent = child;
		child = longAt((void *)((child + BaseHeaderSize) + ((((usqInt)(((childBytes > chunkBytes
	...
	assert((parent == 0)
	 || (isFreeObject(parent)));
	longAtput((void *)((freeChunk + BaseHeaderSize) + (2U << (shiftForWord()))),parent);

	/* begin storePointer:ofFreeChunk:withValue: */
	assert(isFreeObject(parent));
	assert((freeChunk == 0)
	 || (isFreeObject(freeChunk)));
	longAtput((void *)((parent + BaseHeaderSize) + ((((usqInt)(((childBytes > chunkBytes
		? 3 /* freeChunkSmallerIndex */
		: 4 /* freeChunkLargerIndex */))) << (shiftForWord()))))),freeChunk);
	return 0;
}