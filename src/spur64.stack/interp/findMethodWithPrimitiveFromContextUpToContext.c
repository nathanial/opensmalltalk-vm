...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (1) {
		...
		if (!((primitive == 0)
			...
			if ((!((longAt((void *)(theMethod))) & ((classIndexMask()) - (isForwardedObjectClassIndexPun()))))) {
				theMethod = fixFollowedFieldofObjectwithInitialValue(MethodIndex, theContext, theMethod);
			}
		...
		/* begin followObjField:ofObject: */
		objOop = longAt((void *)((theContext + BaseHeaderSize) + ((((usqInt)(SenderIndex) << (shiftForWord()))))));
		assert(isNonImmediate(objOop));
		if ((!((longAt((void *)(objOop))) & ((classIndexMask()) - (isForwardedObjectClassIndexPun()))))) {
			objOop = fixFollowedFieldofObjectwithInitialValue(SenderIndex, theContext, objOop);
		}
		theContext = objOop;
	}
	...
	senderOop = longAt((void *)((theContext + BaseHeaderSize) + ((((usqInt)(SenderIndex) << (shiftForWord()))))));
