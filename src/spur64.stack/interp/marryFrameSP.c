...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	methodHeader = longAt((void *)(((longAt(theFP + FoxMethod)) + BaseHeaderSize) + ((((usqInt)(HeaderIndex) << (shiftForWord()))))));
	...
	if (byteAt((theFP + FoxFrameFlags) + 3)) {
		...
		closureOrNil = longAt(theFP + ((FoxCallerSavedIP + BytesPerWord) + ((((usqInt)((byteAt((theFP + FoxFrameFlags) + 1))) << (shiftForWord()))))));
	}
	else {
	...
	if (numSlots >= (numSlotsMask())) {
		...
		long64Atput((void *)(newObj),((((((usqLong) numSlots)) << (numSlotsFullShift()))) + ((((usqInt)((indexablePointersFormat())) << (formatShift()))))) + ClassMethodContextCompactIndex);
	}
...
l1:
	...
	/* begin withSmallIntegerTags: */
	assert(((oopForPointer(((char *)(longAt(theFP + FoxSavedFP))))) & (BytesPerWord - 1)) == 0);
	valuePointer = (oopForPointer(((char *)(longAt(theFP + FoxSavedFP))))) + (smallIntegerTag());

	/* begin storePointerUnchecked:ofObject:withValue: */
	assert((isNonImmediate(theContext))
	 && (!(isForwarded(theContext))));
	assert(validStorePointerUncheckedArgs(InstructionPointerIndex, theContext, valuePointer));
	longAtput((void *)((theContext + BaseHeaderSize) + ((((usqInt)(InstructionPointerIndex) << (shiftForWord()))))),valuePointer);

	/* begin storePointerUnchecked:ofObject:withValue: */
	assert((isNonImmediate(theContext))
	 && (!(isForwarded(theContext))));
	assert(validStorePointerUncheckedArgs(StackPointerIndex, theContext, ((numStack << 3) | 1)));
	longAtput((void *)((theContext + BaseHeaderSize) + ((((usqInt)(StackPointerIndex) << (shiftForWord()))))),((numStack << 3) | 1));

	/* begin storePointerUnchecked:ofObject:withValue: */
	assert((isNonImmediate(theContext))
	 && (!(isForwarded(theContext))));
	assert(validStorePointerUncheckedArgs(MethodIndex, theContext, longAt(theFP + FoxMethod)));
	longAtput((void *)((theContext + BaseHeaderSize) + ((((usqInt)(MethodIndex) << (shiftForWord()))))),longAt(theFP + FoxMethod));

	/* begin storePointerUnchecked:ofObject:withValue: */
	assert((isNonImmediate(theContext))
	 && (!(isForwarded(theContext))));
	assert(validStorePointerUncheckedArgs(ClosureIndex, theContext, closureOrNil));
	longAtput((void *)((theContext + BaseHeaderSize) + ((((usqInt)(ClosureIndex) << (shiftForWord()))))),closureOrNil);

	/* begin storePointerUnchecked:ofObject:withValue: */
	assert((isNonImmediate(theContext))
	 && (!(isForwarded(theContext))));
	assert(validStorePointerUncheckedArgs(ReceiverIndex, theContext, longAt(theFP + FoxReceiver)));
	longAtput((void *)((theContext + BaseHeaderSize) + ((((usqInt)(ReceiverIndex) << (shiftForWord()))))),longAt(theFP + FoxReceiver));
	for (i = 1; i <= numArgs; i += 1) {
		/* begin storePointerUnchecked:ofObject:withValue: */
		assert((isNonImmediate(theContext))
		 && (!(isForwarded(theContext))));
		assert(validStorePointerUncheckedArgs(ReceiverIndex + i, theContext, longAt((void *)((theFP + FoxCallerSavedIP) + ((numArgs - (i - 1)) * BytesPerWord)))));
		longAtput((void *)((theContext + BaseHeaderSize) + ((((usqInt)((ReceiverIndex + i)) << (shiftForWord()))))),longAt((void *)((theFP + FoxCallerSavedIP) + ((numArgs - (i - 1)) * BytesPerWord))));
	}
