...
/*	Answer if the obj should be scavenged, or simply followed. Sent via the
        ...
        via mapInterpreterOops may be applied twice in the context of a global
   GC where a scavenge, followed by a scan-mark-free, and final compaction
   passes may result in scavenged fields being visited twice. */
             ...
             ? ((/* begin isReallyYoungObject: */ /* begin isYoungObject: */
                 ...
                 oopisLessThan(objOop, oldSpaceStart))) &&
                   (!(oopisGreaterThanOrEqualToandLessThan(
                       objOop, ((futureSpace).start), futureSurvivorStart)))
             : /* isMobile: */
             (oopisGreaterThanOrEqualToandLessThanOrEqualTo(
                 objOop, mobileStart, lastMobileObject)) &&
