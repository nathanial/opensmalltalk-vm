/* Extracted from interp.c:33830 (function
...
/*	This is part of storeImageSegmentInto:outPointers:roots:.
        ...
        been copied. If it is a class in the class table, set the copy's hash to
   0 for reassignment on load, and mark it as a class by setting its
   isRemembered bit. Answer the next segmentAddr if successful. Answer an
...
static sqInt copyObjtoAddrstopAtsavedFirstFieldsindex(sqInt objOop,
  ...
  copy = (numSlots == (numSlotsMask()) ? segAddr + BaseHeaderSize : segAddr);

  /* Clear remembered, mark bits of all headers copied into the segment (except
   * classes) */
  setIsRememberedOfto(copy, 0);

  /* begin setIsMarkedOf:to: */
  assert(!(isFreeObject(copy)));
  byteAtput((void *)(copy + (markBitsByteOffset())),
            (byteAt((void *)(copy + (markBitsByteOffset())))) &
                (0xFF - (1U << (markedBitByteShift()))));

  /* Make any objects with hidden dynamic state (contexts, methods) look like
   * normal objects. */
  ...
  /* If the object is a class, zero its identityHash (which is its classIndex)
     and set its isRemembered bit.  It will be assigned a new hash and entered
     into the table on load. */
  hash = (long32At((void *)(objOop + 4))) & (identityHashHalfWordMask());
  if ((hash > (lastClassIndexPun())) && ((classOrNilAtIndex(hash)) == objOop)) {
    /* begin setHashBitsOf:to: */
    long32Atput((void *)(copy + 4), ((((long32At((void *)(copy + 4))) |
  ...
  valuePointer = fetchPointerofObject(0U, objOop);
  ...
  longAtput((void *)((savedFirstFields + BaseHeaderSize) +
