	...
	if ((fmt == (indexablePointersFormat()))
	 && ((hdr & (classIndexMask())) == ClassMethodContextCompactIndex)) {
		...
		if (fmt <= 5 /* lastPointerFormat */) {
			...
			longAtput((void *)((rcvr + BaseHeaderSize) + ((((usqInt)(fieldIndex) << (shiftForWord()))))),newValue);
			goto l3;
		}
		if (fmt >= (firstByteFormat())) {
			if (!((((newValue) & 7) == 1))) {
			...
			byteAtput((void *)((rcvr + BaseHeaderSize) + (index - 1)),signedValueToStore);
			goto l3;
		}
		if (fmt >= (firstShortFormat())) {
			if (!((((newValue) & 7) == 1))) {
			...
			shortAtput((void *)((rcvr + BaseHeaderSize) + ((((usqInt)((index - 1)) << 1)))),signedValueToStore);
			goto l3;
		}
		if (fmt == (sixtyFourBitIndexableFormat())) {
