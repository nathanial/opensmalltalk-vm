...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	sz = (((numSlots = byteAt((void *)(aContext + (numSlotsFieldByteOffset()))))) == (numSlotsMask())
	...
	if (sz >= (numSlotsMask())) {
		...
		long64Atput((void *)(newObj),((((((usqLong) sz)) << (numSlotsFullShift()))) + ((((usqInt)((indexablePointersFormat())) << (formatShift()))))) + ClassMethodContextCompactIndex);
	}
...
l1:
	...
	for (i = 0; i <= StackPointerIndex; i += 1) {
		...
		assert((isNonImmediate(cloned))
		 && (!(isForwarded(cloned))));
		assert(validStorePointerUncheckedArgs(i, cloned, valuePointer));
		longAtput((void *)((cloned + BaseHeaderSize) + ((((usqInt)(i) << (shiftForWord()))))),valuePointer);
	}
	for (i = MethodIndex; i <= ReceiverIndex; i += 1) {
		valuePointer = longAt((void *)((aContext + BaseHeaderSize) + ((((usqInt)(i) << (shiftForWord()))))));
	...
	if (/* isStillMarriedContext: */
	 ...
	 && (!(isWidowedContext(aContext)))) {
		...
		for (i = 0; i <= sp; i += 1) {
			...
			valuePointer = /* temporary:in: */
					(i < ((frameNumArgs = byteAt((spouseFP + FoxFrameFlags) + 1)))
						? longAt((void *)((spouseFP + FoxCallerSavedIP) + ((frameNumArgs - i) * BytesPerWord)))
						: longAt((void *)(((spouseFP + FoxReceiver) - BytesPerWord) + ((frameNumArgs - i) * BytesPerWord))));

			/* begin storePointerUnchecked:ofObject:withValue: */
			assert((isNonImmediate(cloned))
			 && (!(isForwarded(cloned))));
			assert(validStorePointerUncheckedArgs(fieldIndex, cloned, valuePointer));
			longAtput((void *)((cloned + BaseHeaderSize) + ((((usqInt)(fieldIndex) << (shiftForWord()))))),valuePointer);
		}
	}
	else {
		...
		for (i = 0; i <= sp; i += 1) {
			fieldIndex = i + CtxtTempFrameStart;
			valuePointer = longAt((void *)((aContext + BaseHeaderSize) + ((((usqInt)((i + CtxtTempFrameStart)) << (shiftForWord()))))));
