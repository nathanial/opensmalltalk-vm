...
/*	Implement either ProtoObject>>tryPrimitive: primIndex withArgs: argArray
        or Context>>receiver: anObject tryPrimitive: primIndex withArgs:
   argArray. If this primitive fails, arrange that its error code is a negative
        integer, to
...
static void primitiveDoPrimitiveWithArgs(void) {
  ...
  primitiveFunctionPointer = ((void (*)(void))(
      ((((usqInt)primIdx)) > MaxPrimitiveIndex ? 0 : primitiveTable[primIdx])));
  ...
  /* We use tempOop instead of pushRemappableOop:/popRemappableOop here because
     in the Cogit primitiveEnterCriticalSection, primitiveSignal,
     primitiveResume et al longjmp back to either the interpreter or machine
     code, depending on the process activated.  So if we're executing one of
     these primitives, control won't actually return here and the matching
     popRemappableOop: wouldn't occur, potentially overflowing the remap buffer.
     Note that while recursion could occur (nil tryPrimitive: 118 withArgs:
     #(118 #(110 #()))) it counts as shooting oneself in the foot. */
  ...
  if (!(slowPrimitiveResponse())) {
    ...
    /* the primitive failed in a recursive invocation.  can't fix things with no
     * value... */
