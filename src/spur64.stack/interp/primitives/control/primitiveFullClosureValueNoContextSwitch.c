/* Extracted from interp.c:16790 (function
 * primitiveFullClosureValueNoContextSwitch). */

/*	An exact clone of primitiveFullClosureValue except that this version
   will not check for interrupts on stack overflow. It may invoke the garbage
...
void primitiveFullClosureValueNoContextSwitch(void) {
  sqInt blockClosure;
  sqInt closureMethod;
  sqInt i;
  sqInt methodHeader;
