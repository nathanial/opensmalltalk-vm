...
static void primitiveConstantFill(void) {
  ...
  if (!(((!(rcvr & (tagMask())))) &&
        (((format = (byteAt((void *)(rcvr + (formatFieldByteOffset())))) &
  ...
  if (format >= (firstShortFormat())) {
    if (format >= (firstByteFormat())) {
      if ((fillValue > 0xFF) || (format >= (firstCompiledMethodFormat()))) {
        /* primitiveFailFor: */
        primFailCode =
            (fillValue > 0xFF ? PrimErrBadArgument : PrimErrBadReceiver);
        goto l1;
      }
      fillValue = ((fillValue + ((fillValue << 8))) + ((fillValue << 16))) +
                  ((fillValue << 24));
      oddBytes = format & 7;
    } else {
      if (fillValue > 0xFFFF) {
