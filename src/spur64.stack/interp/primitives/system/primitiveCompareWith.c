...
primitiveCompareWith(void) {
  ...
  if (!(argumentCount == 1)) {
    ...
    if (!((/* isBytes: */
           ((!(order & (tagMask())))) &&
           (((byteAt((void *)(order + (formatFieldByteOffset())))) &
  ...
  if (!((/* isBytes: */
         ((!(string1 & (tagMask())))) &&
         (((byteAt((void *)(string1 + (formatFieldByteOffset())))) &
           (formatMask())) >= (firstByteFormat()))) &&
        (/* isBytes: */
         ((!(string2 & (tagMask())))) &&
         (((byteAt((void *)(string2 + (formatFieldByteOffset())))) &
           (formatMask())) >= (firstByteFormat()))))) {
    /* primitiveFailFor: */
    primFailCode = PrimErrBadArgument;
    return;
  ...
  if (order) {
    ...
    for (i = 0; i < min; i += 1) {
      c1 = byteAt(
          (void *)((order + BaseHeaderSize) +
                   ((byteAt((void *)((string1 + BaseHeaderSize) + i))) + 1)));
      c2 = byteAt(
          (void *)((order + BaseHeaderSize) +
                   ((byteAt((void *)((string2 + BaseHeaderSize) + i))) + 1)));
      if (!(c1 == c2)) {
        result = c1 - c2;
        goto l2;
      }
