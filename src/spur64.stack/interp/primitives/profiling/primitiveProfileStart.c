...
/*	Primitive. Begin profiling execution by using the high-resolution clock
        ...
        profiled trigger off leading to consistently wrong results). The
   argument is the number of ticks of the high-resolution clock to elapse before
   taking a sample. The sample is stored in the profileProcess and profileMethod
   iVars which can be retrieved via primitiveProfileSample and
        primitiveProfilePrimitive. So also void the sample iVars. Once a sample
   is taken, the semaphore installed via primitiveProfileSemaphore is signalled.
   If the argument is less or equal to zero, profiling is disabled. */
...
primitiveProfileStart(void) {
  ...
  if ((((deltaTicks) & 7) == 1)) {
    ...
    nextProfileTick = (deltaTicks > 0 ? (ioHighResClock()) + deltaTicks : 0);
