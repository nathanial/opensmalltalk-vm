...
static NoDbgRegParms void printFrameFlagsForFP(char *theFP) {
  ...
  fprintf(transcript, "       flags: %p", ((void *)it));
  if (it) {
    fprintf(transcript, "=%d", ((int)it));
  }
  fprintf(transcript, "  numArgs: %d %sContext %sBlock\n",
          ((int)(byteAt((theFP + FoxFrameFlags) + 1))),
          (byteAt((theFP + FoxFrameFlags) + 2) ? "is" : "no"),
          (byteAt((theFP + FoxFrameFlags) + 3) ? "is" : "not"));
}