...
static void primitiveSetOrHasIdentityHash(void) {
  ...
  if (!argumentCount) {
    hasHash = ((!((longAt(stackPointer)) & (tagMask())))) &&
              ((long32At((void *)((longAt(stackPointer)) + 4))) &
  ...
  if (!(((((hash) & 7) == 1)) &&
        ((/* if written naturally, clang warns: unsequenced modification and
             access to 'hash' */
          (hash = (hash >> 3)),
  ...
  if (!((!(thisReceiver & (tagMask()))))) {
    /* primitiveFailFor: */
    primFailCode =
        (thisReceiver == (longAt(stackPointer + (argumentCount * BytesPerWord)))
             ? PrimErrBadReceiver
             : PrimErrBadArgument);
    return;
  }
  oldHash =
      (long32At((void *)(thisReceiver + 4))) & (identityHashHalfWordMask());
  ...
  if (isReceiverAClass) {
    ...
    /* this figures out if the index is ambiguous and fixes all the instances if
     * needed */
