...
followForwardedFrameContentsstackPointer(char *theFP, char *theSP)
{
	...
	toDoLimit = theFP + ((FoxCallerSavedIP + BytesPerWord) + ((((usqInt)((byteAt((theFP + FoxFrameFlags) + 1))) << (shiftForWord())))));
	for (ptr = ((theFP + FoxCallerSavedIP) + BytesPerWord); ptr <= toDoLimit; ptr += BytesPerWord) {
		oop = longAt(ptr);
		if (((!(oop & (tagMask()))))
		 && ((!((longAt((void *)(oop))) & ((classIndexMask()) - (isForwardedObjectClassIndexPun())))))) {
