...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	for (i = 1; i < rememberedSetSize; i += 1) {
		...
		if (((!(oop & (tagMask()))))
		 ...
		 && (!(((byteAt((void *)(oop + (formatFieldByteOffset())))) & (1U << (pinnedBitByteShift()))) != 0)))) {
			...
			assert((isNonImmediate(heapEntity))
			 && (!(isForwarded(heapEntity))));
			assert(validStorePointerUncheckedArgs(i, heapEntity, fwd));
			longAtput((void *)((heapEntity + BaseHeaderSize) + ((((usqInt)(i) << (shiftForWord()))))),fwd);
		}
	}
	if (/* isMobile: */
	...
	while (oopisLessThan(objOopSqInt, pastSpaceStart)) {
		...
		fmt = (byteAt((void *)(objOopSqInt + (formatFieldByteOffset())))) & (formatMask());
		if (fmt <= 5 /* lastPointerFormat */) {
			if ((fmt == (indexablePointersFormat()))
			 && (((longAt((void *)(objOopSqInt))) & (classIndexMask())) == ClassMethodContextCompactIndex)) {
				/* begin fetchStackPointerOf: */
				sp = longAt((void *)((objOopSqInt + BaseHeaderSize) + ((((usqInt)(StackPointerIndex) << (shiftForWord()))))));
				if (!((((sp) & 7) == 1))) {
					contextSize = 0;
					goto l3;
				}
...
l3:
			...
			numPointerSlots = (((numSlots = byteAt((void *)(objOopSqInt + (numSlotsFieldByteOffset()))))) == (numSlotsMask())
						? ((((usqInt)(((sqInt)((usqInt)((longAt((void *)(objOopSqInt - BaseHeaderSize)))) << 8)))))) >> 8
						: numSlots);
			goto l2;
		}
		...
		header = longAt((void *)((objOopSqInt + BaseHeaderSize) + ((((usqInt)(HeaderIndex) << (shiftForWord()))))));
		...
		for (i = 0; i < numPointerSlots; i += 1) {
			...
			if (((!(oop & (tagMask()))))
			 ...
			 && (!(((byteAt((void *)(oop + (formatFieldByteOffset())))) & (1U << (pinnedBitByteShift()))) != 0)))) {
				...
				longAtput((void *)((objOopSqInt + BaseHeaderSize) + ((((usqInt)(i) << (shiftForWord()))))),fwd);
			}
		}
		prevPrevObj = prevObj;
		prevObj = objOopSqInt;
...
l8:
					...
					numPointerSlots = (((numSlotsUsqInt = byteAt((void *)(objOop + (numSlotsFieldByteOffset()))))) == (numSlotsMask())
				...
				for (i = 0; i < numPointerSlots; i += 1) {
					...
					if (((!(oop & (tagMask()))))
					 ...
					 && (!(((byteAt((void *)(oop + (formatFieldByteOffset())))) & (1U << (pinnedBitByteShift()))) != 0)))) {
						...
						longAtput((void *)((objOop + BaseHeaderSize) + ((((usqInt)(i) << (shiftForWord()))))),fwd);
					}
				}
			}
			else {
