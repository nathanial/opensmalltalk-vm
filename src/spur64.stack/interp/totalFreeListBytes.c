...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	for (i = 1; i <= 0x3F /* (numFreeLists - 1) */; i += 1) {
		...
		while (listNode != 0) {
			...
			nextNode = longAt((void *)((listNode + BaseHeaderSize) + (0U << (shiftForWord()))));
	...
	do {
		...
		smallChild = longAt((void *)((treeNodeSqInt + BaseHeaderSize) + (3U << (shiftForWord()))));
		largeChild = longAt((void *)((treeNodeSqInt + BaseHeaderSize) + (4U << (shiftForWord()))));
		assert((smallChild == 0)
		...
		if (((smallChild == 0)
		 ...
		 || ((largeChild
			...
			treeNodeSqInt = longAt((void *)((treeNodeSqInt + BaseHeaderSize) + (2U << (shiftForWord()))));
		}
		else {
			if ((smallChild != 0)
			 && (cameFrom != smallChild)) {
