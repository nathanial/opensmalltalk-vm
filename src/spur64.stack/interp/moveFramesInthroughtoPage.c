...
/*	Move frames from the hot end of oldPage through to theFP to newPage.
        This has the effect of making theFP a base frame which can be stored
   into. Answer theFP's new location. */
...
moveFramesInthroughtoPage(StackPage *oldPage, char *theFP, StackPage *newPage) {
  ...
  stackedReceiverOffset =
      ...
      ((((usqInt)((byteAt((theFP + FoxFrameFlags) + 1))) << (shiftForWord()))));

  /* First move the data.  We will fix up frame pointers later. */
  for (source = (theFP + stackedReceiverOffset); source >= ((oldPage->headSP));
       source += (-BytesPerWord)) {
    newSP -= BytesPerWord;
  ...
  assert((newSP < ((newPage->baseAddress))) &&
         (newSP >
          (((newPage->realStackLimit)) - (LargeContextSlots * BytesPerOop))));
  assert((theFP1 < ((newPage->baseAddress))) &&
         (theFP1 > (((newPage->realStackLimit)) -
  ...
  assert((callerFP < ((oldPage->baseAddress))) &&
         (callerFP > (((oldPage->realStackLimit)) -
                      ((LargeContextSlots * BytesPerOop) / 2))));
  ...
  /* Now relocate frame pointers, updating married contexts to refer to their
   * moved spouse frames. */
  ...
  do {
    ...
    if (byteAt((fpInNewPage + FoxFrameFlags) + 2)) {
      ...
      assert(validStorePointerUncheckedArgs(SenderIndex, theContext,
                                            valuePointer));
