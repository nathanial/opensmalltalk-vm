...
static sqInt copyAndForward(sqInt survivor) {
  ...
  assert((isInEden(survivor)) || (isInPastSpace(survivor)));

  /* cog methods should be excluded. */
  bytesInObj = bytesInBody(survivor);
  format =
      (byteAt((void *)(survivor + (formatFieldByteOffset())))) & (formatMask());
      ...
      (/* shouldBeTenured: */
       (tenureCriterion == TenureByAge
            ...
            : (tenureCriterion == TenureToShrinkRT
                   ? (((usqInt)((byteAt(
                          (void *)(survivor + (formatFieldByteOffset())))))) >>
                      (rememberedBitByteShift())) >= refCountToShrinkRT
                   : (tenureCriterion == TenureByClass
                          ? ((longAt((void *)(survivor))) &
                             (classIndexMask())) == tenuringClassIndex
                          : 0))))) {
  ...
  assert((isInFutureSpace(newLocation)) || (isInOldSpace(newLocation)));

  /* begin storePointerUnchecked:ofObject:withValue: */
  assert((isNonImmediate(survivor)) && (!(isForwarded(survivor))));
  assert(validStorePointerUncheckedArgs(0, survivor, newLocation));
  longAtput((void *)((survivor + BaseHeaderSize) + (0U << (shiftForWord()))),
            newLocation);
  ...
  if (((format >= (weakArrayFormat())) && (format <= (ephemeronFormat())))) {
    if (format == (weakArrayFormat())) {
      ...
      if (!(isScavengeSurvivor((/* begin keyOfEphemeron: */
