/* Extracted from interp.c:58223 (function
 * pcPreviousToinSqueakV3PlusClosuresOrSistaV1Method). */

/* StackInterpreter>>#pcPreviousTo:inSqueakV3PlusClosuresOrSistaV1Method: */

usqInt pcPreviousToinSqueakV3PlusClosuresOrSistaV1Method(sqInt theIP,
                                                         sqInt aMethod) {
  sqInt byte;
  static unsigned char extensionSizes[] = {2, 2, 2, 2, 3, 2, 2, 1,
                                           1, 1, 2, 3, 3, 3, 3, 4};
  sqInt methodHeader;
  usqInt previousPC;
  usqInt thisIP;
  ...
  if ((((sqLong)methodHeader)) < 0) {
    ...
    thisIP =
        ((((literalCountOf(aMethod)) + LiteralStart) * BytesPerOop) + aMethod) +
        BaseHeaderSize;
                    ...
                    : (byte >= 160
                           ...
                           : (byte >= 144
                                  ...
                                  : (byte >= 128
                                         ...
                                         : (assertf("illegal bytecode in "
                                                    "sizeOfSqueakV3Bytecode:"),
                                            1))))));
  }
  return previousPC;
}