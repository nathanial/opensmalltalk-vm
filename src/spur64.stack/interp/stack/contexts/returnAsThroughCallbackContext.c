...
sqInt returnAsThroughCallbackContext(sqInt returnTypeOop,
  ...
  calloutMethodContext =
      externalInstVarofContext(SenderIndex, callbackMethodContext);
  ...
  if ((!((longAt((void *)((callbackMethodContext + BaseHeaderSize) +
    ...
    assert(validStorePointerUncheckedArgs(SenderIndex, callbackMethodContext,
                                          nilObj));
    longAtput((void *)((callbackMethodContext + BaseHeaderSize) +
                       ((((usqInt)(SenderIndex) << (shiftForWord()))))),
              nilObj);
    ...
    assert(validStorePointerUncheckedArgs(InstructionPointerIndex,
                                          callbackMethodContext, nilObj));
    longAtput(
        (void *)((callbackMethodContext + BaseHeaderSize) +
    ...
    if (framePointer == theFP) {
      ...
      if (longAt(theFP + FoxSavedFP)) {
        ...
        memcpy(reenterInterpreter,
               ((void *)((vmCallbackContext->savedReenterInterpreter))),
               sizeof(jmp_buf));
