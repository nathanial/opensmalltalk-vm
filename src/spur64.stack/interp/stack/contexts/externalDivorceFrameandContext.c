...
static void externalDivorceFrameandContext(char *theFP, sqInt ctxt) {
  ...
  assert((!stackPage) || (stackPage == (mostRecentlyUsedPage)));
  ...
  callerCtx = marryFrameSP(
      callerFP1, (/* begin frameCallerStackPointer: */
                  assert(!(isBaseFrame(theFP))),
...
l2:
  ...
  if (callerFP) {
    ...
    assert((callerSP < ((thePage->baseAddress))) &&
           (callerSP >
            (((thePage->realStackLimit)) - (LargeContextSlots * BytesPerOop))));
    assert((callerFP < ((thePage->baseAddress))) &&
           (callerFP > (((thePage->realStackLimit)) -
