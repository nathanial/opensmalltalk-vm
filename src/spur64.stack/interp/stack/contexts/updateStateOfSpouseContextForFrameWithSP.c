/* Extracted from interp.c:65480 (function
...
static void updateStateOfSpouseContextForFrameWithSP(char *theFP, char *theSP) {
  ...
  assert(frameReceiver(theFP) ==
         noFixupFollowFieldofObject(ReceiverIndex, theContext));
  assert(frameMethodObject(theFP) ==
         noFixupFollowFieldofObject(MethodIndex, theContext));
  tempIndex = byteAt((theFP + FoxFrameFlags) + 1);

  /* update the arguments. this would appear not to be strictly necessary, but
     is for two reasons. First, the fact that arguments are read-only is only as
     convention in the Smalltalk compiler; other languages may choose to modify
     arguments. Second, the Squeak runUntilErrorOrReturnFrom: nightmare pops the
     stack top, which may, in certain circumstances, be the last argument, and
     hence the last argument may not have been stored into the context. */
  pointer = theFP + ((FoxCallerSavedIP + BytesPerWord) +
                     ((tempIndex << (shiftForWord()))));
