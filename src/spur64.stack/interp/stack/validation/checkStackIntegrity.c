      ...
      while (1) {
        ...
        while (theSP <= frameRcvrOffset) {
          ...
          if (((!(oop & (tagMask())))) &&
            ...
            printFrameThingandFrameat("object leak in frame temp", theFP,
                                      theSP);
            ok = 0;
          }
          theSP += BytesPerWord;
        ...
        if (byteAt((theFP + FoxFrameFlags) + 2)) {
          ...
          if ((((oop & (tagMask())) != 0)) ||
              ((heapMapAtWord(pointerForOop(oop))) == 0)) {
            printFrameThingandFrameat("object leak in frame ctxt", theFP,
          ...
          if (!(/* isContext: */
            ...
            printFrameThingandFrameat("frame ctxt should be context", theFP,
                                      theFP + FoxThisContext);
            ok = 0;
          }
          if (!((/* isContext: */
          ...
          if (!((/* isContext: */
            ...
            printFrameThingandFrameat(
                "frame ctxt should be married to this frame ", theFP,
                theFP + FoxThisContext);
            ok = 0;
          }
        }
        ...
        if ((((oop & (tagMask())) != 0)) ||
            ((heapMapAtWord(pointerForOop(oop))) == 0)) {
          printFrameThingandFrameat("object leak in iframe mthd", theFP,
                                    theFP + FoxMethod);
          ok = 0;
        }
