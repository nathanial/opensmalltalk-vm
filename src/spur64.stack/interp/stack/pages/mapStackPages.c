...
static NeverInline void mapStackPages(void) {
  ...
  for (i = 0; i < numStackPages; i += 1) {
    ...
    if ((thePage->baseFP)) {
      ...
      while (1) {
        ...
        assert((theIPPtr == 0) || (addressIsInPage(thePage, theIPPtr)));
        while (theSP <= (theFP + FoxReceiver)) {
          oop = longAt(theSP);
          if (/* shouldRemapOop: */
              ((!(oop & (tagMask())))) && (shouldRemapObj(oop))) {
        ...
        if (byteAt((theFP + FoxFrameFlags) + 2)) {
          if (shouldRemapObj(longAt(theFP + FoxThisContext))) {
            longAtput(theFP + FoxThisContext,
                      remapObj(longAt(theFP + FoxThisContext)));
          }

          /* With SqueakV3 objectMemory or SpurPlanningCompactor can't assert
           * since object body is yet to move. */
  ...
  statMaxPageCountWhenMapping = ((statMaxPageCountWhenMapping < numLivePages)
                                     ? numLivePages
