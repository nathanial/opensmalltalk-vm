	...
	if ((oopisGreaterThanOrEqualTo(index, /* firstValidIndexOfIndexableObject:withFormat: */
	 ...
	 && (oopisLessThanOrEqualTo(index, stSize))) {
		...
		if (fmt <= 5 /* lastPointerFormat */) {
			...
			assert(validStorePointerArgs(fieldIndex, rcvr, valueSqInt));
			assert(isNonImmediate(rcvr));
			if (oopisGreaterThanOrEqualTo(rcvr, GIV(oldSpaceStart))) {
				if (/* isYoung: */
					((!(valueSqInt & (tagMask()))))
				 && (oopisLessThan(valueSqInt, GIV(oldSpaceStart)))) {
			...
			longAtput((void *)((rcvr + BaseHeaderSize) + ((((usqInt)(fieldIndex) << (shiftForWord()))))),valueSqInt);
			goto l4;
		}
		if (fmt >= (firstByteFormat())) {
			if (!((((valueSqInt) & 7) == 1))) {
			...
			byteAtput((void *)((rcvr + BaseHeaderSize) + ((index + fixedFields) - 1)),signedValueToStore);
			goto l4;
		}
		if (fmt >= (firstShortFormat())) {
			if (!((((valueSqInt) & 7) == 1))) {
			...
			shortAtput((void *)((rcvr + BaseHeaderSize) + ((((usqInt)(((index + fixedFields) - 1)) << 1)))),signedValueToStore);
			goto l4;
		}
		if (fmt == (sixtyFourBitIndexableFormat())) {
			...
			if (!GIV(primFailCode)) {
				...
				long64Atput((void *)((rcvr + BaseHeaderSize) + ((((usqInt)(((index + fixedFields) - 1)) << 3)))),unsigned64BitValueToStore);
			}
			goto l4;
		}
		...
		if (!GIV(primFailCode)) {
			...
			long32Atput((void *)((rcvr + BaseHeaderSize) + ((((usqInt)(((index + fixedFields) - 1)) << 2)))),unsignedValueToStore);
		}
