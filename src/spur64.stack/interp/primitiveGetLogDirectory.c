...
primitiveGetLogDirectory(void)
{
	...
	sz = strlen(ptr);
	stringOop = instantiateClassindexableSize(longAt((void *)((specialObjectsOop + BaseHeaderSize) + ((((usqInt)(ClassByteString) << (shiftForWord())))))), sz);
	for (i = 0; i < sz; i += 1) {
	...
	/* begin pop:thenPush: */
	longAtput((sp = stackPointer + (((argumentCount + 1) - 1) * BytesPerWord)),stringOop);
	stackPointer = sp;
	return 0;
}