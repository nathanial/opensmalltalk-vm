...
allocateSlotsForPinningInOldSpacebytesformatclassIndex(sqInt numSlots, usqInt totalBytes, sqInt formatField, sqInt classIndex)
{
				...
				while (node != 0) {
					...
					if (((segmentContainingObj(node))->containsPinned)) {
						if (prev) {
							...
							lilliputianSqInt = totalBytes == (BaseHeaderSize + 8 /* allocationUnit */);
							...
							assert((next == 0)
							 || (isFreeObject(next)));
							longAtput((void *)((prev + BaseHeaderSize) + (0U << (shiftForWord()))),next);
							if ((next != 0)
							 && (!lilliputianSqInt)) {
								/* begin storePointer:ofFreeChunk:withValue: */
								...
								assert((prev == 0)
								 || (isFreeObject(prev)));
								longAtput((void *)((next + BaseHeaderSize) + (1U << (shiftForWord()))),prev);
							}
						}
						else {
							...
							/* begin isLilliputianSize: */
							assert(totalBytes >= (BaseHeaderSize + (allocationUnit())));
							lilliputian = totalBytes == (BaseHeaderSize + 8 /* allocationUnit */);
							...
							if ((!lilliputian)
							 && (nextSqInt != 0)) {
								...
								longAtput((void *)((nextSqInt + BaseHeaderSize) + (1U << (shiftForWord()))),0);
							}
						}
						chunk = node;
						goto l1;
				...
				if ((node = freeLists[index])) {
					...
					while (node != 0) {
						...
						if (((segmentContainingObj(node))->containsPinned)) {
							...
							freeChunkWithBytesat((index * 8 /* allocationUnit */) - totalBytes, (/* startOfObject: */
	((byteAt((void *)(node + (numSlotsFieldByteOffset())))) == (numSlotsMask())
		? node - BaseHeaderSize
		: node)) + totalBytes);
							chunk = node;
							goto l1;
	...
	while (child != 0) {
		...
		if (childBytes == totalBytes) {
			...
			while (1) {
				...
				if (((segmentContainingObj(node))->containsPinned)) {
					/* begin assertValidFreeObject: */
					assert(assertInnerValidFreeObject(node));
					nextFreeChunk = longAt((void *)((node + BaseHeaderSize) + (0U << (shiftForWord()))));
					...
					longAtput((void *)((prev + BaseHeaderSize) + (0U << (shiftForWord()))),nextFreeChunk);
					if (nextFreeChunk) {
						/* begin storePointer:ofFreeChunk:withValue: */
						assert(isFreeObject(nextFreeChunk));
						assert((prev == 0)
						 || (isFreeObject(prev)));
						longAtput((void *)((nextFreeChunk + BaseHeaderSize) + (1U << (shiftForWord()))),prev);
					}
					chunk = /* startOfObject: */
							((byteAt((void *)(node + (numSlotsFieldByteOffset())))) == (numSlotsMask())
								? node - BaseHeaderSize
			...
			if (((segmentContainingObj(child))->containsPinned)) {
				next = longAt((void *)((child + BaseHeaderSize) + (0U << (shiftForWord()))));
				if (next) {
				...
				chunk = /* startOfObject: */
						((byteAt((void *)(child + (numSlotsFieldByteOffset())))) == (numSlotsMask())
		...
		if (child) {
			if (childBytes <= (totalBytes + 8 /* allocationUnit */)) {
				...
				node = child;
				child = longAt((void *)((node + BaseHeaderSize) + (3U << (shiftForWord()))));
				if (!acceptedNode) {
					acceptedChunk = node;

					/* first search the list. */
					do {
						acceptedChunk = longAt((void *)((acceptedChunk + BaseHeaderSize) + (0U << (shiftForWord()))));
						if ((acceptedChunk != 0)
	...
	if (acceptedNode) {
		if (acceptedChunk) {
			...
			while (1) {
				next = longAt((void *)((acceptedNode + BaseHeaderSize) + (0U << (shiftForWord()))));
				if (!(next != acceptedChunk)) break;
				acceptedNode = next;
			}
			nextFreeChunk = longAt((void *)((acceptedChunk + BaseHeaderSize) + (0U << (shiftForWord()))));
			...
			assert((nextFreeChunk == 0)
			 || (isFreeObject(nextFreeChunk)));
			longAtput((void *)((acceptedNode + BaseHeaderSize) + (0U << (shiftForWord()))),nextFreeChunk);
			if (nextFreeChunk) {
				/* begin storePointer:ofFreeChunk:withValue: */
				assert(isFreeObject(nextFreeChunk));
				assert((acceptedNode == 0)
				 || (isFreeObject(acceptedNode)));
				longAtput((void *)((nextFreeChunk + BaseHeaderSize) + (1U << (shiftForWord()))),acceptedNode);
			}
			freeChunkWithBytesat((bytesInBody(acceptedChunk)) - totalBytes, (/* startOfObject: */
	((byteAt((void *)(acceptedChunk + (numSlotsFieldByteOffset())))) == (numSlotsMask())
		...
		freeChunkWithBytesat((bytesInBody(acceptedNode)) - totalBytes, (/* startOfObject: */
	((byteAt((void *)(acceptedNode + (numSlotsFieldByteOffset())))) == (numSlotsMask())
...
l1:
	...
	if (numSlots >= (numSlotsMask())) {
		longAtput((void *)(chunk),numSlots + ((((usqInt)((numSlotsMask())) << (numSlotsFullShift())))));
		longAtput((void *)(chunk + BaseHeaderSize),(((((((usqLong) (numSlotsMask()))) << (numSlotsFullShift()))) + ((((usqInt)(formatField) << (formatShift()))))) + classIndex) | (1U << (pinnedBitShift())));
	...
	longAtput((void *)(chunk),(((((((usqLong) numSlots)) << (numSlotsFullShift()))) + ((((usqInt)(formatField) << (formatShift()))))) + classIndex) | (1U << (pinnedBitShift())));
