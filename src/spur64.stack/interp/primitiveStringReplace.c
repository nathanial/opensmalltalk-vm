...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if ((stop >= start)
	 && (
#  if IMMUTABILITY
		((((usqInt)((byteAt((void *)(array + (immutableExtraBitsByteOffset())))))) >> (immutableBitByteShift())) & 1) != 0
#  else
		0
#  endif
		)) {
	...
	replFmt = (byteAt((void *)(repl + (formatFieldByteOffset())))) & (formatMask());
	...
	if (arrayFmt <= 5 /* lastPointerFormat */) {
		...
		arrayLength = (((numSlots = byteAt((void *)(array + (numSlotsFieldByteOffset()))))) == (numSlotsMask())
		...
		arrayInstSize = (((longAt((void *)((class + BaseHeaderSize) + ((((usqInt)(InstanceSpecificationIndex) << (shiftForWord()))))))) >> 3)) & ((1U << (fixedFieldsFieldWidth())) - 1);
		/* end fixedFieldsOf:format:length: */
l5:

		/* begin numSlotsOf: */
		assert((classIndexOf(repl)) > (isForwardedObjectClassIndexPun()));
		replLength = (((numSlots = byteAt((void *)(repl + (numSlotsFieldByteOffset()))))) == (numSlotsMask())
					? ((((usqInt)(((sqInt)((usqInt)((longAt((void *)(repl - BaseHeaderSize)))) << 8)))))) >> 8
					: numSlots);
		...
		replInstSize = (((longAt((void *)((class + BaseHeaderSize) + ((((usqInt)(InstanceSpecificationIndex) << (shiftForWord()))))))) >> 3)) & ((1U << (fixedFieldsFieldWidth())) - 1);
		/* end fixedFieldsOf:format:length: */
l6:
		if (!((start >= 1)
			 && (((start - 1) <= stop)
		...
		if (oopisGreaterThanOrEqualTo(array, oldSpaceStart)) {
			...
			for (i = (start - 1); i < stop; i += 1) {
				...
				longAtput((void *)((array + BaseHeaderSize) + ((((usqInt)(i) << (shiftForWord()))))),oop);
			}
			if (mustRemember) {
			...
			for (i = (start - 1); i < stop; i += 1) {
				valuePointer = longAt((void *)((repl + BaseHeaderSize) + ((((usqInt)((srcDelta + i)) << (shiftForWord()))))));

				/* begin storePointerUnchecked:ofObject:withValue: */
				assert((isNonImmediate(array))
				 && (!(isForwarded(array))));
				assert(validStorePointerUncheckedArgs(i, array, valuePointer));
				longAtput((void *)((array + BaseHeaderSize) + ((((usqInt)(i) << (shiftForWord()))))),valuePointer);
			}
		}
	...
	/* Array formats must be the same; but for copying, weak arrays are equivalent to arrays.
	   Non-pointer array formats must match */
	...
	if (arrayFmt >= (firstShortFormat())) {
		if (arrayFmt >= (firstByteFormat())) {
			for (i = (start - 1); i < stop; i += 1) {
				...
				byteAtput((void *)((array + BaseHeaderSize) + i),byteAt((void *)((repl + BaseHeaderSize) + (srcDelta + i))));
			}
		}
		else {
			for (i = (start - 1); i < stop; i += 1) {
				/* storeShort16:ofObject:withValue: */
				shortAtput((void *)((array + BaseHeaderSize) + ((((usqInt)(i) << 1)))),shortAt((void *)((repl + BaseHeaderSize) + ((((usqInt)((srcDelta + i)) << 1))))));
			}
		}
	}
	else {
		if (arrayFmt >= (firstLongFormat())) {
			for (i = (start - 1); i < stop; i += 1) {
				...
				long32Atput((void *)((array + BaseHeaderSize) + ((((usqInt)(i) << 2)))),long32At((void *)((repl + BaseHeaderSize) + ((((usqInt)((srcDelta + i)) << 2))))));
			}
		}
		else {
			for (i = (start - 1); i < stop; i += 1) {
				/* storeLong64:ofObject:withValue: */
				long64Atput((void *)((array + BaseHeaderSize) + ((((usqInt)(i) << 3)))),long64At((void *)((repl + BaseHeaderSize) + ((((usqInt)((srcDelta + i)) << 3))))));
			}
		}
	}
