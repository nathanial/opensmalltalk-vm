...
/*	Crash the VM by indirecting through a null pointer. If the sole argument
   ...
   If the argument is an integer use the method that implies. bit 0 = thread to
   crash in; 1 => this thread bit 1 = crash method; 0 => indirect through null
   pointer; 1 => call exit */
...
primitiveCrashVM(void) {
  ...
  if (primFailCode || (argumentCount != 1)) {
