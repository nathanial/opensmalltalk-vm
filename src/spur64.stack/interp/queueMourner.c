...
static void queueMourner(sqInt anEphemeronOrWeakArray) {
  ...
  /* There is no point queueing weak arrays more than once.  Note that it should
     be impossible for ephemerons to be enqueued more than once since they are
     turned into non-ephemerons in the relevant sender.  Alas this means that
     other ephemerons on the same object not identified in this scavenge or GC
     will not fire until later.  But that's life. */

  /* begin isEphemeron: */
  assert(isNonImmediate(anEphemeronOrWeakArray));
  format =
      (byteAt((void *)(anEphemeronOrWeakArray + (formatFieldByteOffset())))) &
      (formatMask());
  ...
  if (((anEphemeronOrWeakArray & (tagMask())) != 0)) {
    ...
    assert(
        !((objStack == markStack) && (isWeakNonImm(anEphemeronOrWeakArray))));
