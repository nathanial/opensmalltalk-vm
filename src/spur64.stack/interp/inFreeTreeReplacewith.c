...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	for (i = 2 /* freeChunkParentIndex */; i <= 4 /* freeChunkLargerIndex */; i += 1) {
		relative = longAt((void *)((treeNode + BaseHeaderSize) + ((((usqInt)(i) << (shiftForWord()))))));
		if (i == 2 /* freeChunkParentIndex */) {
			if (relative) {
				fieldIndex = (treeNode == (longAt((void *)((relative + BaseHeaderSize) + (3U << (shiftForWord())))))
			...
			if (relative) {
				...
				assert((newNode == 0)
				 || (isFreeObject(newNode)));
				longAtput((void *)((relative + BaseHeaderSize) + (2U << (shiftForWord()))),newNode);
			}
		}
		...
		longAtput((void *)((newNode + BaseHeaderSize) + ((((usqInt)(i) << (shiftForWord()))))),relative);

		/* begin storePointer:ofFreeChunk:withValue: */
		assert(isFreeObject(treeNode));
		longAtput((void *)((treeNode + BaseHeaderSize) + ((((usqInt)(i) << (shiftForWord()))))),0);
	}
}