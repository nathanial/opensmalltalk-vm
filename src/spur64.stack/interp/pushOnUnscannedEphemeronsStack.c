...
/*	Attempt to push anEphemeron on the unscanned ephemerons stack and answer
        if the attempt succeeded. Note that the ephemeron stack overflowing
   isn't a disaster; it simply means treating the ephemeron as strong in this GC
        cycle.  */
...
static NoDbgRegParms sqInt pushOnUnscannedEphemeronsStack(sqInt anEphemeron) {
  ...
  longAtput((void *)((unscannedEphemerons.top =
                          ((unscannedEphemerons.top)) + BytesPerOop)),
            anEphemeron);
  return 1;
}