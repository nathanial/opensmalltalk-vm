...
static NeverInline void handleStackOverflow(void) {
  ...
  /* The stack has overflowed this page.  If the system is executing some
     recursive algorithm, e.g. fibonacci, then the system could thrash
     overflowing the stack if the call soon returns back to the current page. To
     avoid thrashing, since overflow is quite slow, we can move more than one
     frame.  The idea is to record which page has overflowed, and the first time
     it overflows move one frame, the second time two frames, and so on.  We
     move no more frames than would leave the page half occupied. */
  theFP = framePointer;
...
l1:
  ...
  assert(validInstructionPointerinMethodframePointer(instructionPointer + 1,
                                                     method, framePointer));
}