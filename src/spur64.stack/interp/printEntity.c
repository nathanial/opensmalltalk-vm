...
void printEntity(sqInt oop) {
  ...
  if (!(addressCouldBeObj(oop))) {
    print((((oop & (tagMask())) != 0) ? "immediate" : "unknown"));
    return;
  }
  print((((longAt((void *)(oop))) & (classIndexMask())) ==
  ...
  /* begin printHexnpnp: */
  fprintf(transcript, "%" PRIxSQINT "", n);
  print("/");
  n = bytesInBody(oop);
  ...
  if (printFlags) {
    /* begin space */
    printChar(' ');
    print((((byteAt((void *)(oop + (formatFieldByteOffset())))) &
            (formatMask())) <= 15
               ? "f:0"
               : "f:"));
    ...
    /* begin printHexnpnp: */
    fprintf(transcript, "%" PRIxSQINT "", n);
    print(((byteAt((void *)(oop + (markBitsByteOffset())))) &
                   (1U << (greyBitByteShift()))
               ? " g"
               : " ."));
    print(((((usqInt)((
