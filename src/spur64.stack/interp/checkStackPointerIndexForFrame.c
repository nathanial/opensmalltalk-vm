...
/*	Version of stackPointerIndexForFrame: that does not depend on writing
   back head frame pointers. Used for assertion checking. Safe only in external
   primitives (framePointer valid). Answer the 0-based index rel to the given
   frame. (This is what stackPointer used to be before conversion to pointer) */
...
static sqInt checkStackPointerIndexForFrame(char *theFP) {
  ...
  if (theFP == framePointer) {
    return (((usqInt)(((theFP + FoxReceiver) - stackPointer))) >>
            (shiftForWord())) +
  ...
  thePage = stackPageAtpages(
      ...
      pages);
  startFrame = (thePage == stackPage ? framePointer : (thePage->headFP));
  ...
  if (startFrame == theFP) {
    ...
    theSP = (thePage == stackPage ? (thePage->headSP)
  ...
  while (1) {
    ...
    if (theFP == aFrame) {
      ...
      theSP =
          (prevFrame + ((FoxCallerSavedIP + BytesPerWord) +
                        ((((usqInt)((byteAt((prevFrame + FoxFrameFlags) + 1)))
                           << (shiftForWord())))))) +
          BytesPerWord;
      goto l1;
    }
