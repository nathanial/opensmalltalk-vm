...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	stackOrNil = objStack;
	freeList = longAt((void *)((objStack + BaseHeaderSize) + ((((usqInt)(ObjStackFreex) << (shiftForWord()))))));
	while (1) {
		...
		next = longAt((void *)((stackOrNil + BaseHeaderSize) + ((((usqInt)(ObjStackNextx) << (shiftForWord()))))));
		finishIndex = ObjStackNextx + ((relocateContents
		? (long32At((void *)(stackOrNil + 4))) & (identityHashHalfWordMask())
		: 0));
		...
		for (i = ObjStackFreex; i <= finishIndex; i += 1) {
			...
			if (((!(oop & (tagMask()))))
			 ...
			 && (!(((byteAt((void *)(oop + (formatFieldByteOffset())))) & (1U << (pinnedBitByteShift()))) != 0)))) {
				...
				assert((isNonImmediate(stackOrNil))
				 && (!(isForwarded(stackOrNil))));
				assert(validStorePointerUncheckedArgs(i, stackOrNil, fwd));
				longAtput((void *)((stackOrNil + BaseHeaderSize) + ((((usqInt)(i) << (shiftForWord()))))),fwd);
			}
		}
		relocated = (/* isMobile: */
	...
	while (freeList != 0) {
		...
		next = longAt((void *)((freeList + BaseHeaderSize) + ((((usqInt)(ObjStackFreex) << (shiftForWord()))))));
		...
		for (i = ObjStackFreex; i <= ObjStackFreex; i += 1) {
			...
			if (((!(oop & (tagMask()))))
			 ...
			 && (!(((byteAt((void *)(oop + (formatFieldByteOffset())))) & (1U << (pinnedBitByteShift()))) != 0)))) {
				...
				/* begin storePointerUnchecked:ofObject:withValue: */
				assert((isNonImmediate(freeList))
				 && (!(isForwarded(freeList))));
				assert(validStorePointerUncheckedArgs(i, freeList, fwd));
				longAtput((void *)((freeList + BaseHeaderSize) + ((((usqInt)(i) << (shiftForWord()))))),fwd);
			}
		}
		if (/* isMobile: */
