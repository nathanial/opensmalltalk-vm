...
l1:
  ...
  assert((isNonImmediate(encoderNames)) && (!(isForwarded(encoderNames))));
  assert(validStorePointerUncheckedArgs(0, encoderNames, valuePointer));
  longAtput(
      (void *)((encoderNames + BaseHeaderSize) + (0U << (shiftForWord()))),
      valuePointer);
  ...
  longAtput(
      (void *)((encoderNames + BaseHeaderSize) + (1U << (shiftForWord()))),
      valuePointer);
