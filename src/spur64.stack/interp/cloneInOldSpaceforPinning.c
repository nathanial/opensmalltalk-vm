...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	numSlots = (((numSlotsUsqInt = byteAt((void *)(objOop + (numSlotsFieldByteOffset()))))) == (numSlotsMask())
	...
	if (forPinning) {
		...
		: BaseHeaderSize))
					: 8 /* allocationUnit */ + BaseHeaderSize), fmt, (longAt((void *)(objOop))) & (classIndexMask()));
	}
	else {
	...
	if (fmt <= 5 /* lastPointerFormat */) {
		...
		for (i = 0; i < numSlots; i += 1) {
			...
			longAtput((void *)((newObj + BaseHeaderSize) + ((((usqInt)(i) << (shiftForWord()))))),oop);
		}
		if (hasYoung) {
			remember(newObj);
		}
		...
		for (i = 0; i < numSlots; i += 1) {
			...
			longAtput((void *)((newObj + BaseHeaderSize) + ((((usqInt)(i) << (shiftForWord()))))),valuePointer);
		}
	...
	if ((hash = (long32At((void *)(objOop + 4))) & (identityHashHalfWordMask()))) {
		/* begin setHashBitsOf:to: */
		long32Atput((void *)(newObj + 4),((((long32At((void *)(newObj + 4))) | (identityHashHalfWordMask())) - (identityHashHalfWordMask()))) + (hash & (identityHashHalfWordMask())));
	}
	if (
#  if IMMUTABILITY
		((((usqInt)((byteAt((void *)(objOop + (immutableExtraBitsByteOffset())))))) >> (immutableBitByteShift())) & 1) != 0
#  else
		0
#  endif
		) {
		/* begin setIsImmutableOf:to: */
		longAtput((void *)(newObj),(longAt((void *)(newObj))) | (1U << (immutableBitShift())));
	}
	return newObj;
}