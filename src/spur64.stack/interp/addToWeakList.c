...
/*	weakCorpse is the corpse of a weak array that was copied and forwarded.
        ...
        Thread the corpse onto the weakList. Later, the weakList can be
   followed, and the forwarding pointer followed to locate the survivor. */
...
static NoDbgRegParms NeverInline void addToWeakList(sqInt weakCorpse) {
  ...
  long64Atput((void *)(weakCorpse),
              headerForSlotshashformatclassIndex(
                  byteAt((void *)(weakCorpse + (numSlotsFieldByteOffset()))),
                  ((usqInt)(weakListOffset)) >> (formatFieldWidthShift()),
                  weakListOffset & (formatMask()),
                  isForwardedObjectClassIndexPun()));
  weakList = (((usqInt)((weakCorpse - newSpaceStart))) >>
