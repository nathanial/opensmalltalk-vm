...
l1:
	...
	if (format == (sixtyFourBitIndexableFormat())) {
		...
		for (index = 1; index <= lastIndex; index += 1) {
			fprintf(aStream,
					"%18" PRIXSQINT "%c",
					((usqLong) (long64At((void *)((oop + BaseHeaderSize) + ((((usqInt)((index - 1)) << 3))))))),
	...
	if (format < (firstShortFormat())) {
		...
		for (index = 1; index <= lastIndex; index += 1) {
			fprintf(aStream,
					"%10p%c",
					((void *)(long32At((void *)((oop + BaseHeaderSize) + ((((usqInt)((index - 1)) << 2))))))),
					(((index % elementsPerLine) == 0)
			 || (index == lastIndex)
					? '\n'
					: ' '));
	...
	if (format < (firstByteFormat())) {
		...
		for (index = 1; index <= lastIndex; index += 1) {
			fprintf(aStream,
					"%6p%c",
					((void *)(shortAt((void *)((oop + BaseHeaderSize) + ((((usqInt)((index - 1)) << 1))))))),
					(((index % elementsPerLine) == 0)
			 || (index == lastIndex)
					? '\n'
					: ' '));
