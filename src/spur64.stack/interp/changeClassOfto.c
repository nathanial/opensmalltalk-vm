...
#  endif
	...
	classFormat = ((longAt((void *)((argClass + BaseHeaderSize) + ((((usqInt)(InstanceSpecificationIndex) << (shiftForWord()))))))) >> 3);
	fixedFields = classFormat & ((1U << (fixedFieldsFieldWidth())) - 1);
	classFormat = (((usqInt)(classFormat)) >> (fixedFieldsFieldWidth())) & (formatMask());
	instFormat = (byteAt((void *)(rcvr + (formatFieldByteOffset())))) & (formatMask());
	...
	if (classFormat <= 5 /* lastPointerFormat */) {
		...
		(((numSlots = byteAt((void *)(rcvr + (numSlotsFieldByteOffset()))))) == (numSlotsMask())
					? (instSlots = ((((usqInt)(((sqInt)((usqInt)((longAt((void *)(rcvr - BaseHeaderSize)))) << 8)))))) >> 8)
		...
		if ((instFormat == (indexablePointersFormat()))
		 && (((longAt((void *)(rcvr))) & (classIndexMask())) == ClassMethodContextCompactIndex)) {
...
l1:
	...
	longAtput((void *)(rcvr),((longAt((void *)(rcvr))) & (~(usqIntptr_t)(((((usqInt)((formatMask())) << (formatShift())))) + (classIndexMask())))) + (classIndex + ((((usqInt)(newFormat) << (formatShift()))))));
