...
/*	Release the VM to other threads and answer the current thread's index.
        ...
        DisownVMForFFICall		- informs the VM that it is entering an
   FFI call
...
void *disownVM(sqInt flags) {
  ...
  /* In the single-threaded VM here, we treat flags directly as vmHandle. See
   * MTVM. */
  vmHandle = ((void *)flags);

  /* We are either in disowning after a callback (see thunkEntry) or before an
     FFI callout. */
  assert((flags == DisownVMFromCallback) ||
         ((flags == (flags & (DisownVMForFFICall + DisownVMForThreading))) &&
          (((flags & DisownVMForFFICall) != 0))));

  /* If DisownVMForFFICall this is from the FFI plugin and we're making a
   * callout; remember the fact. */
  ...
  /* Otherwise this is a callback return; restore argumentCount and newMethod as
   * per the ownVM: on callback. */
  if (((flags & DisownVMFromCallback) != 0)) {
    argumentCount = ((popStack()) >> 3);
    assert(((argumentCount >= 0) &&
            (argumentCount <= (argumentCountOfMethodHeader(-1)))));
