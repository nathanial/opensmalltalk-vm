...
/*	Reduce the number of indexable fields in objOop, a pointer object, to
        ...
        Without changes to numSlotsForShortening:toIndexableSize: this only
   works for arrayFormat, longFormat, and on 64-bits
   sixtyFourBitIndexableFormat, objects. Answer the number of bytes returned to
   free memory, which may be zero if no change was possible. */
...
static sqInt doShortentoIndexableSize(sqInt objOop,
  ...
  switch ((byteAt((void *)(objOop + (formatFieldByteOffset())))) &
          (formatMask())) {
  case arrayFormat():
  ...
  delta =
      bytesBefore - (/* objectBytesForSlots: */
  ...
  if (!delta) {
    if ((byteAt((void *)(objOop + (numSlotsFieldByteOffset())))) ==
      ...
      longAtput((void *)(objOop - BaseHeaderSize),
                ((((usqInt)((numSlotsMask())) << 56))) + numSlots);
    } else {
      /* rawNumSlotsOf:put: */
  ...
  /* Currently we can't have one word gaps in oldSpace but we can in newSpace.
     So only create a copy and forward if objOop is in old space. If in newSpace
     we'll create a slimbridge below. */
  ...
  l1:
    format =
        ((formatSqInt == (firstLongFormat())) && (((indexableSize & 1) != 0))
             ? formatSqInt + 1
             : formatSqInt);
    ...
    if ((freeStart + numBytes) > scavengeThreshold) {
      ...
      copy = allocateSlotsInOldSpacebytesformatclassIndex(numSlots, numBytes,
                                                          format, classIndex);
      goto l2;
    }
    if (numSlots >= (numSlotsMask())) {
  ...
  l2:
    if (!copy) {
      error("shorten:toIndexableSize: attempted to shorten by allocationUnit "
            "and failed to allocate space for copy!");
    }
    if (/* isPureBitsFormat: */
        (format >= (sixtyFourBitIndexableFormat())) &&
        (format < (firstCompiledMethodFormat()))) {
      memcpy(firstIndexableField(copy), firstIndexableField(objOop),
      ...
      for (i = 0; i < numSlots; i += 1) {
        ...
        longAtput((void *)((copy + BaseHeaderSize) +
                           ((((usqInt)(i) << (shiftForWord()))))),
                  valuePointer);
      }
    }
    ...
    if (oopisGreaterThanOrEqualTo(objOop, oldSpaceStart)) {
      if (/* isYoung: */
          ((!(copy & (tagMask())))) && (oopisLessThan(copy, oldSpaceStart))) {
  ...
  if ((byteAt((void *)(objOop + (numSlotsFieldByteOffset())))) ==
    ...
    /* Setting an overflow slot count to 0 or 1 in newSpace creates a
       slimbridge. So we must also change the normal slot count so that the
       object no longer has the overflow header word, which has become the
       slimbridge. */
    if (numSlots <= 1) {
  ...
  /* {self addressAfter: objOop. delta. (self addressAfter: objOop) + delta.
   * followingAddress} collect: #hex */
  delta = followingAddress - (addressAfter(objOop));
  assert((delta >= (allocationUnit())) && ((delta % (allocationUnit())) == 0));
  if (delta == 8 /* allocationUnit */) {
    ...
    if (oopisGreaterThanOrEqualToandLessThan(objOop, oldSpaceStart,
                                             endOfMemory)) {
      ...
      assert((((sixtyFourBitLongsClassIndexPun()) >= 0) &&
              ((sixtyFourBitLongsClassIndexPun()) <= (classIndexMask()))));
      longAtput((void *)(freeChunk), ((longAt((void *)(freeChunk))) &
                                      (~(usqIntptr_t)(classIndexMask()))) +
                                         (sixtyFourBitLongsClassIndexPun()));

      /* begin setFormatOf:to: */
      assert((((firstLongFormat()) >= 0) &&
              ((firstLongFormat()) <= (formatMask()))));
