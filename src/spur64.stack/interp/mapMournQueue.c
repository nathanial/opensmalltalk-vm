...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	while (objStackPage != 0) {
		...
		for (iSqInt = ((numOnThisPage + ObjStackFixedSlots) - 1); iSqInt >= ObjStackFixedSlots; iSqInt += -1) {
			...
			if ((!(mourner & (tagMask())))) {
				if ((!((longAt((void *)(mourner))) & ((classIndexMask()) - (isForwardedObjectClassIndexPun()))))) {
					/* begin followForwarded: */
					assert(isUnambiguouslyForwarder(mourner));
					referent = longAt((void *)((mourner + BaseHeaderSize) + (0U << (shiftForWord()))));
				...
				longAtput((void *)((objStackPage + BaseHeaderSize) + ((((usqInt)(iSqInt) << (shiftForWord()))))),mourner);
			}
		}
