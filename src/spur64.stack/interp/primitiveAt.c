/* Extracted from interp.c:12065 (function primitiveAt). */
	...
	if ((oopisGreaterThanOrEqualTo(index, /* firstValidIndexOfIndexableObject:withFormat: */
	 ...
	 && (oopisLessThanOrEqualTo(index, stSize))) {
		...
		if (fmt <= 5 /* lastPointerFormat */) {
			result = longAt((void *)((rcvr + BaseHeaderSize) + ((((usqInt)(((index + fixedFields) - 1)) << (shiftForWord()))))));
			goto l4;
		}
		if (fmt >= (firstByteFormat())) {
			result = (((usqInt)(byteAt((void *)((rcvr + BaseHeaderSize) + ((index + fixedFields) - 1)))) << 3) | 1);
			goto l4;
		}
		if (fmt >= (firstShortFormat())) {
			result = (((usqInt)(((unsigned short) (shortAt((void *)((rcvr + BaseHeaderSize) + ((((usqInt)(((index + fixedFields) - 1)) << 1)))))))) << 3) | 1);
			goto l4;
		}
		if (fmt == (sixtyFourBitIndexableFormat())) {
			result = positive64BitIntegerFor(long64At((void *)((rcvr + BaseHeaderSize) + ((((usqInt)(((index + fixedFields) - 1)) << 3))))));
			goto l4;
		}

		/* 32bit-word type objects; for now assume no 64-bit indexable objects */
		result = ((((((usqInt)(long32At((void *)((rcvr + BaseHeaderSize) + ((((usqInt)(((index + fixedFields) - 1)) << 2)))))))) & 0xFFFFFFFFU) << 3) | 1);
		goto l4;
	}
