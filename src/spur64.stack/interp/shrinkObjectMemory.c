...
static NoDbgRegParms int shrinkObjectMemory(usqInt delta) {
  ...
  while (1) {
    ...
    if ((!emptySeg) || (((emptySeg->segSize)) > shrinkage)) {
      /* begin checkFreeSpace: */
      assert(bitsSetInFreeSpaceMaskForAllFreeLists());
      assert(totalFreeOldSpace == (totalFreeListBytes()));
      if (((checkForLeaks & (GCCheckFreeSpace | GCCheckFreeSpace)) ==
  ...
  l1:
    ...
    sqDeallocateMemorySegmentAtOfSize(((void *)((emptySeg->segStart))),
                                      (emptySeg->segSize));
    for (j = i; j <= (numSegments - 2); j += 1) {
      segments[j] = (segments[j + 1]);
    }
    numSegments -= 1;
    bridgeFromto((&(segments[i - 1])),
                 (i <= (numSegments - 1) ? (&(segments[i])) : 0));
    segInfo = (&(segments[numSegments - 1]));

    /* begin setLastSegment: */
    currentEnd =
        (((segInfo->segSize)) + ((segInfo->segStart))) - (2 * BaseHeaderSize);
