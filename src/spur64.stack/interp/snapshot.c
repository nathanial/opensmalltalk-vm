...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (!primFailCode) {
		...
		assert(validStorePointerUncheckedArgs((stackIndex + CtxtTempFrameStart) - 1, activeContext, trueObj));
		longAtput((void *)((activeContext + BaseHeaderSize) + ((((usqInt)(((stackIndex + CtxtTempFrameStart) - 1)) << (shiftForWord()))))),trueObj);
