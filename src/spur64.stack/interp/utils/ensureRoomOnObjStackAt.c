  ...
  if ((stackOrNil == nilObj) ||
    ...
    if (freeOrNewPage) {
      ...
      longAtput((void *)((stackOrNil + BaseHeaderSize) +
                ...
                0);
      assert((!marking) || (isMarked(freeOrNewPage)));
    } else {
      freeOrNewPage = allocateSlotsInOldSpacebytesformatclassIndex(
          ObjStackPageSlots,
          ((((usqInt)(ObjStackPageSlots) << (shiftForWord())))) +
      ...
      /* Allocate a new segment and retry. This is very uncommon. But it
       * happened to me (Clement). */

      /* begin storePointer:ofObjStack:withValue: */
      assert((formatOf(freeOrNewPage)) == (wordIndexableFormat()));
      longAtput((void *)((freeOrNewPage + BaseHeaderSize) +
                         ((((usqInt)(ObjStackFreex) << (shiftForWord()))))),
                0);
      if (marking) {
        /* begin setIsMarkedOf:to: */
        assert(!(isFreeObject(freeOrNewPage)));
        byteAtput((void *)(freeOrNewPage + (markBitsByteOffset())),
                  (byteAt((void *)(freeOrNewPage + (markBitsByteOffset())))) |
    ...
    longAtput((void *)((freeOrNewPage + BaseHeaderSize) +
              ...
              (stackOrNil == nilObj ? 0 : stackOrNil));
