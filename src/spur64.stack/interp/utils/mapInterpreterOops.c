...
static void mapInterpreterOops(void) {
  ...
  if (/* shouldRemapOop: */
      ((!(newMethod & (tagMask())))) && (shouldRemapObj(newMethod))) {
  ...
  if (profileProcess) {
    if ((result =
             (shouldRemapObj(profileProcess) ? remapObj(profileProcess) : 0))) {
      profileProcess = result;
    }
  }
  if (profileMethod) {
    if ((result =
             (shouldRemapObj(profileMethod) ? remapObj(profileMethod) : 0))) {
      profileMethod = result;
    }
  }
  if (profileSemaphore) {
    if ((result = (shouldRemapObj(profileSemaphore) ? remapObj(profileSemaphore)
                                                    : 0))) {
    ...
    if (longRunningPrimitiveCheckSequenceNumber == statCheckForEvents) {
      ...
      if (shouldRemapObj(longRunningPrimitiveCheckMethod)) {
        longRunningPrimitiveCheckMethod =
            remapObj(longRunningPrimitiveCheckMethod);
      }
    }
    sqLowLevelMFence();
  }
  if (longRunningPrimitiveCheckSemaphore) {
    if (shouldRemapObj(longRunningPrimitiveCheckSemaphore)) {
      longRunningPrimitiveCheckSemaphore =
          remapObj(longRunningPrimitiveCheckSemaphore);
    }
  }
#endif // LRPCheck
  ...
  if ((tempOop != 0) &&
      (/* shouldRemapOop: */
       ((!(tempOop & (tagMask())))) && (shouldRemapObj(tempOop)))) {
    tempOop = remapObj(tempOop);
  }
  if ((tempOop2 != 0) &&
      (/* shouldRemapOop: */
       ((!(tempOop2 & (tagMask())))) && (shouldRemapObj(tempOop2)))) {
