...
/*	Set the dimensions of the heap, answering the start of oldSpace.
   edenBytes holds the desired ``size of eden'' which is actually the total size
   of new space minus the reserve. edenBytes is then divided up between eden and
   the two survivor spaces, where each survivor space is a scavengerDenominator
   (one seventh) of the total. */
/*	Transcript
        cr; nextPutAll: 'heapBase: '; print: baseOfHeap; nextPut: $/;
   nextPutAll: baseOfHeap hex; nextPutAll: ' memLimit '; print: memLimit;
   nextPut: $/; nextPutAll: memLimit hex; nextPutAll: ' memEnd '; print: memEnd;
   nextPut: $/; nextPutAll: memEnd hex; cr; flush. */
...
static NeverInline sqInt setHeapBasememoryLimitendOfMemory(sqInt baseOfHeap,
  ...
  survivorBytes = ((((oldSpaceStart - newSpaceStart) - reserve) /
                    7 /* scavengerDenominator */) &
                   ~7);
  actualEdenBytes = (((totalBytes - survivorBytes) - survivorBytes) & ~7);
  assert((((totalBytes - actualEdenBytes) - survivorBytes) - survivorBytes) <
         (allocationUnit()));

  /* for tenuring we require older objects below younger objects.  since
     allocation grows up this means that the survivor spaces must precede eden.
   */
  ...
  assert((((((eden).start)) % (allocationUnit())) +
          ((((eden).limit)) % (allocationUnit()))) == 0);
  assert((((((pastSpace).start)) % (allocationUnit())) +
          ((((pastSpace).limit)) % (allocationUnit()))) == 0);
  assert((((((futureSpace).start)) % (allocationUnit())) +
          ((((futureSpace).limit)) % (allocationUnit()))) == 0);
