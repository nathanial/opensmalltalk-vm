...
sqInt printFrameWithSP(char *theFP, char *theSP) {
  ...
  /* empty frameRange:to: */
  printFrameOopat("rcvr/clsr", theFP + ((FoxCallerSavedIP + BytesPerWord) +
                                        ((numArgs << (shiftForWord())))));
  for (i = numArgs; i >= 1; i += -1) {
    printFrameOopat("arg", (theFP + FoxCallerSavedIP) + (i * BytesPerWord));
  }

  /* begin printFrameThing:at: */
  printFrameThingatextraString("cllr ip/ctxt", theFP + FoxCallerSavedIP,
                               ((char *)null));
  ...
  /* begin printFrameThing:at: */
  printFrameThingatextraString("context", theFP + FoxThisContext,
                               ((char *)null));
  ...
  if ((topThing >= theMethod) &&
    ...
    for (addr = ((theFP + FoxReceiver) - BytesPerWord); addr >= theSP;
         addr += (-BytesPerWord)) {
      printFrameOopat("temp/stck", addr);
    }
  }
