...
static sqInt printPrimLogEntryAthasParameter(sqInt i, sqInt hasParameter) {
  char *className;
  sqInt entryOop;
  sqInt length;
  sqInt methodClass;
  ...
  if (((entryOop & (tagMask())) != 0)) {
    ...
    if (addressCouldBeObj(entryOop)) {
      if (((byteAt((void *)(entryOop + (formatFieldByteOffset())))) &
        ...
        if (methodSel == nilObj) {
          fprintf(transcript, "%.*s>>(selector not found)\n", ((int)length),
                  className);
        } else {
