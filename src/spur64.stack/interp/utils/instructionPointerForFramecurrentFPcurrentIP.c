/* Extracted from interp.c:54870 (function
 * instructionPointerForFramecurrentFPcurrentIP). */

/*	Answer the bytecode pc object (i.e. SmallInteger) for an active frame.
   The bytecode pc is derived from the frame's pc. If the frame is the top frame
        on the current stack
        the frame pc is whatever the current instruction pointer is. If the
   frame is the top frame on some other stack the frame pc is the value on top
   of stack. Otherwise the frame pc is the saved pc of the frame above. Once the
   frame pc is found it must be mapped to a bytecode pc. */
...
static sqInt instructionPointerForFramecurrentFPcurrentIP(
  ...
  if (spouseFP == currentFP) {
    ...
    thePage = stackPageAtpages(pageIndexForstackMemorybytesPerPage(
                                   spouseFP, stackMemory, bytesPerPage),
                               pages);
    callerFP = ((char *)0);
    fp = (thePage->headFP);
    if (fp == spouseFP) {
  ...
  l1:
    theIP = (theFPAbove ? longAt(theFPAbove + FoxCallerSavedIP)
  ...
  return (
      ((usqInt)(((theIP - (longAt(spouseFP + FoxMethod))) - BaseHeaderSize) + 2)
       << 3) |
      1);
}