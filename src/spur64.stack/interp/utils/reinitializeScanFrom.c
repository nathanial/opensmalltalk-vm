...
/*	Search for firstFreeObject and firstMobileObject from initialObject,
   which is the hiddenRootsObject on the first pass, and the
   objectAfterLastMobileObject on subsequent passes). */
...
static void reinitializeScanFrom(sqInt initialObject) {
  ...
  if (objectAfterLastMobileObject) {
    ...
    while (1) {
      ...
      if (!(oopisLessThanOrEqualTo(objOop, objectAfterLastMobileObject)))
        break;
