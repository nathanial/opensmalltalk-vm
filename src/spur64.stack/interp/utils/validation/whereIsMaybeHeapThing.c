...
static char *whereIsMaybeHeapThing(sqInt anOop) {
  if (/* isInNewSpace: */
    ...
    if (oopisGreaterThanOrEqualToandLessThan(anOop, ((eden).start),
                                             freeStart)) {
      return " is in eden";
    }
    if (oopisGreaterThanOrEqualToandLessThan(anOop, ((futureSpace).start),
                                             futureSurvivorStart)) {
      return " is in future space";
    }
    if (oopisGreaterThanOrEqualToandLessThan(anOop, ((pastSpace).start),
                                             pastSpaceStart)) {
