...
sqInt checkOkayOop(usqInt oop) {
  ...
  if (!(addressCouldBeObj(oop))) {
    fprintf(transcript, "oop %p is not a valid address", ((void *)oop));
    return 0;
  }
  ...
  /* header type checks */
  if (!(((classIndex = (longAt((void *)(oop))) & (classIndexMask()))) >=
        (firstClassIndexPun()))) {
    fprintf(transcript, "oop %p is a free chunk, or bridge, not an object",
            ((void *)oop));
    return 0;
  }
  if (((byteAt((void *)(oop + (numSlotsFieldByteOffset())))) ==
       (numSlotsMask())) &&
      ((byteAt((void *)((oop - BaseHeaderSize) +
                        (numSlotsFieldByteOffset())))) != (numSlotsMask()))) {
    fprintf(transcript,
            "oop %p header has overflow header word, but overflow word does "
            "not have a saturated numSlots field",
            ((void *)oop));
    return 0;
  }
  ...
  if ((fmt == 6) || (fmt == 8)) {
    fprintf(transcript, "oop %p has an unknown format type", ((void *)oop));
    return 0;
  }
  if ((fmt == (forwardedFormat())) !=
      (classIndex == (isForwardedObjectClassIndexPun()))) {
    fprintf(transcript,
            "oop %p has mis-matched format/classIndex fields; only one of them "
            "is the isForwarded value",
            ((void *)oop));
    return 0;
  }
  ...
  unusedBitsInYoungObjects =
      (1U << (unusedBitShift())) |
      ((1U << (pinnedBitShift())) | (1U << (rememberedBitShift())));
  if ((longAt((void *)(oop))) & unusedBitsInYoungObjects) {
    fprintf(transcript,
            "oop %p has some header bits unused in young objects set; should "
            "be zero",
            ((void *)oop));
    return 0;
  }
