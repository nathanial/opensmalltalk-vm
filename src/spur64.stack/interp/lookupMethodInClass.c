	...
	while (currentClass != nilObj) {
		...
		if (mask <= methodDictLinearSearchLimit) {
			...
			while (index <= mask) {
				...
				if (nextSelector == messageSelector) {
					...
					objOop = longAt((void *)((methodArray + BaseHeaderSize) + ((((usqInt)(index) << (shiftForWord()))))));
					if (/* isOopForwarded: */
						((!(objOop & (tagMask()))))
		...
		while (1) {
			...
			if (nextSelector == messageSelector) {
				...
				/* begin followField:ofObject: */
				objOop = longAt((void *)((methodArray + BaseHeaderSize) + ((((usqInt)((index - SelectorStart)) << (shiftForWord()))))));
				if (/* isOopForwarded: */
					((!(objOop & (tagMask()))))
				 && ((!((longAt((void *)(objOop))) & ((classIndexMask()) - (isForwardedObjectClassIndexPun())))))) {
