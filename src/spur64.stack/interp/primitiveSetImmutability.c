...
static void primitiveSetImmutability(void) {
  ...
  if ((longAt(stackPointer)) == trueObj) {
    ...
    /* For now we fail the primitive for contexts to we ensure there are no
       immutable contexts. Later we can consider having immutable contexts and
       send cannotReturn callback when returning to an immutable context. That
       would mean that setting a context to immutable would require a divorce
       and returns to immutable context are necessarily across stack pages */
    ...
    /* Simple version of process management: we forbid Process and LinkedList
    ...
    /* The rest of the code is relative to process management: the Processor
       (the active process scheduler) can't be immutable, as well as all the
       objects relative to Process management scheduler := self fetchPointer:
       ValueIndex ofObject: (self splObj: SchedulerAssociation). processLists :=
       objectMemory fetchPointer: ProcessListsIndex ofObject: scheduler.
  ...
  l2:
    ...
    longAtput((void *)(rcvr),
              (longAt((void *)(rcvr))) | (1U << (immutableBitShift())));
  } else {
