...
static NoDbgRegParms sqInt okayOop(sqInt signedOop) {
  ...
  if (((byteAt((void *)(oop + (numSlotsFieldByteOffset())))) ==
    ...
    error("oop header has overflow header word, but overflow word does not "
          "have a saturated numSlots field");
    return 0;
  }
  ...
  if ((fmt == (forwardedFormat())) !=
    ...
    error("oop has mis-matched format/classIndex fields; only one of them is "
          "the isForwarded value");
    return 0;
  }
  ...
  unusedBitsInYoungObjects =
      ((1ULL << (greyBitShift())) | (1U << (pinnedBitShift()))) |
      (1U << (rememberedBitShift()));
