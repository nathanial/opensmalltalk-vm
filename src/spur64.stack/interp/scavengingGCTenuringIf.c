...
{   DECL_MAYBE_SQ_GLOBAL_STRUCT
	...
	if (!(asserta(((((eden).limit)) - freeStart) > (interpreterAllocationReserveBytes())))) {
		/* begin tab */
		printChar('\t');
	...
	for (i = 1; i <= MethodCacheEntries; i += 1) {
		if (methodCache[probe + MethodCacheSelector]) {
			if (((oopisGreaterThanOrEqualTo(methodCache[probe + MethodCacheSelector], newSpaceStart))
			 && (oopisLessThan(methodCache[probe + MethodCacheSelector], oldSpaceStart)))
			 || ((oopisGreaterThanOrEqualTo(methodCache[probe + MethodCacheMethod], newSpaceStart))
			 && (oopisLessThan(methodCache[probe + MethodCacheMethod], oldSpaceStart)))) {
