...
addToEphemeronList(sqInt ephemeronCorpse) {
  ...
  assert(
      !(isScavengeSurvivor(keyOfEphemeron(followForwarded(ephemeronCorpse)))));
  ephemeronListOffset = (!(ephemeronList) ? 0 : ephemeronList);

  /* begin setCorpseOffsetOf:to: */
  assert(isYoung(ephemeronCorpse));
  assert(isForwarded(ephemeronCorpse));
  long64Atput(
      (void *)(ephemeronCorpse),
      headerForSlotshashformatclassIndex(
          byteAt((void *)(ephemeronCorpse + (numSlotsFieldByteOffset()))),
          ((usqInt)(ephemeronListOffset)) >> (formatFieldWidthShift()),
          ephemeronListOffset & (formatMask()),
          isForwardedObjectClassIndexPun()));
  ephemeronList = (((usqInt)((ephemeronCorpse - newSpaceStart))) >>
