...
l1:
	...
	theIP = longAt((void *)((aContext + BaseHeaderSize) + ((((usqInt)(InstructionPointerIndex) << (shiftForWord()))))));

	/* begin followObjField:ofObject: */
	theMethod = longAt((void *)((aContext + BaseHeaderSize) + ((((usqInt)(MethodIndex) << (shiftForWord()))))));
	assert(isNonImmediate(theMethod));
	if ((!((longAt((void *)(theMethod))) & ((classIndexMask()) - (isForwardedObjectClassIndexPun()))))) {
		theMethod = fixFollowedFieldofObjectwithInitialValue(MethodIndex, aContext, theMethod);
	}
	if (!((((theIP) & 7) == 1))) {
	...
	rcvr = longAt((void *)((aContext + BaseHeaderSize) + ((((usqInt)(ReceiverIndex) << (shiftForWord()))))));
	if (/* isOopForwarded: */
		((!(rcvr & (tagMask()))))
	 && ((!((longAt((void *)(rcvr))) & ((classIndexMask()) - (isForwardedObjectClassIndexPun())))))) {
		rcvr = fixFollowedFieldofObjectwithInitialValue(ReceiverIndex, aContext, rcvr);
	}
	...
	if (maybeClosure != nilObj) {
		...
		oop = longAt((void *)((maybeClosure + BaseHeaderSize) + ((((usqInt)(ClosureNumArgsIndex) << (shiftForWord()))))));
		assert((((oop) & 7) == 1));
		numArgs = (oop >> 3);
		...
		header = longAt((void *)((theMethod + BaseHeaderSize) + ((((usqInt)(HeaderIndex) << (shiftForWord()))))));
...
/* literalCountOfAlternateHeader: */
	...
	for (i = 1; i <= numArgs; i += 1) {
		longAtput((pointer -= BytesPerWord),longAt((void *)((aContext + BaseHeaderSize) + ((((usqInt)((ReceiverIndex + i)) << (shiftForWord())))))));
	...
	/* begin quickFetchInteger:ofObject: */
	oop = longAt((void *)((aContext + BaseHeaderSize) + ((((usqInt)(StackPointerIndex) << (shiftForWord()))))));
	assert((((oop) & 7) == 1));
	stackPtrIndex = (oop >> 3);
	...
	longAtput((void *)((aContext + BaseHeaderSize) + ((((usqInt)(SenderIndex) << (shiftForWord()))))),valuePointer);
	/* begin withSmallIntegerTags: */
	...
	assert((isNonImmediate(aContext))
	 && (!(isForwarded(aContext))));
	assert(validStorePointerUncheckedArgs(InstructionPointerIndex, aContext, valuePointer));
	longAtput((void *)((aContext + BaseHeaderSize) + ((((usqInt)(InstructionPointerIndex) << (shiftForWord()))))),valuePointer);
