...
/*	This primitive will load a binary image segment created by
        ...
        been reduced to an unrecognizable and unusable jumble. But what more
   could you have done with it anyway?

        The primitive, if it succeeds, also becomes the segmentWordArray into
   the array of loaded objects. This allows fixing up of loaded objects
   directly, without nextObject, which Spur doesn't support. */
...
loadImageSegmentFromoutPointers(sqInt segmentWordArray, sqInt outPointerArray) {
  ...
  /* version info
     one object header
     Verify format.  If the format is wrong, word-swap (since ImageSegment data
     are 32-bit longs). If it is still wrong, undo the damage and fail. */
  segVersion = longAt((void *)(segmentWordArray + BaseHeaderSize));
  if (!(((((segVersion & 0xFFFFFF) | MultipleBytecodeSetsBitmask) -
  ...
  segmentLimit =
      ((segmentLimit * BytesPerOop) + segmentWordArray) + BaseHeaderSize;

  /* Notionally reverse the Byte type objects if the data is from opposite
     endian machine. Test top byte.  $d on the Mac or $s on the PC.  Rest of
     word is equal.  If Spur is ever ported to big-endian machines then the
     segment may have to be byte/word swapped, but so far it only runs on
     little-endian machines, so for now just fail if endianness is wrong. */
  if (((((usqInt)(segVersion)) >> 24) & 0xFF) !=
      ((((usqInt)((imageSegmentVersion()))) >> 24) & 0xFF)) {
    return PrimErrBadArgument;
  }

  /* Reverse the byte-type objects once
     Avoid having to remember by arranging that there are no young outPointers
     if segment is in old space. */
  ...
  /* scan through mapping oops and validating class references. Defer entering
     any class objects into the class table and/or pinning objects until a
     second pass. */
  ...
  objOop = (numSlots == (numSlotsMask()) ? segmentStart + BaseHeaderSize
                                         : segmentStart);
  while (oopisLessThan(objOop, segmentLimit)) {
    ...
    /* validate the class ref, but don't update it until any internal classes
     * have been added to the class table. */
    if (((classIndex & TopHashBit) != 0)) {
      ...
      if (((oopUsqInt = ((classIndex - (firstClassIndexPun())) *
                         8 /* allocationUnit */) +
...
l8:
  ...
  loadedObjectsArray = noInlineAllocateSlotsformatclassIndex(
      numLoadedObjects, arrayFormat(), ClassArrayCompactIndex);
  if (!loadedObjectsArray) {
    growOldSpaceByAtLeast((BaseHeaderSize + BaseHeaderSize) +
  ...
  /* Scan for classes contained in the segment, entering them into the class
     table. Classes are at the front, after the root array and have the
     remembered bit set. */
  ...
  while ((oopisLessThan(objOop, segmentLimit)) &&
    ...
    objOop = ((((usqInt)(followingWordSqInt)) >> (numSlotsFullShift())) ==
                      (numSlotsMask())
...
l3:
   ...
   * and collecting the loaded objects in loadedObjectsArray */
  ...
  while (oopisLessThan(objOop, segmentLimit)) {
    ...
    longAtput((void *)((loadedObjectsArray + BaseHeaderSize) +
                       ((((usqInt)(fillIdx) << (shiftForWord()))))),
              objOop);
    fillIdx += 1;
...
l5:
  ...
  if ((byteAt((void *)(segmentWordArray + (numSlotsFieldByteOffset())))) ==
    ...
    if (oopisLessThan(segmentWordArray, oldSpaceStart)) {
      ...
      longAtput((void *)(segmentWordArray - BaseHeaderSize),
                ((((usqInt)((numSlotsMask())) << 56))));
      ...
      /* begin rawOverflowSlotsOf:put: */
      longAtput((void *)(segmentWordArray - BaseHeaderSize),
                ((((usqInt)((numSlotsMask())) << 56))) + numSlotsSqInt);
    }
  } else {
    /* rawNumSlotsOf:put: */
    byteAtput((void *)(segmentWordArray + (numSlotsFieldByteOffset())),
              8 /* allocationUnit */ / BytesPerOop);
  }

  /* N.B. setting the overflow slots to 1 creates a slimbridge in eden, so we
     also need to delete the overflow slot count in the segment itself Finally
     forward the segmentWordArray to the loadedObjectsArray */
  ...
  longAtput((void *)(segmentWordArray),
            ((longAt((void *)(segmentWordArray))) &
             (~(usqIntptr_t)(((((usqInt)((formatMask())) << (formatShift())))) +
                             (classIndexMask())))) +
                ((isForwardedObjectClassIndexPun()) +
                 ((((usqInt)((forwardedFormat())) << (formatShift()))))));
  ...
  if (oopisGreaterThanOrEqualTo(segmentWordArray, oldSpaceStart)) {
    if (/* isYoung: */
      ...
      /* begin possibleRootStoreInto: */
      if (!((byteAt((void *)(segmentWordArray + (formatFieldByteOffset())))) &
            (1U << (rememberedBitByteShift())))) {
  ...
  /* most stores into young objects */
  longAtput(
      (void *)((segmentWordArray + BaseHeaderSize) + (0U << (shiftForWord()))),
      loadedObjectsArray);
  ...
  /* Answer the first object in the segment because this is what
     primitiveLoadImageSegment tests for, even though the real result is the
     segmentWordArray forwarded to the loadedObjectsArray. */

  /* begin objectStartingAt: */
  numSlots = byteAt((void *)(segmentStart + (numSlotsFieldByteOffset())));
  return (numSlots == (numSlotsMask()) ? segmentStart + BaseHeaderSize
                                       : segmentStart);
}