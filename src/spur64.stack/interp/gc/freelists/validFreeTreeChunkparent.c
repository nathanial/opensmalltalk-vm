...
static const char *validFreeTreeChunkparent(sqInt chunk, sqInt parent) {
  ...
  if (parent != (fetchPointerofObject(2U, chunk))) {
    return "bad parent";
  }
  ...
  if ((reason =
           validFreeTreeChunkparent(fetchPointerofObject(3U, chunk), chunk))) {
    return reason;
  }
  if ((reason =
           validFreeTreeChunkparent(fetchPointerofObject(4U, chunk), chunk))) {
    return reason;
  }
  return null;
}