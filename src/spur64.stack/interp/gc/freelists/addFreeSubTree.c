...
static void addFreeSubTree(sqInt freeTree) {
  ...
  while (1) {
    ...
    assert((oopisLessThanOrEqualTo((freeTree + bytesInArg) - BaseHeaderSize,
                                   treeNode)) ||
    ...
    if (bytesInNode > bytesInArg) {
      ...
      if (!subNode) {
        ...
        assert((freeTree == 0) || (isFreeObject(freeTree)));
        longAtput(
            (void *)((treeNode + BaseHeaderSize) + (3U << (shiftForWord()))),
            freeTree);
        ...
        /* begin storePointer:ofFreeChunk:withValue: */
        assert(isFreeObject(freeTree));
        assert((treeNode == 0) || (isFreeObject(treeNode)));
        longAtput(
            (void *)((freeTree + BaseHeaderSize) + (2U << (shiftForWord()))),
            treeNode);
        return;
      }
    } else {
