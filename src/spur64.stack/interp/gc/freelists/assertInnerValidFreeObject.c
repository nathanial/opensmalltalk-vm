...
/*	we don't want to inline so we can nest that in an assertion with the
        return true so the production VM does not generate any code here, while
   in simulation, the code breaks on the assertion we want to.
 */
  ...
  if (!(chunkBytes == (BaseHeaderSize + 8 /* allocationUnit */))) {
    ...
    if ((index < 64 /* numFreeLists */) && ((1ULL << index) <= freeListsMask)) {
      ...
      do {
        ...
        /* apply if the node has no children, or it has no large children and
           we're returning from the small child, or we're returning from the
           large child. */
