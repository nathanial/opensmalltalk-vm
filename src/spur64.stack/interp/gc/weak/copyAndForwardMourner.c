...
static sqInt copyAndForwardMourner(sqInt mourner) {
  ...
  assert((isInEden(mourner)) || (isInPastSpace(mourner)));

  /* cog methods should be excluded. */
  bytesInObj = bytesInBody(mourner);
  format =
      (byteAt((void *)(mourner + (formatFieldByteOffset())))) & (formatMask());

  /* Allow Slang to inline. */
  tenure = !((tenureCriterion == TenureByAge) && (tenureThreshold == 0));
  if (tenure || ((futureSurvivorStart + bytesInObj) > ((futureSpace.limit)))) {
    ...
    startOfSurvivor = /* startOfObject: */
        ((byteAt((void *)(mourner + (numSlotsFieldByteOffset())))) ==
                 (numSlotsMask())
  ...
  assert((isNonImmediate(mourner)) && (!(isForwarded(mourner))));
  assert(validStorePointerUncheckedArgs(0, mourner, newLocation));
  longAtput((void *)((mourner + BaseHeaderSize) + (0U << (shiftForWord()))),
            newLocation);
