...
static sqInt allocateOldSpaceChunkOfBytes(usqInt chunkBytes) {
    ...
    if (((freeListsMask & (1ULL << initialIndex)) != 0)) {
      if ((chunk = freeLists[initialIndex])) {
        ...
        if ((!lilliputian) && (next != 0)) {
          ...
          longAtput(
              (void *)((next + BaseHeaderSize) + (1U << (shiftForWord()))), 0);
        }
        return chunk;
      }
      ...
      if (((freeListsMask & (1ULL << index)) != 0)) {
        if ((chunk = freeLists[index])) {
          ...
          if (next) {
            ...
            assert(isFreeObject(next));
            longAtput(
                (void *)((next + BaseHeaderSize) + (1U << (shiftForWord()))),
                0);
          }
          assert((bytesInBody(chunk)) == (index * (allocationUnit())));
          freeChunkWithBytesat(
              (index * 8 /* allocationUnit */) - chunkBytes,
              (/* startOfObject: */
  ...
  if (chunk) {
    ...
    assert((nextFreeChunk == 0) || (isFreeObject(nextFreeChunk)));
    longAtput((void *)((parent + BaseHeaderSize) + (0U << (shiftForWord()))),
              nextFreeChunk);
    if (nextFreeChunk) {
      /* begin storePointer:ofFreeChunk:withValue: */
      assert(isFreeObject(nextFreeChunk));
