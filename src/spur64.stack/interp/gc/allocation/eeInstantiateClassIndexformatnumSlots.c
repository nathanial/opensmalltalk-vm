/* Extracted from interp.c:34514 (function
 * eeInstantiateClassIndexformatnumSlots). */

/*	Instantiate an instance of a compact class. ee stands for execution
   engine and implies that this allocation will *NOT* cause a GC. N.B. the
        instantiated object
...
usqInt eeInstantiateClassIndexformatnumSlots(sqInt knownClassIndex,
                                             sqInt objFormat, sqInt numSlots) {
  usqInt newObj;
  usqInt numBytes;

  assert((numSlots >= 0) && ((knownClassIndex != 0) &&
                             ((knownClassAtIndex(knownClassIndex)) != nilObj)));
  assert(((objFormat < (firstByteFormat()) ? objFormat
                                           : objFormat & (byteFormatMask()))) ==
         (instSpecOfClass(knownClassAtIndex(knownClassIndex))));
