...
NeverInline void markAndTrace(sqInt objOop) {
  sqInt contextSize;
  sqInt field;
  sqInt fmt;
  sqInt format;
  ...
  /* avoid pushing non-pointer objects on the markStack.
     Avoid tracing classes of non-objects on the heap, e.g. IRC caches, Sista
     counters. */
  if (format == (weakArrayFormat())) {
    objStack = weaklingStack;
    ...
    if (((objOop & (tagMask())) != 0)) {
      ...
      assert(!((objStack == markStack) && (isWeakNonImm(objOop))));

      /* There should only be weaklings on the weaklingStack */
      assert((objStack != weaklingStack) || (isWeakNonImm(objOop)));
    }
  ...
  if ((format == (ephemeronFormat())) && (activeAndDeferredScan(objOop))) {
    return;
  }
  ...
  /* To avoid overflowing the mark stack when we encounter large objects, we
     push the obj, then its numStrongSlots, and then index the object from the
     stack. */
  do {
      ...
      while (index > 0) {
        ...
        if ((!(field & (tagMask())))) {
          if ((!((longAt((void *)(field))) &
            ...
            field = fixFollowedFieldofObjectwithInitialValue(index, objToScan,
                                                             field);
          }
          ...
          /* avoid pushing non-pointer objects on the markStack.
             Avoid tracing classes of non-objects on the heap, e.g. IRC caches,
             Sista counters. */
          if (format == (weakArrayFormat())) {
          ...
          if (index > 0) {
            if ((topOfObjStack(markStack)) != objToScan) {
              ...
              if (((objToScan & (tagMask())) != 0)) {
                ...
                assert(!((objStack == markStack) && (isWeakNonImm(objToScan))));
      ...
      while (index > 0) {
        ...
        if ((!(field & (tagMask())))) {
          ...
          byteAtput((void *)(field + (markBitsByteOffset())),
                    (byteAt((void *)(field + (markBitsByteOffset())))) |
                        (1U << (markedBitByteShift())));
          formatSqInt = (byteAt((void *)(field + (formatFieldByteOffset())))) &
                        (formatMask());
          if (/* isPureBitsFormat: */
          ...
          if (((byteAt((void *)(field + (numSlotsFieldByteOffset())))) >
            ...
            if (((objOop1 & (tagMask())) != 0)) {
              ...
              assert(!((objStack1 == markStack) && (isWeakNonImm(objOop1))));

              /* There should only be weaklings on the weaklingStack */
              assert((objStack1 != weaklingStack) || (isWeakNonImm(objOop1)));
            }
