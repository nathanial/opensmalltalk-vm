...
NeverInline usqLong fullGC(void) {
  sqInt address;
  ...
  /* Mark objects /before/ scavenging, to empty the rememberedTable of unmarked
   * roots. */
  markObjects(1);
  ...
  /* Mid-way the leak check must be more lenient.  Unmarked classes will have
     been expunged from the table, but unmarked instances will not yet have been
     reclaimed. */
  runLeakCheckerForexcludeUnmarkedObjsclassIndicesShouldBeValid(GCModeFull, 1,
                                                                1);
  ...
  /* begin bridgeAt: */
  /* begin bridgeFor: */
  address = (((((&(segments[numSegments - 1])))->segSize)) +
             ((((&(segments[numSegments - 1])))->segStart))) -
            (2 * BaseHeaderSize);
  numSlots = byteAt((void *)(address + (numSlotsFieldByteOffset())));
  objOop = (numSlots == (numSlotsMask()) ? address + BaseHeaderSize : address);
  ...
  if ((totalFreeOldSpace > shrinkThreshold) &&
      ((totalFreeOldSpace > growHeadroom) &&
       (shrinkObjectMemory((((totalFreeOldSpace - growHeadroom) < growHeadroom)
                                ? growHeadroom
                                : (totalFreeOldSpace - growHeadroom)))))) {
    statShrinkMemory += 1;
  }
  ...
  return (freeLists[0] ? bytesInBody(findLargestFreeChunk()) : 0);
}