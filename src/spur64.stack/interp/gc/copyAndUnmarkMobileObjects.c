...
/*	Sweep the mobile portion of the heap, moving objects to their eventual
   ...
   have been forwarded. Answer if the end of the heap was reached
   (savedFirstFieldsSpace has not overflowed). The enumerations in
   planCompactSavingForwarders, updatePointersInMobileObjects and
   copyAndUnmarkMobileObjects match. We could implement them as a single
   enumeration method taking several block arguments, but arguably that would
   make understanding an already tricky algorithm more difficult. Instead we
   tolerate the duplication and encourage the reader to diff the three methods
   to see where they diverge (e.g. via Cmd-shift-C).  */
...
static NeverInline sqInt copyAndUnmarkMobileObjects(void) {
  ...
  finalObject = (!(lastMobileObject) ? nilObj : lastMobileObject);
  ...
  limit = (oopisLessThan(finalObject, endOfMemory) ? addressAfter(finalObject)
                                                   : endOfMemory);
  while (1) {
    assert((objOop % (allocationUnit())) == 0);
    if (!(oopisLessThan(objOop, limit)))
      break;
    ...
    nextObj = ((((usqInt)(followingWord)) >> (numSlotsFullShift())) ==
                       (numSlotsMask())
    ...
    if ((byteAt((void *)(objOop + (markBitsByteOffset())))) &
      ...
      if ((byteAt((void *)(objOop + (formatFieldByteOffset())))) &
        ...
        while ((toFinger <= startOfPreviousPin) &&
          ...
          do {
            assert((isMarked(previousPin)) && (isPinned(previousPin)));

            /* begin unmarkPinned: */
            if (((longAt((void *)(previousPin))) & (classIndexMask())) ==
                (segmentBridgePun())) {
              ...
              byteAtput(
                  (void *)(previousPin + (markBitsByteOffset())),
        ...
        destObj = (numSlots == (numSlotsMask()) ? toFinger + BaseHeaderSize
                                                : toFinger);
        start = /* startOfObject:given: */
            (numSlots == (numSlotsMask()) ? objOop - BaseHeaderSize : objOop);

        /* memmove must be used since the ranges may overlap. */
        memmove(((void *)toFinger), ((void *)start), bytes);

        /* begin storePointerUnchecked:ofObject:withValue: */
        assert((isNonImmediate(destObj)) && (!(isForwarded(destObj))));
        assert(validStorePointerUncheckedArgs(0, destObj, firstField));
        longAtput(
            (void *)((destObj + BaseHeaderSize) + (0U << (shiftForWord()))),
            firstField);
        toFinger += bytes;
        if (((top += BytesPerOop)) >= ((savedFirstFieldsSpace.limit))) {
          ...
          if (!previousPin) {
            previousPin = nextObj;
            startOfPreviousPin = /* startOfObject: */
                ((byteAt((void *)(nextObj + (numSlotsFieldByteOffset())))) ==
                         (numSlotsMask())
                     ? nextObj - BaseHeaderSize
                     : nextObj);
          }

          /* Create a free object for firstFreeObject to be set to on the next
             pass, but do not link it into the free tree as it will be written
             over in that next pass. */
          if (toFinger < startOfPreviousPin) {
            firstFreeObject = initFreeChunkWithBytesat(
                startOfPreviousPin - toFinger, toFinger);
          } else {
  ...
  freeFromupTonextObject(toFinger, endOfMemory,
                         (!(previousPin) ? (!(objectAfterLastMobileObject)
  ...
  /* begin objectStartingAt: */
  numSlots = byteAt((void *)(toFinger + (numSlotsFieldByteOffset())));
  obj = (numSlots == (numSlotsMask()) ? toFinger + BaseHeaderSize : toFinger);
  while (1) {
    next = oldSpaceObjectAfter(obj);
    if (!(next < endOfMemory))
      break;
    if ((((longAt((void *)(obj))) & (classIndexMask())) ==
      ...
      obj = freeChunkWithBytesat(
          objBytes + nextBytes, /* startOfObject: */
