...
static NoDbgRegParms SpurSegmentInfo *addSegmentOfSize(sqInt ammount) {
  ...
  newSegIndex = 0;
  allocatedSize = 0;
  if ((segAddress = sqAllocateMemorySegmentOfSizeAboveAllocatedSizeInto(
           ammount, firstGapOfSizeAtLeast(ammount), (&allocatedSize)))) {
    ...
    numSegments += 1;
    while (1) {
      if (segAddressSqInt >= ((((segments[segIndex]).segSize)) +
  ...
  /* sent to the manager so that the simulator can increase memory to simulate a
   * new segment */
  return null;
}