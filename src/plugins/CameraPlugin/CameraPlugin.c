/* Automatically generated by
   ...
   1a32964f-d425-4ed5-b830-6e8ce3d98688
        (* Compiler-eem.518)
   from
        CameraPlugin VMMaker.oscog-eem.3599 uuid:
   1a32964f-d425-4ed5-b830-6e8ce3d98688
 */
...
EXPORT(sqInt) primGetLatestBufferIndex(void);
...
EXPORT(sqInt) primSetParam(void);
...
static const char *moduleName = "CameraPlugin VMMaker.oscog-eem.3599 " INT_EXT;
...
EXPORT(sqInt)
initialiseModule(void) { return cameraInit(); }

/*	Acknowledge the receipt of one or more buffered frames, installed via
        primSetCameraBuffers. Answer the number of times a buffer was filled
   since the last invocation. This is a version of primGetFrame that does not
   need to supply a buffer to receiver the result. */
...
/*	Answer the frame extent of the given camera, or zero if the camera is
   not open. The extent is 16 bits of width and height packed into a single
        integer.
 */
...
primGetCameraSemaphore(void) {
  ...
  cameraNum = stackIntegerValue(0);
  if (!(failed())) {
    if ((index = CameraGetSemaphore(cameraNum))) {
...
/*	Copy a camera frame into the given Bitmap. The Bitmap should be for a
   Form of depth 32 that is the same width and height as the current camera
   frame. Fail if the camera is not open or if the bitmap is not the right size.
   If successful, answer the number of frames received from the camera since the
...
primGetFrame(void) {
  ...
  bitmap = firstIndexableField(bitmapOop);
...
/*	If double-buffering is in effect (set via primSetCameraBuffers) answer
        which buffer contains the freshest data, either A (1) or B (2). If no
        buffer has been filled yet, answer nil. Otherwise fail with an
   appropriate error code. */

/* CameraPlugin>>#primGetLatestBufferIndex */
EXPORT(sqInt)
primGetLatestBufferIndex(void) {
  sqInt cameraNum;
  sqInt result;
  ...
  if ((result = CameraGetLatestBufferIndex(integerValueOf(cameraNum)))) {
    if (result < 0) {
...
/*	Answer the requested parameter value of the given camera.
        See platforms/Cross/plugins/CameraPlugin/CameraPlugin.h for the list of
        parameter names. */
...
/*	Open a camera. Takes one argument, the index of the device to open.
        ...
        desiredFrameWidth and desiredFrameHeight are zero, and the closest to
   the non-zero value(s) if either or both of desiredFrameWidth and
   desiredFrameHeight are non-zero. */
...
/*	Sets one or two frame buffers to receive camera frames. The buffer(s)
   must be big enough and pinned. Provides these two interfaces: camera:
   cameraNum setFrameBuffer: pinnedNonPointers camera: cameraNum
   setFrameBufferA: pinnedNonPointers B: pinnedNonPointersOrNil
 */
  ...
  if (!((isIntegerObject(cameraNum)) &&
        ((isWordsOrBytes(bufferOne)) &&
         ((isPinned(bufferOne)) &&
          ((!bufferTwo) ||
           ((bufferTwo == (nilObject())) ||
            ((isWordsOrBytes(bufferTwo)) && (isPinned(bufferTwo))))))))) {
    return primitiveFailFor(PrimErrBadArgument);
  }

  /* coerce nilObject to nil */
  failCode =
      CameraSetFrameBuffers(integerValueOf(cameraNum), bufferOne,
                            (!(bufferTwo == (nilObject())) ? bufferTwo : 0));
  if (failCode) {
...
/*	Supplies a semaphoreIndex through which to signal a semaphore when a
   frame is available from a camera. Fails if the platform does not provide an
   interrupt-driven camera interface.
 */
...
/*	Set the requested parameter value of the given camera, and answer its
        previous value.
        See platforms/Cross/plugins/CameraPlugin/CameraPlugin.h for the list of
        parameter names.
 */

/* CameraPlugin>>#primSetParam */
EXPORT(sqInt)
primSetParam(void) {
  sqInt cameraNum;
  sqInt paramNum;
  ...
  cameraNum = stackIntegerValue(2);
  paramNum = stackIntegerValue(1);
  paramValue = stackValue(0);
  paramValue = (isIntegerObject(paramValue) ? integerValueOf(paramValue)
                                            : booleanValueOf(paramValue));
  if (failed()) {
    primitiveFailFor(PrimErrBadArgument);
  } else {
    result = CameraSetParam(cameraNum, paramNum, paramValue);
    if (result < 0) {
      primitiveFailFor(-result);
    } else {
      if (paramNum == MirrorImage) {
...
EXPORT(sqInt)
shutdownModule(void) { return cameraShutdown(); }
...
void *CameraPlugin_exports[][3] = {
    ...
    {(void *)_m, "primAcknowledgeFrame\000\000\000",
     (void *)primAcknowledgeFrame},
    {(void *)_m, "primCameraName\000\000\000", (void *)primCameraName},
    {(void *)_m, "primCameraUID\000\000\000", (void *)primCameraUID},
    {(void *)_m, "primCloseCamera\000\000\000", (void *)primCloseCamera},
    {(void *)_m, "primFrameExtent\000\000\000", (void *)primFrameExtent},
    {(void *)_m, "primGetCameraSemaphore\000\000\000",
     (void *)primGetCameraSemaphore},
    {(void *)_m, "primGetFrame\000\001\000", (void *)primGetFrame},
    {(void *)_m, "primGetLatestBufferIndex\000\000\000",
     (void *)primGetLatestBufferIndex},
    {(void *)_m, "primGetParam\000\000\000", (void *)primGetParam},
    {(void *)_m, "primOpenCamera\000\000\000", (void *)primOpenCamera},
    {(void *)_m, "primSetCameraBuffers\000\000\000",
     (void *)primSetCameraBuffers},
    {(void *)_m, "primSetCameraSemaphore\000\000\000",
     (void *)primSetCameraSemaphore},
    {(void *)_m, "primSetParam\000\000\000", (void *)primSetParam},
...
#if SPURVM
EXPORT(signed short) primAcknowledgeFrameMetadata = 0;
EXPORT(signed short) primCameraNameMetadata = 0;
EXPORT(signed short) primCameraUIDMetadata = 0;
EXPORT(signed short) primCloseCameraMetadata = 0;
EXPORT(signed short) primFrameExtentMetadata = 0;
EXPORT(signed short) primGetCameraSemaphoreMetadata = 0;
EXPORT(signed short) primGetFrameMetadata = 0x100;
EXPORT(signed short) primGetLatestBufferIndexMetadata = 0;
...
EXPORT(signed short) primSetParamMetadata = 0;
#endif // SPURVM
