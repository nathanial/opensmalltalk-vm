/* Automatically generated by
        SmartSyntaxPluginCodeGenerator VMMaker.oscog-eem.3601 uuid:
   6617a9c2-d1a4-4065-87c1-65631276475a
        (* Compiler-eem.518)
   from
        VectorEnginePlugin VectorEnginePlugin-eem.22 uuid:
   b78f39f5-368d-4cba-99a5-399c4ffe554c
 */
#if defined(__clang__) || defined(__GNUC__)
static const char __buildInfo[] __attribute__((unused)) =
    "VectorEnginePlugin VectorEnginePlugin-eem.22 uuid: "
    "b78f39f5-368d-4cba-99a5-399c4ffe554c " __DATE__;
#else
...
/*** Function Prototypes ***/
static sqInt
blendFillOnlyAtredIsInsidegreenIsInsideblueIsInsideantiAliasAlphasWord(
    sqInt pixelIndex, sqInt isRedInside, sqInt isGreenInside,
    sqInt isBlueInside, uint32_t strokeAntiAliasAlphasWord);
static sqInt blendFillOnlyWPAtantiAliasAlphaByte(sqInt pixelIndex,
                                                 uint8_t antiAliasAlphaBits);
static sqInt
blendStrokeOnlyAtantiAliasAlphasWord(sqInt pixelIndex,
                                     uint32_t strokeAntiAliasAlphasWord);
static sqInt
blendStrokeOnlyWPAtantiAliasAlphaByte(sqInt pixelIndex,
...
static sqInt pvt_cubicBezierFromXytoXycontrol1Xycontrol2Xy(
    float xFrom, float yFrom, float xTo, float yTo, float xControl1,
    float yControl1, float xControl2, float yControl2);
static sqInt pvt_cubicBezierWPFromXytoXycontrol1Xycontrol2Xy(
    float xFrom, float yFrom, float xTo, float yTo, float xControl1,
    float yControl1, float xControl2, float yControl2);
static sqInt pvt_lineFromXytoXy(float xFrom, float yFrom, float xTo, float yTo);
static sqInt pvt_lineWPFromXytoXy(float xFrom, float yFrom, float xTo,
                                  float yTo);
static sqInt pvt_quadraticBezierFromXytoXycontrolXy(float xFrom, float yFrom,
                                                    float xTo, float yTo,
                                                    float xControl,
                                                    float yControl);
static sqInt pvt_quadraticBezierWPFromXytoXycontrolXy(float xFrom, float yFrom,
                                                      float xTo, float yTo,
                                                      float xControl,
                                                      float yControl);
...
static const char *moduleName =
    "VectorEnginePlugin VectorEnginePlugin-eem.22 " INT_EXT;
...
/*	Blends fill color over background.
        Sub pixel anti aliasing means each R, G, B subpixel might use a
   different anti aliasing alpha. Target translucency is computed correctly. For
   blending, alphas are in [0 .. 1.0] and R, G, B color components are in [0 ..
   255]
 */
...
blendFillOnlyAtredIsInsidegreenIsInsideblueIsInsideantiAliasAlphasWord(
   ...
   * This means that in the inside of the shape, and away from the stroke, they
   * must be 1.0 (not 0.0). */
  ...
  /* These if are not really needed. just ignore them if we use simd
   * instructions. */
  if (!(alphaR == 0.0)) {
    unAlphaR = 1.0 - alphaR;
    resultAlphaR = alphaR + (unAlphaR * targetAlpha);
    resultR = (alphaR * fillR) +
              ((unAlphaR * ((((usqInt)(resultRBits)) >> 16))) * targetAlpha);
    resultRBits = ((
        (sqInt)((usqInt)((((sqInt)((resultR / resultAlphaR) + 0.5)))) << 16)));
  }
  if (!(alphaG == 0.0)) {
    unAlphaG = 1.0 - alphaG;
    resultAlphaG = alphaG + (unAlphaG * targetAlpha);
    resultG = (alphaG * fillG) +
              ((unAlphaG * ((((usqInt)(resultGBits)) >> 8))) * targetAlpha);
    resultGBits =
        (((sqInt)((usqInt)((((sqInt)((resultG / resultAlphaG) + 0.5)))) << 8)));
    resultAlphaBits =
        (((sqInt)((usqInt)((((sqInt)((resultAlphaG * 255.0) + 0.5)))) << 24)));
  }
  if (!(alphaB == 0.0)) {
    unAlphaB = 1.0 - alphaB;
...
/*	Blends fill color over background.
        ...
        For blending, alphas are in [0 .. 1.0] and R, G, B color components are
   in [0 .. 255]
 */
...
static sqInt blendFillOnlyWPAtantiAliasAlphaByte(sqInt pixelIndex,
  ...
  resultBBits = targetWord & 0xFF;
  resultR = (alpha * fillR) +
            ((unAlpha * ((((usqInt)(resultRBits)) >> 16))) * targetAlpha);
  resultG = (alpha * fillG) +
            ((unAlpha * ((((usqInt)(resultGBits)) >> 8))) * targetAlpha);
  resultB = (alpha * fillB) + ((unAlpha * resultBBits) * targetAlpha);
  resultAlphaBits =
      (((sqInt)((usqInt)((((sqInt)((resultAlpha * 0xFF) + 0.5)))) << 24)));
  resultRBits =
      (((sqInt)((usqInt)((((sqInt)((resultR / resultAlpha) + 0.5)))) << 16)));
  resultGBits =
      (((sqInt)((usqInt)((((sqInt)((resultG / resultAlpha) + 0.5)))) << 8)));
...
blendStrokeOnlyWPAtantiAliasAlphaByte(sqInt pixelIndex,
  ...
  resultGBits = targetWord & 0xFF00;
  resultBBits = targetWord & 0xFF;
  resultR = (alpha * strokeR) +
            ((unAlpha * ((((usqInt)(resultRBits)) >> 16))) * targetAlpha);
  resultG = (alpha * strokeG) +
            ((unAlpha * ((((usqInt)(resultGBits)) >> 8))) * targetAlpha);
...
primArc(void) {
  ...
  hops =
      (((sqInt)(((((trx < try) ? try : trx)) * (fabs(sweepAngle))) / hop))) + 2;
  d = hops;
  for (h = 0; h <= hops; h += 1) {
...
/*	Blends fill color over background.
        Sub pixel anti aliasing means each R, G, B subpixel might use a
   different anti aliasing alpha. If targetAssumedOpaque is false, target
   translucency is computed correctly. If targetAssumedOpaque is true (and
   target translucency is actually 255 on input), an optimization is made and
   target translucency is set to 255. For blending, alphas are in [0 .. 1.0] and
   R, G, B color components are in [0 .. 255]
 */
...
primBlendFillOnly(void) {
  ...
  for (displayY = t; displayY <= b; displayY += 1) {
    if (clippingSpec) {
      ...
      antiAliasedClippedLeftPixel =
          (clippingSpecL >= l ? clippingSpecL : targetWidth);
      antiAliasedClippedRightPixel =
          (clippingSpecR <= r ? clippingSpecR : targetWidth);
    }
        ...
        for (idx = 1; idx <= toDoLimit; idx += 1) {
          ...
          if (alphasOrEdgeCountsInThisSegment) {
            ...
            if (edgesThisPixelWord) {
              ...
              edgesThisPixelR =
                  (((usqInt)((edgesThisPixelWord & 0xFF0000))) >> 16);
              edgesThisPixelG =
                  (((usqInt)((edgesThisPixelWord & 0xFF00))) >> 8);
              edgesThisPixelB = edgesThisPixelWord & 0xFF;

              /* Not needed in C or OpenCL, just use a wraparound 8 bit type,
               * such uint8_t */
          ...
          if ((displayX >= clippingSpecL) && (displayX <= clippingSpecR)) {
            ...
            if ((opaqueFillColorWord != 0) &&
                ((strokeAntiAliasAlphasWord == 0) && (isGreenInside))) {
              targetBits[pixelIndex] = opaqueFillColorWord;
              morphIds[pixelIndex] = currentMorphId;
            } else {
              if ((strokeAntiAliasAlphasWord != 0) || (isGreenInside)) {
                blendFillOnlyAtredIsInsidegreenIsInsideblueIsInsideantiAliasAlphasWord(
                    pixelIndex, isRedInside, isGreenInside, isBlueInside,
                    strokeAntiAliasAlphasWord);
              }
            }

            /* Overwrite with fill color is ok and we are in the fill, far from
               anti aliasing If no alpha, and isGreenInside is true, isRedInside
               and isBlueInside are also true General case.
               (strokeAntiAliasAlphasWord = 0 and outside the shape means NOP)
             */
            if (mustResetColor) {
              fillA = realFillAlpha;
...
/*	Blends fill color over background.
        ...
        actually 255 on input), an optimization is made and target translucency
   is set to 255. For blending, alphas are in [0 .. 1.0] and R, G, B color
          ...
          if ((displayX >= clippingSpecL) && (displayX <= clippingSpecR)) {
            ...
            if (edgesUpToThisPixel) {
              if ((opaqueFillColorWord != 0) &&
                ...
                blendFillOnlyWPAtantiAliasAlphaByte(
                    pixelIndex, 0x7F - strokeAntiAliasAlphaBits);
              }
            } else {
              if (strokeAntiAliasAlphaBits) {
                blendFillOnlyWPAtantiAliasAlphaByte(pixelIndex,
                                                    strokeAntiAliasAlphaBits);
              }
            }

            /* Inside the shape
               Still in the anti aliasing area, but outside the shape, strictly
               speaking. */
            if (mustResetColor) {
              fillA = realFillAlpha;
              opaqueFillColorWord = realOpaqueFillColorWord;
          ...
          if ((displayX >= clippingSpecL) && (displayX <= clippingSpecR)) {
            ...
            if (strokeAntiAliasAlphasWord) {
              if (strokeAntiAliasAlphasWord == 0x7F7F7F) {
                ...
                /* begin
                 * blendStrokeAndFillAt:redIsInside:greenIsInside:blueIsInside:antiAliasAlphasWord:
                 */
                ...
                if (isRedInside) {
                  alphaR = (strokeAARedAlpha * strokeA) +
                           ((1.0 - strokeAARedAlpha) * fillA);
                  foreR = (strokeAARedAlpha * strokeR) +
                          ((1.0 - strokeAARedAlpha) * fillR);
                } else {
                  alphaR = strokeAARedAlpha * strokeA;
                  foreR = strokeR;
                }

                /* Do gradient between stroke and fill. Blend the result over
                   background Blend stroke over background */
                if (isGreenInside) {
                  alphaG = (strokeAAGreenAlpha * strokeA) +
                           ((1.0 - strokeAAGreenAlpha) * fillA);
                  foreG = (strokeAAGreenAlpha * strokeG) +
                          ((1.0 - strokeAAGreenAlpha) * fillG);
                } else {
                  alphaG = strokeAAGreenAlpha * strokeA;
                  foreG = strokeG;
                }
                ...
                if (isBlueInside) {
                  alphaB = (strokeAABlueAlpha * strokeA) +
                           ((1.0 - strokeAABlueAlpha) * fillA);
                  foreB = (strokeAABlueAlpha * strokeB) +
                          ((1.0 - strokeAABlueAlpha) * fillB);
                } else {
                  alphaB = strokeAABlueAlpha * strokeA;
                  foreB = strokeB;
                }
                ...
                /* These if are not really needed. just ignore them if we use
                 * simd instructions. */
                if (!(alphaR == 0.0)) {
                  unAlphaR = 1.0 - alphaR;
                  resultAlphaR = alphaR + (unAlphaR * targetAlpha);
                  resultR = (alphaR * foreR) +
                            ((unAlphaR * ((((usqInt)(resultRBits)) >> 16))) *
                             targetAlpha);
                  resultRBits =
                      (((sqInt)((usqInt)((
                                    ((sqInt)((resultR / resultAlphaR) + 0.5))))
                                << 16)));
                }
                if (!(alphaG == 0.0)) {
                  unAlphaG = 1.0 - alphaG;
                  resultAlphaG = alphaG + (unAlphaG * targetAlpha);
                  resultG = (alphaG * foreG) +
                            ((unAlphaG * ((((usqInt)(resultGBits)) >> 8))) *
                             targetAlpha);
                  resultGBits =
                      (((sqInt)((usqInt)((
                                    ((sqInt)((resultG / resultAlphaG) + 0.5))))
                                << 8)));
                  resultAlphaBits = (((
                      sqInt)((usqInt)((((sqInt)((resultAlphaG * 255.0) + 0.5))))
                             << 24)));
                }
                if (!(alphaB == 0.0)) {
                  unAlphaB = 1.0 - alphaB;
                  resultAlphaB = alphaB + (unAlphaB * targetAlpha);
                  resultB = (alphaB * foreB) +
                            ((unAlphaB * resultBBits) * targetAlpha);
                  resultBBits = ((sqInt)((resultB / resultAlphaB) + 0.5));
                }
                targetWord = ((resultAlphaBits | resultRBits) | resultGBits) |
                             resultBBits;
                targetBits[pixelIndex] = targetWord;
                morphIds[pixelIndex] = currentMorphId;
              }
            ...
            /* Not in the stroke at all. Either fully in the fill, or outside
               the shape (pixel is unaffected). At least one subpixel in the
               stroke. */
            if (mustResetColor) {
      ...
      if (alphasOrEdgeCountsInThisSegment || (edgesUpToThisPixel != 0)) {
        ...
        for (idx = 1; idx <= toDoLimit; idx += 1) {
          ...
          if ((displayX >= clippingSpecL) && (displayX <= clippingSpecR)) {
            ...
            if (strokeAntiAliasAlphaBits) {
              if (strokeAntiAliasAlphaBits == 0x7F) {
                ...
                if (edgesUpToThisPixel) {
                  ...
                  resultBBits = targetWord & 0xFF;
                  resultR = (alpha * foreR) +
                            ((unAlpha * ((((usqInt)(resultRBits)) >> 16))) *
                             targetAlpha);
                  resultG = (alpha * foreG) +
                            ((unAlpha * ((((usqInt)(resultGBits)) >> 8))) *
                             targetAlpha);
                  resultB =
                      (alpha * foreB) + ((unAlpha * resultBBits) * targetAlpha);
                  resultAlphaBits = ((
                      (sqInt)((usqInt)((((sqInt)((resultAlpha * 0xFF) + 0.5))))
                              << 24)));
                  resultRBits =
                      (((sqInt)((usqInt)((
                                    ((sqInt)((resultR / resultAlpha) + 0.5))))
                                << 16)));
                  resultGBits =
                      (((sqInt)((usqInt)((
                                    ((sqInt)((resultG / resultAlpha) + 0.5))))
                                << 8)));
                  resultBBits = ((sqInt)((resultB / resultAlpha) + 0.5));
                  targetWord = ((resultAlphaBits | resultRBits) | resultGBits) |
                               resultBBits;
                  ...
                  blendStrokeOnlyWPAtantiAliasAlphaByte(
            ...
            /* Not in the stroke at all. Either fully in the fill, or outside
               the shape (pixel is unaffected). At least partially in the
               stroke. */
            if (mustResetColor) {
              strokeA = realStrokeAlpha;
              fillA = realFillAlpha;
          ...
          if (strokeAntiAliasAlphasWord) {
            ...
            if ((displayX >= clippingSpecL) && (displayX <= clippingSpecR)) {
              ...
              if ((opaqueStrokeColorWord != 0) &&
                ...
                blendStrokeOnlyAtantiAliasAlphasWord(pixelIndex,
                                                     strokeAntiAliasAlphasWord);
              }
            }
            if (mustResetColor) {
          ...
          if (strokeAntiAliasAlphaBits) {
            ...
            if ((displayX >= clippingSpecL) && (displayX <= clippingSpecR)) {
              ...
              if ((opaqueStrokeColorWord != 0) &&
                ...
                blendStrokeOnlyWPAtantiAliasAlphaByte(pixelIndex,
                                                      strokeAntiAliasAlphaBits);
              }

              /* Optimize inner part of a wide stroke: Fully opaque stroke (and
                 target), no anti aliasing, no clipping at this point. General
                 case. */
              if (mustResetColor) {
                strokeA = realStrokeAlpha;
                opaqueStrokeColorWord = realOpaqueStrokeColorWord;
...
primCubicBezier(void) {
  ...
  pvt_cubicBezierFromXytoXycontrol1Xycontrol2Xy(
      xFrom, yFrom, xTo, yTo, xControl1, yControl1, xControl2, yControl2);
  if (!(failed())) {
    pop(8);
  }
  return null;
...
primDisplayByteString(void) {
  ...
  /* begin
   * displayStringLoop:displayIf:wholePixel:contourIndexAccessor:from:to:atx:y:scalex:y:contourData:
   */
  trajectoryLength = 0.0;
  needsFullAlphaCircle = 1;
  txA11 = txA11 * sx;
  ...
  for (index = (startIndex - 1); index < stopIndex; index += 1) {
    ...
    for (idx = 1; idx <= numContours; idx += 1) {
      ...
      for (idx2 = 1; idx2 <= numBeziers; idx2 += 1) {
        ...
        spanLeft = ((spanLeft < (((xMinEnd < ((xMinEnd + controlX) / 2.0))
                                      ? xMinEnd
                                      : ((xMinEnd + controlX) / 2.0))))
                        ? spanLeft
                        : (((xMinEnd < ((xMinEnd + controlX) / 2.0))
                                ? xMinEnd
                                : ((xMinEnd + controlX) / 2.0))));
        spanRight = ((spanRight < (((xMaxEnd < ((xMaxEnd + controlX) / 2.0))
                                        ? ((xMaxEnd + controlX) / 2.0)
                                        : xMaxEnd)))
                         ? (((xMaxEnd < ((xMaxEnd + controlX) / 2.0))
                                 ? ((xMaxEnd + controlX) / 2.0)
                                 : xMaxEnd))
                         : spanRight);
        spanTop = ((spanTop < (((yMinEnd < ((yMinEnd + controlY) / 2.0))
                                    ? yMinEnd
                                    : ((yMinEnd + controlY) / 2.0))))
                       ? spanTop
                       : (((yMinEnd < ((yMinEnd + controlY) / 2.0))
                               ? yMinEnd
                               : ((yMinEnd + controlY) / 2.0))));
        spanBottom = ((spanBottom < (((yMaxEnd < ((yMaxEnd + controlY) / 2.0))
                                          ? ((yMaxEnd + controlY) / 2.0)
                                          : yMaxEnd)))
                          ? (((yMaxEnd < ((yMaxEnd + controlY) / 2.0))
                                  ? ((yMaxEnd + controlY) / 2.0)
                                  : yMaxEnd))
                          : spanBottom);
        ...
        /* Will be corrected for each hop. This, being close to pointFrom, is a
         * good initial guess for first correction. */
        ...
        while (1) {
          ...
          /* Now adjust the increment to aim at the required hop length, and
           * recompute next point. */
          dx = x - x0;
          dy = y - y0;
          length = sqrt((dx * dx) + (dy * dy));

          /* Don't grow increment too much in one step. More importantly, don't
           * divide by zero under any circumstances. */
          ...
          do {
            ...
            /* Don't grow increment too much in one step. More importantly,
             * don't divide by zero under any circumstances. */
            correction = hop / (((length < 0.1) ? 0.1 : length));
        ...
        /* Note: For TrueType font definitions, we assume that all contour
           fragments start exactly where the previous ends. This means that the
           end point is only added for the last fragment of the contour, and not
      ...
      /* Similar effect to ensureClosePath in #finishPath:,
         but assume the TrueType definition is essentially right, and there
         might only be a rounding error. So, don't draw a line, but just
         (possibly) correct edgeCounts. The possibility of rounding error is
         most likely zero. Anyway, this is cheap. */
      updateEdgeCountAtXy(contourStartX, contourStartY);
    }
    nextGlyphX += advanceWidth;
  }

  /* Index points to a byte in a ByteString or UTF8String, or to a code point in
   * an UTF32String */
  txA11 = txA11 / sx;
  txA12 = txA12 / sy;
  txA21 = txA21 / sx;
  ...
  for (index = (startIndex - 1); index < stopIndex; index += 1) {
    ...
    for (idx = 1; idx <= numContours; idx += 1) {
      ...
      /* Similar effect to ensureClosePath in #finishPath:,
         but assume the TrueType definition is essentially right, and there
         might only be a rounding error. So, don't draw a line, but just
         (possibly) correct edgeCountsWP. The possibility of rounding error is
         most likely zero. Anyway, this is cheap. */
...
primDisplayUtf32(void) {
  ...
  for (index = (startIndex - 1); index < stopIndex; index += 1) {
    ...
    iSqInt =
        (utf32 <= 0x7F
             ? ((utf8Byte = utf32), contourDataIndexes[utf8Byte])
             : (utf32 <= 0x7FF
    ...
    if (!baseIndex) {
      ...
      for (idx = 1; idx <= numContours; idx += 1) {
        ...
        for (idx2 = 1; idx2 <= numBeziers; idx2 += 1) {
          ...
          /* Will be corrected for each hop. This, being close to pointFrom, is
           * a good initial guess for first correction. */
          dx = fabsf(endX - startX);
          dy = fabsf(endY - startY);
          aux1 = ((dx < dy) ? dy : dx);
          aux1 = 0.5 / aux1;
          ...
          /* Note: For TrueType font definitions, we assume that all contour
             fragments start exactly where the previous ends. This means that
             the end point is only added for the last fragment of the contour,
             and not for each one of them. */
          startX = endX;
          startY = endY;
        }
        updateAlphasForXy(endX, endY);
...
primPathSequence(void) {
  ...
  while (i < size) {
    ...
    case 2:
      ...
      i += 1;
      pvt_quadraticBezierFromXytoXycontrolXy(startX, startY, endX, endY,
                                             control1X, control1Y);
      startX = endX;
      startY = endY;
      break;
    case 3:
      ...
      i += 1;
      pvt_cubicBezierFromXytoXycontrol1Xycontrol2Xy(startX, startY, endX, endY,
                                                    control1X, control1Y,
                                                    control2X, control2Y);
      startX = endX;
      startY = endY;
...
primPathSequenceWP(void) {
  ...
  while (i < size) {
    ...
    case 2:
      ...
      control1Y = aFloat32Array[i];
      i += 1;
      pvt_quadraticBezierWPFromXytoXycontrolXy(startX, startY, endX, endY,
                                               control1X, control1Y);
      startX = endX;
      startY = endY;
      break;
    case 3:
      ...
      i += 1;
      pvt_cubicBezierWPFromXytoXycontrol1Xycontrol2Xy(
          startX, startY, endX, endY, control1X, control1Y, control2X,
          control2Y);
      startX = endX;
      startY = endY;
      break;
...
primQuadraticBezier(void) {
  ...
  pvt_quadraticBezierFromXytoXycontrolXy(xFrom, yFrom, xTo, yTo, xControl,
                                         yControl);
  if (!(failed())) {
    pop(6);
  }
...
primQuadraticBezierWP(void) {
  ...
  xControl = stackFloatValue(1);
  yControl = stackFloatValue(0);
  pvt_quadraticBezierWPFromXytoXycontrolXy(xFrom, yFrom, xTo, yTo, xControl,
                                           yControl);
  if (!(failed())) {
    pop(6);
  }
...
primSpanBottom(void) {
  methodReturnValue(
      integerObjectOf((((sqInt)(spanBottom + auxStrokeWidthDilatedHalf)))));
  return null;
}
...
EXPORT(sqInt)
primSpanLeft(void) {
  /* (int(z+1)) works equally well than the more intuitive but slower
   * (int(ceil(z)) */
  methodReturnValue(integerObjectOf(((
      (sqInt)(((spanLeft - auxStrokeWidthDilatedHalf) - subPixelDelta) + 1)))));
  return null;
}
...
primSpanRight(void) {
  ...
  methodReturnValue(integerObjectOf(
      ((((sqInt)((spanRight + auxStrokeWidthDilatedHalf) + subPixelDelta))) +
       1)));
  return null;
}
...
primSpanTop(void) {
   ...
   * (int(ceil(z)) */
  methodReturnValue(
      integerObjectOf((((sqInt)((spanTop - auxStrokeWidthDilatedHalf) + 1)))));
  return null;
}
...
primStrokeWidthHop(void) {
  ...
  auxStrokeWidthDilatedHalfSquared =
      auxStrokeWidthDilatedHalf * auxStrokeWidthDilatedHalf;
...
static sqInt pvt_cubicBezierFromXytoXycontrol1Xycontrol2Xy(
  ...
  /* This computed span of the Bezier curve is a bit pessimistic (larger than
   * strict bounds), but safe. */
  ...
  spanLeft =
      ((spanLeft <
        (((xMinEnd <
           ((xMinEnd * 0.25) +
            ((((txControl1 < txControl2) ? txControl1 : txControl2)) * 0.75)))
              ? xMinEnd
              : ((xMinEnd * 0.25) +
                 ((((txControl1 < txControl2) ? txControl1 : txControl2)) *
                  0.75)))))
           ? spanLeft
           : (((xMinEnd <
                ((xMinEnd * 0.25) +
  ...
  spanRight =
      ((spanRight <
        (((xMaxEnd <
           ((xMaxEnd * 0.25) +
            ((((txControl1 < txControl2) ? txControl2 : txControl1)) * 0.75)))
              ? ((xMaxEnd * 0.25) +
                 ((((txControl1 < txControl2) ? txControl2 : txControl1)) *
                  0.75))
              : xMaxEnd)))
           ? (((xMaxEnd <
                ((xMaxEnd * 0.25) +
  ...
  spanTop =
      ((spanTop <
        (((yMinEnd <
           ((yMinEnd * 0.25) +
            ((((tyControl1 < tyControl2) ? tyControl1 : tyControl2)) * 0.75)))
              ? yMinEnd
              : ((yMinEnd * 0.25) +
                 ((((tyControl1 < tyControl2) ? tyControl1 : tyControl2)) *
                  0.75)))))
           ? spanTop
           : (((yMinEnd <
                ((yMinEnd * 0.25) +
  ...
  spanBottom =
      ((spanBottom <
        (((yMaxEnd <
           ((yMaxEnd * 0.25) +
            ((((tyControl1 < tyControl2) ? tyControl2 : tyControl1)) * 0.75)))
              ? ((yMaxEnd * 0.25) +
                 ((((tyControl1 < tyControl2) ? tyControl2 : tyControl1)) *
                  0.75))
              : yMaxEnd)))
           ? (((yMaxEnd <
                ((yMaxEnd * 0.25) +
  ...
  /* Will be corrected for each hop. This, being close to pointFrom, is a good
   * initial guess for first correction. */
  increment =
      (((0.5 / (((dx < dy) ? dy : dx))) < 0.5) ? (0.5 / (((dx < dy) ? dy : dx)))
                                               : 0.5);
  t = 0.0;
  while (1) {
    t0 = t;
    ...
    /* Now adjust the increment to aim at the required hop length, and recompute
     * next point. */
    dx = x - x0;
    dy = y - y0;
    length = sqrt((dx * dx) + (dy * dy));

    /* Don't grow increment too much in one step. More importantly, don't divide
     * by zero under any circumstances. */
    correction = hop / (((length < 0.1) ? 0.1 : length));
    do {
      increment = increment * correction;
      t = t0 + increment;
      ...
      f3 = f23 * t;
      f4 = (t * t) * t;
      x = (((f1 * txFrom) + (f2 * txControl1)) + (f3 * txControl2)) +
          (f4 * txTo);
      y = (((f1 * tyFrom) + (f2 * tyControl1)) + (f3 * tyControl2)) +
          (f4 * tyTo);
      dx = x - x0;
      dy = y - y0;
      length = sqrt((dx * dx) + (dy * dy));
...
static sqInt pvt_lineFromXytoXy(float xFrom, float yFrom, float xTo,
                                float yTo) {
  float dx;
  float dy;
  int hops;
  ...
  spanLeft = ((spanLeft < (((txFrom < txTo) ? txFrom : txTo)))
                  ? spanLeft
                  : (((txFrom < txTo) ? txFrom : txTo)));
  spanRight = ((spanRight < (((txFrom < txTo) ? txTo : txFrom)))
                   ? (((txFrom < txTo) ? txTo : txFrom))
                   : spanRight);
  spanTop = ((spanTop < (((tyFrom < tyTo) ? tyFrom : tyTo)))
                 ? spanTop
                 : (((tyFrom < tyTo) ? tyFrom : tyTo)));
  spanBottom = ((spanBottom < (((tyFrom < tyTo) ? tyTo : tyFrom)))
                    ? (((tyFrom < tyTo) ? tyTo : tyFrom))
                    : spanBottom);
  t = 0.0;
  increment = 1.0 / hops;
  while (t < 1.0) {
...
static sqInt pvt_quadraticBezierFromXytoXycontrolXy(float xFrom, float yFrom,
  ...
  yMinEnd = ((tyFrom < tyTo) ? tyFrom : tyTo);
  yMaxEnd = ((tyFrom < tyTo) ? tyTo : tyFrom);
  spanLeft = ((spanLeft < (((xMinEnd < ((xMinEnd + txControl) / 2.0))
                                ? xMinEnd
                                : ((xMinEnd + txControl) / 2.0))))
                  ? spanLeft
                  : (((xMinEnd < ((xMinEnd + txControl) / 2.0))
                          ? xMinEnd
                          : ((xMinEnd + txControl) / 2.0))));
  spanRight = ((spanRight < (((xMaxEnd < ((xMaxEnd + txControl) / 2.0))
                                  ? ((xMaxEnd + txControl) / 2.0)
                                  : xMaxEnd)))
                   ? (((xMaxEnd < ((xMaxEnd + txControl) / 2.0))
                           ? ((xMaxEnd + txControl) / 2.0)
                           : xMaxEnd))
                   : spanRight);
  spanTop = ((spanTop < (((yMinEnd < ((yMinEnd + tyControl) / 2.0))
                              ? yMinEnd
                              : ((yMinEnd + tyControl) / 2.0))))
                 ? spanTop
                 : (((yMinEnd < ((yMinEnd + tyControl) / 2.0))
                         ? yMinEnd
                         : ((yMinEnd + tyControl) / 2.0))));
  spanBottom = ((spanBottom < (((yMaxEnd < ((yMaxEnd + tyControl) / 2.0))
                                    ? ((yMaxEnd + tyControl) / 2.0)
                                    : yMaxEnd)))
                    ? (((yMaxEnd < ((yMaxEnd + tyControl) / 2.0))
                            ? ((yMaxEnd + tyControl) / 2.0)
                            : yMaxEnd))
                    : spanBottom);
...
/*	In a basic implementation, this method computes anti aliasing
   translucency for all possibly affected pixels as determined by the outer
   radius of the pen nib: auxStrokeWidthDilatedHalf. But usually the next point
   visited is very close to the last, and there's no real need to fill the inner
   part of the pen nib as opaque, as it was already set. So, unless
   needsFullAlphaCircle, only the outer ring is computed and stored. The inner
   radius of this ring is auxStrokeWidthErodedHalfSquared
 */
...
static sqInt updateAlphasForXy(float x, float y) {
  ...
  if (!(dashBitLength == 0.0)) {
    ...
    bit = ((((sqInt)(trajectoryLength / dashBitLength))) + dashBitOffset) %
          dashBitCount;
  ...
  /* Compute trajectory length. This is not precise. In many cases the actual
     hop used is smaller than this. Compute affected rect. Honor clipRect */
      ...
      if (needsFullAlphaCircle ||
        ...
        if (!(alphaWord == 0x7F7F7F)) {
          ...
          if (distanceToAxisSquared < auxStrokeWidthDilatedHalfSquared) {
            candidateAlpha = ((sqInt)((auxStrokeWidthDilatedHalf -
                                       (sqrt(distanceToAxisSquared))) *
                                      auxAntiAliasingWidthScaledInverse));
            candidateAlpha = ((((usqInt)(candidateAlpha) << 16)));
            if (candidateAlpha > redAlpha) {
              doUpdate = 1;
              redAlpha =
                  ((candidateAlpha < 0x7F0000) ? candidateAlpha : 0x7F0000);
            }
          }
          ...
          if (distanceToAxisSquared < auxStrokeWidthDilatedHalfSquared) {
            ...
            if (candidateAlpha > greenAlpha) {
              doUpdate = 1;
              greenAlpha =
                  ((candidateAlpha < 0x7F00) ? candidateAlpha : 0x7F00);
            }
          }
...
static sqInt updateAlphasWPForXy(float x, float y) {
  ...
  if ((strokeWidth == 0.0) && ((fabs(antiAliasingWidth - 1.6)) < 1.0e-6)) {
      ...
      if (distanceToAxisSquared < auxStrokeWidthDilatedHalfSquared) {
        if (needsFullAlphaCircle ||
          ...
          if (!(alphaByte == 0x7F)) {
            ...
            candidateAlpha =
                ((sqInt)((((aux1 < antiAliasingWidth) ? aux1
                                                      : antiAliasingWidth)) *
                         auxAntiAliasingWidthScaledInverse));
...
static sqInt updateAlphasWPZeroStrokeForXy(float x, float y) {
  ...
  if (distanceToAxisSquared < 0.64) {
    ...
    if (!(alphaByte == 0x7F)) {
      candidateAlpha =
          ((sqInt)((0.8 - (sqrt(distanceToAxisSquared))) * 79.375));
      if (candidateAlpha > alphaByte) {
...
/*	The Contour of the current morph is the pixel area affected, but without
        holes. For each y line where a pixel is affected, record the leftmost
   and rightmost affected pixels. Note: Only includes pen trajectory, but not
   stroke width. */
...
static sqInt updateEdgeCountAtXy(float x, float y) {
  ...
  if (!(((thisYTruncated >= (clipTop - 1)) &&
         (thisYTruncated <= clipBottom)))) {
    return 0;
  }
  ...
  /* All edge count at the left of the clipRect are added there (at the left of
     the clipRect). The effect is the same, and we need to clean up less stuff
     afterwards. More important, it avoids trying to acess pixels outside our
     form, i.e. invalid array acesses. */
  pixelIndexBase = pixelY * targetWidth;

  /* take the next red subpixel center to the right of x */
  redOffset = (((((sqInt)((x + subPixelDelta) + 1))) < clipLeft)
                   ? clipLeft
                   : (((sqInt)((x + subPixelDelta) + 1))));

  /* take the next green subpixel center to the right of x */
  greenOffset =
      (((((sqInt)(x + 1))) < clipLeft) ? clipLeft : (((sqInt)(x + 1))));

  /* take the next blue subpixel center to the right of x */
  blueOffset = (((((sqInt)((x - subPixelDelta) + 1))) < clipLeft)
                    ? clipLeft
                    : (((sqInt)((x - subPixelDelta) + 1))));
  ...
  /* Three possible cases here: RGB in one word (pixel); RG in one, and G in
   * another; R in one, GB in another */
...
void *VectorEnginePlugin_exports[][3] = {
    {(void *)_m, "dashedStrokeBitsSet\000\000\000",
     (void *)dashedStrokeBitsSet},
    {(void *)_m, "getModuleName", (void *)getModuleName},
    {(void *)_m, "pluginApiVersion\000\377\000", (void *)pluginApiVersion},
    {(void *)_m, "primAntiAliasingWidthsubPixelDelta\000\000\000",
     (void *)primAntiAliasingWidthsubPixelDelta},
    ...
    {(void *)_m, "primBlendFillOnlyWP\000\000\000",
     (void *)primBlendFillOnlyWP},
    {(void *)_m, "primBlendStrokeAndFill\000\000\000",
     (void *)primBlendStrokeAndFill},
    {(void *)_m, "primBlendStrokeAndFillWP\000\000\000",
     (void *)primBlendStrokeAndFillWP},
    {(void *)_m, "primBlendStrokeOnly\000\000\000",
     (void *)primBlendStrokeOnly},
    {(void *)_m, "primBlendStrokeOnlyWP\000\000\000",
     (void *)primBlendStrokeOnlyWP},
    {(void *)_m, "primClearClippingSpec\000\377\000",
     (void *)primClearClippingSpec},
    {(void *)_m, "primClipLeftclipTopclipRightclipBottom\000\000\000",
     (void *)primClipLeftclipTopclipRightclipBottom},
    ...
    {(void *)_m, "primDisplayByteString\000\000\000",
     (void *)primDisplayByteString},
    {(void *)_m, "primDisplayByteStringWP\000\000\000",
     (void *)primDisplayByteStringWP},
    ...
    {(void *)_m, "primNewTrajectoryFragment\000\377\000",
     (void *)primNewTrajectoryFragment},
    {(void *)_m, "primPathSequence\000\000\000", (void *)primPathSequence},
    {(void *)_m, "primPathSequenceWP\000\000\000", (void *)primPathSequenceWP},
    {(void *)_m, "primQuadraticBezier\000\000\000",
     (void *)primQuadraticBezier},
    {(void *)_m, "primQuadraticBezierWP\000\000\000",
     (void *)primQuadraticBezierWP},
    {(void *)_m, "primResetContour\000\000\000", (void *)primResetContour},
    {(void *)_m, "primSetClippingSpec\000\000\000",
     (void *)primSetClippingSpec},
    ...
    {(void *)_m, "primTargetAssumedOpaque\000\000\000",
     (void *)primTargetAssumedOpaque},
    {(void *)_m, "primUpdateContourLastLine\000\377\000",
     (void *)primUpdateContourLastLine},
