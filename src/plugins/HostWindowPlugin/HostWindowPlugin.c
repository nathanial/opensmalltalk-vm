/* Automatically generated by
        SmartSyntaxPluginCodeGenerator VMMaker.oscog-eem.3599 uuid:
   1a32964f-d425-4ed5-b830-6e8ce3d98688
        (* Compiler-eem.518)
   from
        HostWindowPlugin VMMaker.oscog-eem.3599 uuid:
   1a32964f-d425-4ed5-b830-6e8ce3d98688
 */
...
static const char *moduleName =
    "HostWindowPlugin VMMaker.oscog-eem.3599 " INT_EXT;
...
/*	Create a host window of width 'w' pixels, height 'h' with the origin of
        ...
        Return the SmallInt value of the internal index to the window
   description block - which is whatever the host platform code needs it to be.
 */
...
primitiveCreateHostWindow(void) {
  ...
  x = integerValueOf(x);
  y = integerValueOf(y);
  list = firstIndexableField(stackValue(0));

  /* createWindowWidthheightoriginXyattr(int w, int h, int x, int y, int*
     attributeList) must create a hostwindow and return an integer index. Return
     0 if failed */
  listLength = slotSizeOf(((sqInt)(sqIntptr_t)(list)-BaseHeaderSize));
  windowIndex =
      createWindowWidthheightoriginXyattrlength(w, h, x, y, list, listLength);
  if (windowIndex > 0) {
...
/*	Set the icon of the window by fetching the logo in given path. Fail if
   the windowIndex is invalid or the platform routine returns -1 to indicate
        failure
 */
...
primitiveHostWindowIcon(void) {
  ...
  if (!((isIntegerObject((id = stackValue(1)))) && (isBytes(stackValue(0))))) {
    return primitiveFailFor(PrimErrBadArgument);
  }
  id = integerValueOf(id);
  pathString = firstIndexableField(stackValue(0));
  ...
  /* As the GetLastError function can return 0, but it is still an error, 1 is
   * added to all the GetLastError error codes. */
  return primitiveFail();
}
...
primitiveHostWindowPosition(void) {
  ...
  _return_value = makePointwithxValueyValue(((short)(((usqInt)(pos)) >> 16)),
                                            ((short)(pos & 0xFFFF)));
  if (!(failed())) {
...
/*	Set the origin position of the user area of the window in pixels from
   the topleft corner of the screen- return the position actually set by the
        OS/GUI/window manager. Fail if the windowIndex is invalid or the
   platform routine returns -1 to indicate failure
 */
...
/*	Copyright (c) 2005-2024, 3D Immersive Collaboration Consulting, LLC.,
   All Rights Reserved.
 */
...
primitiveHostWindowScreenWorkAreaPosition(void) {
  ...
  _return_value = makePointwithxValueyValue(((short)(((usqInt)(size)) >> 16)),
                                            ((short)(size & 0xFFFF)));
  if (!(failed())) {
    methodReturnValue(_return_value);
  }
...
/*	Set the title bar label of the window. Fail if the windowIndex is
   invalid or the platform routine returns -1 to indicate failure
 */
...
primitiveHostWindowTitle(void) {
  ...
  id = integerValueOf(id);
  titleString = firstIndexableField(stackValue(0));
...
/*	Copyright (c) 2005-2024, 3D Immersive Collaboration Consulting, LLC.,
   All Rights Reserved.
 */
/*	Answer the origin position of the user area of the given native window
   in pixels from the topleft corner of the screen. Fail if the windowHandle is
   invalid or the platform routine returns -1 to indicate failure
 */
...
primitiveNativeDisplayPosition(void) {
  ...
  windowHandle = (BytesPerOop == 4 ? positive32BitValueOf(stackValue(0))
...
/*	Answer the size of the user area of the given native window in pixels.
        Fail if the windowHandle is invalid or the platform routine returns -1
   to indicate failure */
...
/*	Copyright (c) 2005-2024, 3D Immersive Collaboration Consulting, LLC.,
   All Rights Reserved.
 */
/*	Answer an Array of pairs of packed x,y repreeentation origins and
   extents for all the available monitors.
 */
...
primitiveScreenRectangles(void) {
  ...
  if ((screenRectangles = ioScreenRectangles())) {
...
/*	Host window analogue of DisplayScreen>
   primShowRectLeft:right:top:bottom: (Interpreter>primitiveShowDisplayRect)
   which takes the window index, bitmap details and the rectangle bounds. Fail
   if the windowIndex is invalid or the platform routine returns false to
   indicate failure
 */
...
primitiveShowHostWindowRect(void) {
  ...
  windowIndex = integerValueOf(windowIndex);
  dispBits = firstIndexableField(stackValue(7));
  w = integerValueOf(w);
  h = integerValueOf(h);
  d = integerValueOf(d);
  ...
  if (!(ioShowDisplayOnWindow(((unsigned char *)dispBits), w, h, d, left, right,
                              top, bottom, windowIndex))) {
    primitiveFail();
  }
  if (!(failed())) {
    pop(9);
...
/*	Answer the native Squeak window handle, presumably a pointer, as an
        unsigned integer.
        This value should be usable in native calls to the window manager as
   well as the argument to primitiveNativeWindowPosition: */
...
primitiveSqueakWindowHandle(void) {
  ...
  _return_value = /* positiveMachineIntegerFor: */
      (BytesPerWord == 8 ? positive64BitIntegerFor(((usqInt)handle))
                         : positive32BitIntegerFor(((usqInt)handle)));
  if (!(failed())) {
    methodReturnValue(_return_value);
  }
  return null;
...
EXPORT(sqInt)
shutdownModule(void) { return ioCloseAllWindows(); }
...
void *HostWindowPlugin_exports[][3] = {
    ...
    {(void *)_m, "primitiveCloseHostWindow\000\000\000",
     (void *)primitiveCloseHostWindow},
    {(void *)_m, "primitiveCreateHostWindow\000\000\000",
     (void *)primitiveCreateHostWindow},
    {(void *)_m, "primitiveHostWindowIcon\000\000\000",
     (void *)primitiveHostWindowIcon},
    {(void *)_m, "primitiveHostWindowPosition\000\000\000",
     (void *)primitiveHostWindowPosition},
    {(void *)_m, "primitiveHostWindowPositionSet\000\000\000",
     (void *)primitiveHostWindowPositionSet},
    {(void *)_m, "primitiveHostWindowScreenWorkAreaPosition\000\000\000",
     (void *)primitiveHostWindowScreenWorkAreaPosition},
    {(void *)_m, "primitiveHostWindowScreenWorkAreaSize\000\000\000",
     (void *)primitiveHostWindowScreenWorkAreaSize},
    {(void *)_m, "primitiveHostWindowSize\000\000\000",
     (void *)primitiveHostWindowSize},
    {(void *)_m, "primitiveHostWindowSizeSet\000\000\000",
     (void *)primitiveHostWindowSizeSet},
    {(void *)_m, "primitiveHostWindowTitle\000\000\000",
     (void *)primitiveHostWindowTitle},
    {(void *)_m, "primitiveNativeDisplayPosition\000\000\000",
     (void *)primitiveNativeDisplayPosition},
    {(void *)_m, "primitiveNativeDisplaySize\000\000\000",
     (void *)primitiveNativeDisplaySize},
    {(void *)_m, "primitiveNativeWindowPosition\000\000\000",
     (void *)primitiveNativeWindowPosition},
    {(void *)_m, "primitiveNativeWindowSize\000\000\000",
     (void *)primitiveNativeWindowSize},
    {(void *)_m, "primitiveScreenRectangles\000\377\000",
     (void *)primitiveScreenRectangles},
    {(void *)_m, "primitiveSetCursorPosition\000\000\000",
     (void *)primitiveSetCursorPosition},
    {(void *)_m, "primitiveShowHostWindowRect\000\000\000",
     (void *)primitiveShowHostWindowRect},
    {(void *)_m, "primitiveSqueakWindowHandle\000\377\000",
     (void *)primitiveSqueakWindowHandle},
...
EXPORT(signed short) primitiveNativeDisplayPositionMetadata = 0;
EXPORT(signed short) primitiveNativeDisplaySizeMetadata = 0;
EXPORT(signed short) primitiveNativeWindowPositionMetadata = 0;
EXPORT(signed short) primitiveNativeWindowSizeMetadata = 0;
