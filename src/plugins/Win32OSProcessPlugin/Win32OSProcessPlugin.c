/* Automatically generated by
   ...
   d9ef6856-42d8-4074-bde7-82f1e1d41282 (Compiler-eem.512) from
        Win32OSProcessPlugin VMConstruction-Plugins-OSProcessPlugin.oscog-eem.81
   uuid: 9b4cc8a5-4a9a-4dbb-92f1-f5290a13a66e
 */
#if defined(__clang__) || defined(__GNUC__)
static const char __buildInfo[] __attribute__((unused)) =
    "Win32OSProcessPlugin VMConstruction-Plugins-OSProcessPlugin.oscog-eem.81 "
    "uuid: 9b4cc8a5-4a9a-4dbb-92f1-f5290a13a66e " __DATE__;
#else
...
/* N.B. Use STACK_SIZE_PARAM_IS_A_RESERVATION because Windows apparently
   ignores the stack size parameter and uses the default in the executable. So
   the only way to reduce memory usage is via STACK_SIZE_PARAM_IS_A_RESERVATION.
 */
...
#define READCHARARRAYSIZE                                                      \
  512 /* Corresponds to expected max size of external semaphore table */
...
static const char *moduleName =
    ...
    "VMConstruction-Plugins-OSProcessPlugin.oscog-eem.81 " INT_EXT;
...
static HANDLE createThreadMutex(void) { return CreateMutex(NULL, false, NULL); }
...
static sqInt getEnvironmentStringsAsType(sqInt classIdentifier) {
  ...
  while (*p != 0) {
    ...
    len = strlen(aCString);
    string = instantiateClassindexableSize(classIdentifier, len);
    strncpy(arrayValueOf(string), aCString, len);
...
/*	Wait for a process HANDLE to exit, then signal the Semaphore at index.
        Answer the exit status of the process. This is the code to be executed
   in a thread to provide asychronous notification of an external process exit.
 */
...
/*	For debugging only. Answer the current values of readCharBufferArray,
        ...
        corresponding to a semaphore for one read handler thread. Answer an
   Array with the buffered character, the character count, and the status value.
 */
...
/*	Create a process to run commandString. Answer an Array formed from the
        PROCESS_INFORMATION structure for the new process. The caller is
   expected to close the process and thread handles when they are no longer
   needed. The result array contains hProcess, hThread, dwProcessId, dwThreadId.

        This primitive has been replaced by
   #primitiveCommandWithInputOutputError, and will be removed in future versions
   of OSProcess.
 */
...
primitiveCommand(void) {
  ...
  if (!(CreateProcess(NULL, cStringPtr, NULL, NULL, TRUE, NORMAL_PRIORITY_CLASS,
...
/*	Create a process to run commandString, using the specified input,
   output, and error streams. If any of the input, output, or error streams is
   nil, then the current values of these handles in this OS process will be
   passed to the new process. Answer an Array formed from the
   PROCESS_INFORMATION structure for the new process. The caller is expected to
   close the process and thread handles when they are no longer needed. The
   result array contains hProcess, hThread, dwProcessId, dwThreadId.
 */
...
primitiveGetCurrentWorkingDirectory(void) {
  ...
  /* Determine required size of result buffer (this form counts the terminating
   * null) */
  ...
  /* Call GetCurrentDirectory() again to get the actual result (which does not
   * count the terminating null (!!)) */
  cwdLen = GetCurrentDirectoryW(requiredLen, buffer);
  if ((cwdLen == 0) || (cwdLen > requiredLen)) {
    return primitiveFail();
  }

  /* Get required length of UTF8 encoded string, without terminating NULL */
  utf8Len = WideCharToMultiByte(OSPP_CP_UTF8(), 0, buffer, cwdLen, null, 0,
                                null, null);

  /* Allocate the real buffer */
  cwdString = instantiateClassindexableSize(classString(), utf8Len);
  if (utf8Len > 0) {
    WideCharToMultiByte(OSPP_CP_UTF8(), 0, buffer, cwdLen,
                        arrayValueOf(cwdString), utf8Len, null, null);
  }

  /* copy directly to the buffer, because this form does not include terminating
   * NULL */
...
primitiveGetCurrentWorkingDirectoryAsBytes(void) {
  ...
  popthenPush(1, cwdString);
  /* end getCurrentWorkingDirectoryAsType: */
l1:;
  return 0;
}
...
primitiveGetEnvironmentStrings(void) {
  ...
  while (*p != 0) {
    ...
    /* Get required length of UTF8 encoded string, without terminating NULL */
    utf8Len = WideCharToMultiByte(OSPP_CP_UTF8(), 0, p, wcslen(p), null, 0,
                                  null, null);
    pushRemappableOop(envArray);
    ...
    /* Write the UTF8 conversion directly in the Smalltalk String, without
     * terminating NULL */
    WideCharToMultiByte(OSPP_CP_UTF8(), 0, p, wcslen(p), arrayValueOf(string),
                        utf8Len, null, null);
    stObjectatput(envArray, count, string);
    while (*p != 0) {
      p += 1;
...
primitiveGetExitStatusForHandle(void) {
  ...
  if (GetExitCodeProcess(handle, &exitStatus)) {
...
/*	Answer a two element array containing the sqFile data structure
        representing standard error stream for my OS process, and a flag (true
   or false) to indicate whether the sqFile data structure contains a valid
   HANDLE. If no standard error stream is available for this OS process, the
   sqFile data structure will contain an invalid HANDLE value, which will result
   in failures on subsequent accesses.
 */
...
/*	Answer a two element array containing the sqFile data structure
        representing standard input stream for my OS process, and a flag (true
   or false) to indicate whether the sqFile data structure contains a valid
   HANDLE. If no standard input stream is available for this OS process, the
   sqFile data structure will contain an invalid HANDLE value, which will result
   in failures on subsequent accesses.
 */
...
/*	Answer a two element array containing the sqFile data structure
        representing standard output stream for my OS process, and a flag (true
   or false) to indicate whether the sqFile data structure contains a valid
   HANDLE. If no standard output stream is available for this OS process, the
   sqFile data structure will contain an invalid HANDLE value, which will result
   in failures on subsequent accesses.
 */
...
/*	A character has been read into an external buffer corresponding to
        ...
        of the most
        recent read into a three element array provided by the sender. Contents
   of the array will be status of the read call, character read, and character
   count (which should always be 1).
 */
...
primitiveLastReadForStoreIn(void) {
  ...
  stObjectatput(resultArray, 1,
                positive32BitIntegerFor(readCharStatusArray[index]));
...
/*	Read the next character from anIOHandle (a SQFile struct) into a buffer
   in the VM. When the read completes, signal the specified Semaphore to notify
   that the character is available. Answer the handle of the new thread, or nil
   on error.
 */
...
primitiveNextFromSignaling(void) {
  ...
  hThread = CreateThread(null, 0x400, readCharThread, (&threadArgs),
                         STACK_SIZE_PARAM_IS_A_RESERVATION, (&threadID));

  /* Safety net: Remember the identity of the thread which should be active for
     this semaphore index. */
...
/*	Set up a thread to wait for a process HANDLE to exit, then signal the
        Semaphore at index. This provides asychronous notification of an
   external process exit. Answer the handle of the handler thread.
 */
...
primitiveSetWaitForAnyProcessExitThenSignalSemaphoreWithIndex(void) {
  ...
  /* begin startHandlerThreadForAny:count:signalingSemaphoreAt: */
  waitForThreadMutex(exitThreadMutexHandle());

  /* Set up the thread argument structure. */
  (threadArgs.count = handleCount);
  (threadArgs.handles = handleArrayPtr);
  (threadArgs.semaIndex = semaIndex);
  hThread = CreateThread(null, 0x400, handleAnyChildProc, (&threadArgs),
                         STACK_SIZE_PARAM_IS_A_RESERVATION, (&threadID));
  if (!hThread) {
    threadInformation = nilObject();
    goto l1;
  }
  pushRemappableOop(positive32BitIntegerFor(threadID));
  pushRemappableOop(handleObjectFrom(hThread));
  threadInformationSqInt = instantiateClassindexableSize(classArray(), 2);
  storePointerofObjectwithValue(0, threadInformationSqInt, popRemappableOop());
  storePointerofObjectwithValue(1, threadInformationSqInt, popRemappableOop());
  threadInformation = threadInformationSqInt;
  /* end startHandlerThreadForAny:count:signalingSemaphoreAt: */
l1:
...
/*	Kill the thread. No cleanup is performed, so use with caution for a
   thread which is (for example) manipulating a mutex. Answer true for success,
   else false.
 */
...
/*	This is the thread procedure for reading one character from a file or
   pipe handle. The values of index and handle have been passed on the local
        thread stack, so they are now local to the thread which is executing
   this method. Once the values are safely on a local thread stack, the mutex
...
static DWORD WINAPI readCharThreadIndexhandle(sqInt index, HANDLE handle) {
  ...
  status = ReadFile(handle, &readCharBufferArray[index], 1,
                    &readCharCountArray[index], NULL);
...
/*	This is the entry point for the thread procedure for reading one
   character from a file or pipe handle. There may be many instances of this
   thread procedure, in which case each thread may be distinguished by the index
   value with which it is called. Remember the index value by saving it in a
   variable on the thread stack. Read one character, and signal a Smalltalk
...
static DWORD stdErrHandle(void) { return STD_ERROR_HANDLE; }

/*	Answer the pseudo HANDLE for standard input. */

/* Win32OSProcessPlugin>>#stdInHandle */
static DWORD stdInHandle(void) { return STD_INPUT_HANDLE; }

/*	Answer the pseudo HANDLE for standard output. */

/* Win32OSProcessPlugin>>#stdOutHandle */
static DWORD stdOutHandle(void) { return STD_OUTPUT_HANDLE; }
...
void *Win32OSProcessPlugin_exports[][3] = {
    ...
    {(void *)_m, "primitiveAllocConsole\000\377\000",
     (void *)primitiveAllocConsole},
    {(void *)_m, "primitiveBufferValuesAt\000\000\000",
     (void *)primitiveBufferValuesAt},
    {(void *)_m, "primitiveCanAccessChildProcess\000\001\000",
     (void *)primitiveCanAccessChildProcess},
    {(void *)_m, "primitiveCloseHandle\000\001\000",
     (void *)primitiveCloseHandle},
    {(void *)_m, "primitiveCommand\000\002\000", (void *)primitiveCommand},
    {(void *)_m, "primitiveCommandWithInputOutputError\000\002\000",
     (void *)primitiveCommandWithInputOutputError},
    ...
    {(void *)_m, "primitiveFreeConsole\000\377\000",
     (void *)primitiveFreeConsole},
    ...
    {(void *)_m, "primitiveGetEnvironmentStrings\000\377\000",
     (void *)primitiveGetEnvironmentStrings},
    {(void *)_m, "primitiveGetEnvironmentStringsAsBytes\000\377\000",
     (void *)primitiveGetEnvironmentStringsAsBytes},
    {(void *)_m, "primitiveGetExitStatusForHandle\000\001\000",
     (void *)primitiveGetExitStatusForHandle},
    {(void *)_m, "primitiveGetMainThreadHandle\000\377\000",
     (void *)primitiveGetMainThreadHandle},
    {(void *)_m, "primitiveGetMainThreadID\000\377\000",
     (void *)primitiveGetMainThreadID},
    {(void *)_m, "primitiveGetPid\000\377\000", (void *)primitiveGetPid},
    {(void *)_m, "primitiveGetPidHandle\000\377\000",
     (void *)primitiveGetPidHandle},
    {(void *)_m, "primitiveGetSession\000\377\000",
     (void *)primitiveGetSession},
    {(void *)_m, "primitiveGetStdError\000\001\000",
     (void *)primitiveGetStdError},
    {(void *)_m, "primitiveGetStdInput\000\001\000",
     (void *)primitiveGetStdInput},
    {(void *)_m, "primitiveGetStdOutput\000\001\000",
     (void *)primitiveGetStdOutput},
    {(void *)_m, "primitiveLastReadFor\000\000\000",
     (void *)primitiveLastReadFor},
    {(void *)_m, "primitiveLastReadForStoreIn\000\001\000",
     (void *)primitiveLastReadForStoreIn},
    ...
    {(void *)_m, "primitiveNextFromSignaling\000\001\000",
     (void *)primitiveNextFromSignaling},
    ...
    {(void *)_m,
     "primitiveSetWaitForAnyProcessExitThenSignalSemaphoreWithIndex\000\001"
     "\000",
     (void *)primitiveSetWaitForAnyProcessExitThenSignalSemaphoreWithIndex},
    ...
    {(void *)_m, "primitiveTerminateThread\000\001\000",
     (void *)primitiveTerminateThread},
...
EXPORT(signed short)
primitiveSetWaitForAnyProcessExitThenSignalSemaphoreWithIndexMetadata = 0x100;
