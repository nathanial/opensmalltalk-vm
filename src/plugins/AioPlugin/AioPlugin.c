/* Automatically generated by
   ...
   d9ef6856-42d8-4074-bde7-82f1e1d41282 (Compiler-eem.512) from UnixAioPlugin
   VMConstruction-Plugins-AioPlugin.oscog-eem.26 uuid:
   9d32c4ef-9646-4ce2-b069-44fe86a2dbfb
 */
#if defined(__clang__) || defined(__GNUC__)
static const char __buildInfo[] __attribute__((unused)) =
    "UnixAioPlugin VMConstruction-Plugins-AioPlugin.oscog-eem.26 uuid: "
    "9d32c4ef-9646-4ce2-b069-44fe86a2dbfb " __DATE__;
#else
...
static const char *moduleName =
    "AioPlugin VMConstruction-Plugins-AioPlugin.oscog-eem.26 " INT_EXT;

/*** Macros ***/
#define sessionIdentifierFromSqFile(sqFile)                                    \
  (SESSIONIDENTIFIERTYPE)(((SQFile *)(sqFile))->sessionID)
...
/*	Answer the OS file descriptor, an integer value, from a SQFile data
        structure byte array, or answer -1 if unable to obtain the file
   descriptor (probably due to receiving an incorrect type of object as
   aFileHandle).
 */
...
/*	Enable asynchronous notification for a descriptor. The first parameter
   is an OS level integer file descriptor. The second parameter is the index of
   a Semaphore to be notified, and the third parameter is a flag indicating that
   descriptor represents an external object and should not be closed on
   termination of aio handling. Answer the semaphore index.
 */
...
primitiveAioEnable(void) {
  ...
  /* The following is to prevent the need to coerce integers into pointers in
   * clientData */
  eventSemaphoreIndices[semaIndex] = semaIndex;
  flags = (externalNotification ? AIO_EXT : 0);
...
/*	Temporarily suspend asynchronous event notification for a descriptor.
   The first parameter is an OS level integer file descriptor. The remaining
        three parameters
        are Boolean flags representing the types of events for which
   notification is being requested: handle exceptions, handle for read, and
   handle for write. Flags are defined in the aio.h source as: AIO_X	(1<<0)
   handle for exceptions AIO_R	(1<<1)	handle for read AIO_W	(1<<2)	handle
   for write */
...
/*	Answer the OS file descriptor, an integer value, from a SQSocket data
        structure, or answer -1 if unable to obtain the file descriptor
   (probably due to receiving an incorrect type of object as aFileHandle).
...
/*	Answer a string containing the version string for this plugin. Handle
   MNU errors, which can occur if class InterpreterPlugin has been removed from
        the system.
...
void *AioPlugin_exports[][3] = {
    ...
    {(void *)_m, "primitiveAioDisable\000\000\000",
     (void *)primitiveAioDisable},
    ...
    {(void *)_m, "primitiveAioSuspend\000\000\000",
     (void *)primitiveAioSuspend},
    {(void *)_m, "primitiveModuleName\000\377\000",
     (void *)primitiveModuleName},
    {(void *)_m, "primitiveOSFileHandle\000\001\000",
     (void *)primitiveOSFileHandle},
    {(void *)_m, "primitiveOSSocketHandle\000\001\000",
     (void *)primitiveOSSocketHandle},
