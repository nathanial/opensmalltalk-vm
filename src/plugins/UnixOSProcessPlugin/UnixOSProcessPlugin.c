/* Automatically generated by
   ...
   1a32964f-d425-4ed5-b830-6e8ce3d98688
        (* Compiler-eem.518)
   from
        UnixOSProcessPlugin VMConstruction-Plugins-OSProcessPlugin.oscog-eem.82
   uuid: 1510486b-819a-4643-921a-1e39553cc4a8
 */
#if defined(__clang__) || defined(__GNUC__)
static const char __buildInfo[] __attribute__((unused)) =
    "UnixOSProcessPlugin VMConstruction-Plugins-OSProcessPlugin.oscog-eem.82 "
    "uuid: 1510486b-819a-4643-921a-1e39553cc4a8 " __DATE__;
#else
...
/*** Constants ***/
#define FILEHANDLETYPE                                                         \
  FILE * /* the type of low level stream to be used in a struct SQFile */
#define SESSIONIDENTIFIERTYPE int

/*** Function Prototypes ***/
static void *callocWrappersize(sqInt count, sqInt objectSize);
static sqInt createPipeForReaderwriter(FILEHANDLETYPE *readerIOStreamPtr,
...
static char **fixPointersInArrayOfStringswithOffsets(sqInt flattenedStrings,
                                                     sqInt offsets);
...
static sqInt makePipeForReaderwriter(FILEHANDLETYPE *readerIOStreamPtr,
                                     FILEHANDLETYPE *writerIOStreamPtr);
...
static void *setSignalNumberhandler(sqInt signalNumber,
...
static const char *moduleName =
    "UnixOSProcessPlugin "
    "VMConstruction-Plugins-OSProcessPlugin.oscog-eem.82 " INT_EXT;
...
/*	Using malloc() and calloc() is something I would like to avoid, since it
        is likely to cause problems some time in the future if somebody
   redesigns object memory allocation. This wrapper just makes it easy to find
   senders of calloc() in my code. -dtl */
...
/*	Dup a file descriptor to allow it to be attached as the standard error
        ...
        file descriptor
        0 is stdin, 1 is stdout, and 2 is stderr. The dup2() call is used to
   copy the open file descriptors into file descriptors 0, 1 and 2 so that the
   image which we execute will use them as stdin, stdout, and stderr.
 */
...
static void dupToStdErr(sqInt anSQFileDataStructure) {
  ...
  if (!(/* isSQFileObject: */
        (((isBytes(anSQFileDataStructure)) &&
         ...
         ((getThisSessionID()) == (sessionIdentifierFromSqFile(
                                      arrayValueOf(anSQFileDataStructure))))) &&
        (isNonNullSQFile(anSQFileDataStructure)))) {
    filenoToDup = -1;
    goto l1;
  }
  filenoToDup = ((int)(fileno(fileHandleFrom(anSQFileDataStructure))));
  /* end fileDescriptorFrom: */
l1:
  if (!(filenoToDup < 0)) {
    if (!(filenoToDup == 2)) {
      fflush(stderr);
...
/*	Answer a string containing the OS process environment string at index
...
static sqInt environmentAtAsType(sqInt classIdentifier) {
  ...
  if (!(failed())) {
    if ((p = getEnvironmentVector())) {
      q = p;
      envCnt = 0;
      while (*q++)
        envCnt++;
      methodReturnValue(
          (((index >= 1) && (index <= envCnt))
...
/*	Fork a child OS process, and do an exec in the child. The parent
   continues on in Smalltalk, and this method answers the pid of the child which
   was created. If useSignalHandler is true, set the signal handler for SIGCHLD.
        Otherwise, assume
   ...
   is allocated in the image prior to calling this primitive. This allows us to
        avoid invoking the
        ...
        environment and
        argument vectors, but I think it is a good idea to avoid malloc as much
   as possible so as not to limit future ObjectMemory implementations.

        This primitive replaces #primitiveForkAndExec from earlier versions of
   the plugin. The new name permits backward compatibility for an image running
        on a VM
        ...
        all file descriptors (including sockets) not required (that is,
   everything except stdin, stdout, and stderr on descriptors 0, 1 and 2). This
   eliminates some flakey behavior in child processes connected to Squeak by
   pipes, which failed to exit at expected times due to the old file descriptors
   remaining open. This is also cleaner in that garbage descriptors are not left
   hanging around the the child.
...
static sqInt forkAndExecInDirectory(sqInt useSignalHandler) {
  ...
  /* Turn off the interval timer. If this is not done, then the program which we
     exec in the child process will receive a timer interrupt, and will not know
     how to handle it. */
  ...
  /* Set up the environment, a C array of pointers to C strings, where each
     string is of the form 'KEY=value'. Refer to previously allocated object
     memory rather than using a call to malloc(). */
  if (envVecBuffer == (nilObject())) {
    env = envVec;
  } else {
    env = ((char **)(fixPointersInArrayOfStringswithOffsets(envVecBuffer,
                                                            envOffsets)));
  }

  /* Set up the argument array. Allocate space for a C array of pointers to
     strings. Refer to previously allocated object memory rather than using a
     call to malloc(). */
  args = ((char **)(fixPointersInArrayOfStringswithOffsets(argVecBuffer,
                                                           argOffsets)));
  if ((env == 0) || (args == 0)) {
  ...
  /* Clean things up before clobbering the running image.
     Note: If any file descriptors, signal handlers, or other references to
     external resources need to be cleaned up, do it here. */
...
/*	Fork a child process, and continue running squeak in the child process.
        ...
        After calling fork(), two OS processes exist, one of which is the child
   of the other. On systems which implement copy-on-write memory management, and
   which support the fork() system call, both processes will be running
   Smalltalk images, and will be sharing the same memory space. In the original
   OS process, the resulting value of pid is the process id of the child process
   (a non-zero integer). In the child process, the value of pid is zero.

        The child recreates sufficient external resources to continue running.
        This is done by
        attaching to a new X session. The child is otherwise a copy of the
   parent process, and will continue executing the Smalltalk image at the same
   point as its parent. The return value of this primitive may be used by the
   two running Smalltalk images to determine which is the parent and which is
   the child.
   ...
   allowed to save the image to the file system, otherwise one Smalltalk may
   overwrite the image of the other.

        This is a simple call to fork(), rather than the more common idiom of
        vfork() followed
        by exec(). The vfork() call cannot be used here because it is designed
   to be followed by an exec(), and its semantics require the parent process to
   wait for the child to exit. See the BSD programmers documentation for
   details.
 */
...
/*	Set a signal handler in the VM which will signal a Smalltalk semaphore
   at semaphoreIndex whenever an external signal sigNum is received. Answer the
        prior value of the signal handler. If semaphoreIndex is zero, the
   handler is unregistered, and the VM returns to its default behavior for
   handling that signal. A handler must be unregistered before it can be
   registered again. The Smalltalk semaphore is expected to be kept at the same
   index location indefinitely during the lifetime of a Squeak session. If that
   is not the case, the handler must be unregistered prior to unregistering the
   Smalltalk semaphore.
 */
...
static void *forwardSignaltoSemaphoreAt(sqInt sigNum, sqInt semaphoreIndex) {
  ...
  oldHandler = setSignalNumberhandler(
      sigNum, handleSignal /* handleSignalFunctionAddress */);
...
/*	Answer a pointer to the environment vector. If the main module has
        exported a function called getEnvVec, call that function. Otherwise try
   to use a global reference to the variable envVec, which is expected to be
   declared in the main VM, and may or may not be visible depending on platform
   and compiler conventions. The value of the environment vector is cached in a
   static variable, so this method should not be inlined.
 */
...
static sqInt getStdHandle(sqInt n) {
  ...
  /* In the simulator file handles are integer indexes of FilePluginSimulator's
   * openFiles dictionary. */
...
/*	Create a pipe and populate the readerIOStream and writerIOStream
        variables. Make sure the SIGPIPE handler is set before creating the
   pipe. Answer true for success, else false
 */
...
static sqInt makePipeForReaderwriter(FILEHANDLETYPE *readerIOStreamPtr,
  ...
  setSignalNumberhandler(SIGPIPE /* sigPipeNumber */,
                         SIG_IGN /* sigIgnoreNumber */);
  if ((pipe(filedes)) == -1) {
    return 0;
  } else {
    *writerIOStreamPtr = (FILE *)fdopen(filedes[1], "a");
...
/*	Answer if the plugin should use a signalstack to avoid delivering
   signals on the native stack, which can interfere with the JIT. As a
   side-effect, allocate one if one is not already available. We use
   GetAttributeString to detect the JIT, and if so detected we use a
   signalstack, We reuse any existing signalstack if available. */
...
static sqInt needSigaltstack(void) {
  ...
  if ((GetAttributeString == null) || ((GetAttributeString(0x3F0)) == null)) {
    useSignalStack = 0;
    return 0;
  }

  /* 1008 is the Cogit buildInfo string */
  useSignalStack = 1;
  ...
  SigStackSize = ((((0x400 * (sizeof(void *))) * 16) < MINSIGSTKSZ)
                      ? MINSIGSTKSZ
                      : ((0x400 * (sizeof(void *))) * 16));
...
static void **originalSignalHandlers(void) { return originalSigHandlers; }
...
/*	Send a null signal to the OS process identified by the argument. Answer
        false for
        a bad parameter on the stack (the common case is for the argument equal
   to nil, for which case we should answer false). Answer true if the process
        exists and can
...
/*	Answer the string result of confstr() for the supplied option index, an
        integer value defined in unistd.h. On some platforms, the available
...
/*	Create a pipe, and answer an array of two file handles for the pipe
   writer and reader. The readerIOStream and writerIOStream variables represent
   the low level pipe streams, which will be of type (FILE *) or HANDLE,
   depending on what the FilePlugin support code is using to represent file
   streams. FILEHANDLETYPE is defined in my subclasses in the #declareCVarsIn:
   class method.
 */
...
/*	Create a pipe, and answer an array of two file handles for the pipe
   writer and reader. The session identifier is passed as the parameter to this
   primitive. Use this variant if the session identifier is not available
   directly in the VM (as may be the case if it is not possible to link from
   this plugin to a variable elsewhere in the VM). The readerIOStream and
   writerIOStream variables represent the low level pipe streams, which will be
   of type (FILE *) or HANDLE, depending on what the FilePlugin support code is
   using to represent file streams. FILEHANDLETYPE is defined in my subclasses
...
/*	Answer a byte array containing the OS process environment string at
   index (an Integer) in the environment list. */
...
/*	Call stat(2) to obtain the file protection mask for a file. Answer an
   ...
   octal digit (0-7), with bit values 4, 2, and 1. The first digit selects the
   set user ID (4) and set group ID (2) and save text image (1) attributes. The
   second digit selects permissions for the user who owns the file: read (4),
   write (2), and execute (1); the third selects permissions for other users in
   the file's group, with the same values; and the fourth for other users not in
...
/*	Call stat(2) to obtain the file protection mask for a file. Answer errno
   ...
   first digit selects the set user ID (4) and set group ID (2) and save text
   image (1) attributes. The second digit selects permissions for the user who
   owns the file: read (4), write (2), and execute (1); the third selects
   permissions for other users in the file's group, with the same values; and
...
/*	This primitive exists only for purposes of testing the
        fixPointersInArrayOfStrings:withOffsets:count: method. I believe it to
   be reasonably machine and compiler independent, but have no way of verifying
        this on a variety of machines, so I'll leave this test method here in
   case someone runs into problems on other hardware or compilers. -dtl */
...
primitiveFixPointersInArrayOfStrings(void) {
  ...
  if ((failed()) || ((fixPointersInArrayOfStringswithOffsets(
                         cStringArray, offsetArray)) == 0)) {
    primitiveFail();
  } else {
...
EXPORT(sqInt)
primitiveForkAndExecInDirectory(void) { return forkAndExecInDirectory(1); }
...
EXPORT(sqInt)
primitiveForkExec(void) { return forkAndExecInDirectory(0); }
...
/*	Set a signal handler in the VM which will signal a Smalltalk semaphore
   ...
   the case, the handler must be unregistered prior to unregistering the
   Smalltalk semaphore.
 */
...
primitiveForwardSignalToSemaphore(void) {
  ...
  if (index == (nilObject())) {
    ...
    semaphoreIndex = integerValueOf(index);
  }
  ...
  priorHandlerObject =
      instantiateClassindexableSize(classByteArray(), sizeof(void *));
...
primitiveFpathconf(void) {
  ...
  if (!(/* isSQFileObject: */
        (((isBytes(aSQFileByteArray)) &&
         ...
         ((getThisSessionID()) ==
          (sessionIdentifierFromSqFile(arrayValueOf(aSQFileByteArray))))) &&
...
primitiveGetPid(void) {
  /* see the various implementations of canAccessSystem */
...
/*	Answer the file handle for standard error of my OS process. The session
        identifier is passed as the parameter to this primitive. Use this
   variant if the session identifier is not available directly in the VM (as may
   be the case if it is not possible to link from this plugin to a variable
        elsewhere in the VM). */
...
/*	Answer the ID of the pthread that is currently executing (the
   interpreter thread). A thread ID may be a 64 bit value on some platforms, so
   answer a byte array containing the value in machine-dependent byte order.
 */
...
/*	Take a struct SQFile from the stack, and call feof(3) to determine if
   the file has reached end of file.
 */
...
primitiveIsAtEndOfFile(void) {
  ...
  if (!(/* isSQFileObject: */
        (((isBytes(sqFileOop)) &&
         ...
         ((getThisSessionID()) ==
          (sessionIdentifierFromSqFile(arrayValueOf(sqFileOop))))) &&
...
/*	Take a struct SQFile from the stack, and request a lock on the specified
        region. If the exclusive flag is true, then request an exclusive
   (F_WRLCK) lock on the file. Otherwise, request a shared (F_RDLCK) lock. Any
   number of Unix processes may hold a read lock (shared lock) on a file region,
   but only one process may hold a write lock (exclusive lock). Answer the
   result of the call to fcntl(). If length is zero, then the entire file will
   be locked, including region extents that have not yet been allocated for the
   file. */
...
/*	Change the scheduling priority of this process by the given nice
   ...
   superuser can specify a negative value (to increase the priority). See man(2)
   nice.
...
EXPORT(sqInt)
primitiveRealpath(void) { return realpathAsType(classString()); }

/*	Answer a byte array with the real path for a path string as determined
   by realpath().
 */

/* UnixOSProcessPlugin>>#primitiveRealpathAsBytes */
EXPORT(sqInt)
primitiveRealpathAsBytes(void) { return realpathAsType(classByteArray()); }

/*	Clean up after the death of a child, and answer an array with the pid
   and the exit status of the child process. Answer nil if the pidToHandle does
        not exist.
 */
...
/*	Send SIGABRT (abort) to the OS process identified by the argument. Use
...
/*	Send SIGCHLD (child status has changed, usually death of child) to the
   OS process identified by the argument. Use an explicit check for
        isIntegerObject so we can
        return -1 on error (the stackIntegerValue: method answers 1 on error,
   and 1 is a valid pid number).
 */
...
/*	Send SIGCONT (continue) to the OS process identified by the argument.
   Use an explicit check for isIntegerObject so we can return -1 on error (the
        stackIntegerValue: method
        answers 1 on error, and 1 is a valid pid number).
 */
...
/*	Send SIGHUP (hangup) to the OS process identified by the argument. Use
   an explicit check for isIntegerObject so we can return -1 on error (the
        stackIntegerValue: method
        answers 1 on error, and 1 is a valid pid number).
 */
...
/*	Send SIGINT (interrupt) to the OS process identified by the argument.
   Use an explicit check for isIntegerObject so we can return -1 on error (the
        stackIntegerValue: method
        answers 1 on error, and 1 is a valid pid number).
 */
...
/*	Set the process group ID of the process identified by pid to a new
   process group ID. */
...
/*	Set a new process group for this OS process. Newly created child
   processes will be members of the new process group. Note: Use setpgid(0,0)
   rather than the equivalent setpgrp() because setpgrp() is implemented
   differently on some flavors of Unix. */
...
/*	Set the index of the semaphore used by the OSProcess with which I
   ...
   SIGCHLD signals (death of child). In the C translation this is a static int
   which would be shared by all instances of UnixOSProcessPlugin, which is
   expected to be a singleton. Answer the value of the semaphore index.
 */
...
/*	Quoted from Linux man pages:
        ...
        calling process will be the only process in this new process group and
   in this new session. */
...
/*	Given a file size requirement in bytes, and a path specifying a location
        within a file system, answer true if that file system has sufficient
   free space to save a file of that size. The file size parameter must be a
        SmallInteger (not LargePositiveInteger) and the directory path is a
        string. This primitive is entended as a guard to protect against saving
   an image file when the file system is full. */
...
primitiveSpaceForByteSizeInDirectoryPath(void) {
  ...
  fileSystemFull =
      (freeBlocks >= requiredBlocks ? trueObject() : falseObject());
...
/*	Take a struct SQFile from the stack, and call fcntl() to set the file
   for blocking I/O. Use this variant if the session identifier is not available
   directly in the VM (as may be the case if it is not possible to link from
   this plugin to a variable elsewhere in the VM).
 */
...
/*	Take a struct SQFile from the stack, and call setbuf() to set the OS
   file stream (implemented in the C library) for unbuffered I/O. Answers the
        result of a fflush()
...
/*	Take a struct SQFile from the stack, and call setbuf() to set the OS
   file stream (implemented in the C library) for unbuffered I/O. Answers the
   result of a fflush() call, not the result of the setbuf() call (which is type
   void). This is nearly useless, but may at least provide an indicator that we
   are operating on a valid file stream. Use this variant if the session
   identifier is not available directly in the VM (as may be the case if it is
   not possible to link from this plugin to a variable elsewhere in the VM).
 */
...
/*	Take a struct SQFile from the stack, and call feof(3) to determine if
   the file has reached end of file. The flag is set only by a previous read
   operation, so end of file is not detected until an actual EOF condition has
   been detected by a read attempt.
 */
...
/*	Take a struct SQFile from the stack, and check for ability to lock the
        specified region.
        If the exclusive flag is true, then specify an exclusive (F_WRLCK) lock
   on the file. Otherwise, specify a shared (F_RDLCK) lock. Any number of Unix
   ...
   preventing this lock request). l_start (offset of the region lock preventing
   this lock request) l_len (length of the region lock preventing this lock
   request) */
...
/*	Take a struct SQFile from the stack, and answer the value of its Unix
   file number. The Unix file number is not directly useful to Squeak, but may
   be interesting for debugging problems involving failure to close unused file
   handles.
 */
...
static sqInt realpathAsType(sqInt classIdentifier) {
  ...
  if (realpathResult) {
    if ((strlen(realpathResult)) >= 0x400) {
      perror("warning: statically allocated array exceeded in "
             "UnixOSProcessPlugin>>primitiveRealPath, object memory may have "
             "been corrupted");
      return primitiveFail();
    }
...
/*	This is a signal handler for SIGCHLD. It is not meant to be called from
        Smalltalk, and should only be called indirectly as a result of a death
   of child signal from the operating system.
   ...
   signal handler to catch the next SIGCHLD signal, then sets a semaphore to
   notify the system that a child process needs to be cleaned up. The actual
   clean up is done by a Smalltalk process which waits on the semaphore, then
   calls primitiveReapChildProcess. Note: If child processes die faster than we
   can clean them up, signals will be lost and child processes will remain as
   zombies.
 */
...
/*	Answer 1 if running in secure mode, else 0. The osprocessSandboxSecurity
        ...
        value to
        either 0 (user has full access to the plugin) or 1 (user is not
   permitted to do dangerous things).
 */
...
/*	If the security plugin can be loaded, use it to check. If not, assume
   it's ok
 */
...
static sqInt securityHeurisitic(void) {
  ...
  return ((canWriteImage && (hasFileAccess)) && (hasSocketAccess) ? 0 : 1);
}

/*	Exit function to be registered with atexit() to signal child processes
   on VM exit.
 */
...
static SESSIONIDENTIFIERTYPE sessionIdentifierFrom(sqInt aByteArray) {
  ...
  union {
    SESSIONIDENTIFIERTYPE session;
    unsigned char bytes[sizeof(SESSIONIDENTIFIERTYPE)];
  } sessionUnion;
...
static void setSigChldHandler(void) {
  ...
  if (needSigaltstack()) {
    (sigchldHandlerAction.sa_flags =
         ((sigchldHandlerAction.sa_flags)) | SA_ONSTACK);
  }
  sigemptyset((&((sigchldHandlerAction.sa_mask))));
  if ((sigaction(SIGCHLD, (&sigchldHandlerAction), 0)) ==
      (((sqInt)SIG_ERR /* sigErrorNumber */))) {
    perror("sigaction");
  }
...
static void *setSignalNumberhandler(sqInt signalNumber,
  ...
  if ((sigaction(signalNumber, (&sigHandlerAction), (&oldHandlerAction))) ==
      (((sqInt)SIG_ERR /* sigErrorNumber */))) {
    perror("signal");
  }
...
/*	Number of possible signals for this OS plus one. The signal handler
   arrays declared in #declareCVarsIn: are this size. */

/* UnixOSProcessPlugin>>#signalArraySize */
static sqInt signalArraySize(void) { return NSIG; }
...
static sqInt sizeOfSession(void) { return sizeof(SESSIONIDENTIFIERTYPE); }
...
void *UnixOSProcessPlugin_exports[][3] = {
    ...
    {(void *)_m, "getCurrentWorkingDirectoryAsType\000\377\000",
     (void *)getCurrentWorkingDirectoryAsType},
    {(void *)_m, "getModuleName", (void *)getModuleName},
    {(void *)_m, "initialiseModule", (void *)initialiseModule},
    {(void *)_m, "primitiveArgumentAt\000\377\000",
     (void *)primitiveArgumentAt},
    {(void *)_m, "primitiveArgumentAtAsBytes\000\377\000",
     (void *)primitiveArgumentAtAsBytes},
    {(void *)_m, "primitiveCanReceiveSignals\000\000\000",
     (void *)primitiveCanReceiveSignals},
    {(void *)_m, "primitiveChdir\000\001\000", (void *)primitiveChdir},
    {(void *)_m, "primitiveConfstr\000\000\000", (void *)primitiveConfstr},
    {(void *)_m, "primitiveCreatePipe\000\377\000",
     (void *)primitiveCreatePipe},
    {(void *)_m, "primitiveCreatePipeWithSessionIdentifier\000\001\000",
     (void *)primitiveCreatePipeWithSessionIdentifier},
    {(void *)_m, "primitiveDup\000\000\000", (void *)primitiveDup},
    {(void *)_m, "primitiveDupTo\000\000\000", (void *)primitiveDupTo},
    {(void *)_m, "primitiveEnvironmentAt\000\377\000",
     (void *)primitiveEnvironmentAt},
    {(void *)_m, "primitiveEnvironmentAtAsBytes\000\377\000",
     (void *)primitiveEnvironmentAtAsBytes},
    {(void *)_m, "primitiveEnvironmentAtSymbol\000\377\000",
     (void *)primitiveEnvironmentAtSymbol},
    {(void *)_m, "primitiveEnvironmentAtSymbolAsBytes\000\377\000",
     (void *)primitiveEnvironmentAtSymbolAsBytes},
    {(void *)_m, "primitiveErrorMessageAt\000\000\000",
     (void *)primitiveErrorMessageAt},
    {(void *)_m, "primitiveFileProtectionMask\000\001\000",
     (void *)primitiveFileProtectionMask},
    {(void *)_m, "primitiveFileStat\000\001\000", (void *)primitiveFileStat},
    {(void *)_m, "primitiveFixPointersInArrayOfStrings\000\001\000",
     (void *)primitiveFixPointersInArrayOfStrings},
    {(void *)_m, "primitiveForkAndExecInDirectory\000\377\000",
     (void *)primitiveForkAndExecInDirectory},
    {(void *)_m, "primitiveForkExec\000\377\000", (void *)primitiveForkExec},
    {(void *)_m, "primitiveForkSqueak\000\377\000",
     (void *)primitiveForkSqueak},
    {(void *)_m, "primitiveForkSqueakWithoutSigHandler\000\377\000",
     (void *)primitiveForkSqueakWithoutSigHandler},
    {(void *)_m, "primitiveForwardSignalToSemaphore\000\000\000",
     (void *)primitiveForwardSignalToSemaphore},
    {(void *)_m, "primitiveFpathconf\000\001\000", (void *)primitiveFpathconf},
    {(void *)_m, "primitiveGetCurrentWorkingDirectory\000\377\000",
     (void *)primitiveGetCurrentWorkingDirectory},
    {(void *)_m, "primitiveGetCurrentWorkingDirectoryAsBytes\000\377\000",
     (void *)primitiveGetCurrentWorkingDirectoryAsBytes},
    ...
    {(void *)_m, "primitiveGetSession\000\377\000",
     (void *)primitiveGetSession},
    {(void *)_m, "primitiveGetStdErrHandle\000\377\000",
     (void *)primitiveGetStdErrHandle},
    {(void *)_m, "primitiveGetStdErrHandleWithSessionIdentifier\000\001\000",
     (void *)primitiveGetStdErrHandleWithSessionIdentifier},
    {(void *)_m, "primitiveGetStdInHandle\000\377\000",
     (void *)primitiveGetStdInHandle},
    {(void *)_m, "primitiveGetStdInHandleWithSessionIdentifier\000\001\000",
     (void *)primitiveGetStdInHandleWithSessionIdentifier},
    {(void *)_m, "primitiveGetStdOutHandle\000\377\000",
     (void *)primitiveGetStdOutHandle},
    {(void *)_m, "primitiveGetStdOutHandleWithSessionIdentifier\000\001\000",
     (void *)primitiveGetStdOutHandleWithSessionIdentifier},
    {(void *)_m, "primitiveGetThreadID\000\377\000",
     (void *)primitiveGetThreadID},
    {(void *)_m, "primitiveGetUid\000\377\000", (void *)primitiveGetUid},
    {(void *)_m, "primitiveIsAtEndOfFile\000\001\000",
     (void *)primitiveIsAtEndOfFile},
    {(void *)_m, "primitiveKillOnExit\000\001\000",
     (void *)primitiveKillOnExit},
    {(void *)_m, "primitiveLockFileRegion\000\001\000",
     (void *)primitiveLockFileRegion},
    {(void *)_m, "primitiveMakePipe\000\377\000", (void *)primitiveMakePipe},
    {(void *)_m, "primitiveMakePipeWithSessionIdentifier\000\001\000",
     (void *)primitiveMakePipeWithSessionIdentifier},
    {(void *)_m, "primitiveModuleName\000\377\000",
     (void *)primitiveModuleName},
    ...
    {(void *)_m, "primitiveRealpathAsBytes\000\377\000",
     (void *)primitiveRealpathAsBytes},
    {(void *)_m, "primitiveReapChildProcess\000\000\000",
     (void *)primitiveReapChildProcess},
    {(void *)_m, "primitiveSemaIndexFor\000\000\000",
     (void *)primitiveSemaIndexFor},
    {(void *)_m, "primitiveSendSigabrtTo\000\000\000",
     (void *)primitiveSendSigabrtTo},
    {(void *)_m, "primitiveSendSigalrmTo\000\000\000",
     (void *)primitiveSendSigalrmTo},
    {(void *)_m, "primitiveSendSigchldTo\000\000\000",
     (void *)primitiveSendSigchldTo},
    {(void *)_m, "primitiveSendSigcontTo\000\000\000",
     (void *)primitiveSendSigcontTo},
    {(void *)_m, "primitiveSendSighupTo\000\000\000",
     (void *)primitiveSendSighupTo},
    {(void *)_m, "primitiveSendSigintTo\000\000\000",
     (void *)primitiveSendSigintTo},
    {(void *)_m, "primitiveSendSigkillTo\000\000\000",
     (void *)primitiveSendSigkillTo},
    {(void *)_m, "primitiveSendSigpipeTo\000\000\000",
     (void *)primitiveSendSigpipeTo},
    {(void *)_m, "primitiveSendSigquitTo\000\000\000",
     (void *)primitiveSendSigquitTo},
    {(void *)_m, "primitiveSendSigstopTo\000\000\000",
     (void *)primitiveSendSigstopTo},
    {(void *)_m, "primitiveSendSigtermTo\000\000\000",
     (void *)primitiveSendSigtermTo},
    {(void *)_m, "primitiveSendSigusr1To\000\000\000",
     (void *)primitiveSendSigusr1To},
    {(void *)_m, "primitiveSendSigusr2To\000\000\000",
     (void *)primitiveSendSigusr2To},
    {(void *)_m, "primitiveSetPGid\000\000\000", (void *)primitiveSetPGid},
    {(void *)_m, "primitiveSetPGrp\000\377\000", (void *)primitiveSetPGrp},
    {(void *)_m, "primitiveSetSemaIndex\000\000\000",
     (void *)primitiveSetSemaIndex},
    {(void *)_m, "primitiveSetSid\000\377\000", (void *)primitiveSetSid},
    {(void *)_m, "primitiveSigChldNumber\000\377\000",
     (void *)primitiveSigChldNumber},
    {(void *)_m, "primitiveSigHupNumber\000\377\000",
     (void *)primitiveSigHupNumber},
    {(void *)_m, "primitiveSigIntNumber\000\377\000",
     (void *)primitiveSigIntNumber},
    {(void *)_m, "primitiveSigKillNumber\000\377\000",
     (void *)primitiveSigKillNumber},
    {(void *)_m, "primitiveSigPipeNumber\000\377\000",
     (void *)primitiveSigPipeNumber},
    {(void *)_m, "primitiveSigQuitNumber\000\377\000",
     (void *)primitiveSigQuitNumber},
    {(void *)_m, "primitiveSigTermNumber\000\377\000",
     (void *)primitiveSigTermNumber},
    {(void *)_m, "primitiveSigUsr1Number\000\377\000",
     (void *)primitiveSigUsr1Number},
    {(void *)_m, "primitiveSigUsr2Number\000\377\000",
     (void *)primitiveSigUsr2Number},
    {(void *)_m, "primitiveSizeOfInt\000\377\000", (void *)primitiveSizeOfInt},
    {(void *)_m, "primitiveSizeOfPointer\000\377\000",
     (void *)primitiveSizeOfPointer},
    {(void *)_m, "primitiveSpaceForByteSizeInDirectoryPath\000\002\000",
     (void *)primitiveSpaceForByteSizeInDirectoryPath},
    {(void *)_m, "primitiveSQFileFlush\000\001\000",
     (void *)primitiveSQFileFlush},
    {(void *)_m, "primitiveSQFileFlushWithSessionIdentifier\000\001\000",
     (void *)primitiveSQFileFlushWithSessionIdentifier},
    {(void *)_m, "primitiveSQFileSetBlocking\000\001\000",
     (void *)primitiveSQFileSetBlocking},
    {(void *)_m, "primitiveSQFileSetBlockingWithSessionIdentifier\000\001\000",
     (void *)primitiveSQFileSetBlockingWithSessionIdentifier},
    {(void *)_m, "primitiveSQFileSetNonBlocking\000\001\000",
     (void *)primitiveSQFileSetNonBlocking},
    {(void *)_m,
     "primitiveSQFileSetNonBlockingWithSessionIdentifier\000\001\000",
     (void *)primitiveSQFileSetNonBlockingWithSessionIdentifier},
    {(void *)_m, "primitiveSQFileSetUnbuffered\000\001\000",
     (void *)primitiveSQFileSetUnbuffered},
    {(void *)_m,
     "primitiveSQFileSetUnbufferedWithSessionIdentifier\000\001\000",
     (void *)primitiveSQFileSetUnbufferedWithSessionIdentifier},
    {(void *)_m, "primitiveSysconf\000\000\000", (void *)primitiveSysconf},
    {(void *)_m, "primitiveTestEndOfFileFlag\000\001\000",
     (void *)primitiveTestEndOfFileFlag},
    {(void *)_m, "primitiveTestLockableFileRegion\000\001\000",
     (void *)primitiveTestLockableFileRegion},
    {(void *)_m, "primitiveUnixFileClose\000\000\000",
     (void *)primitiveUnixFileClose},
    {(void *)_m, "primitiveUnixFileNumber\000\001\000",
     (void *)primitiveUnixFileNumber},
    {(void *)_m, "primitiveUnlockFileRegion\000\001\000",
     (void *)primitiveUnlockFileRegion},
...
EXPORT(signed short)
primitiveGetStdErrHandleWithSessionIdentifierMetadata = 0x100;
EXPORT(signed short)
primitiveGetStdInHandleWithSessionIdentifierMetadata = 0x100;
EXPORT(signed short)
primitiveGetStdOutHandleWithSessionIdentifierMetadata = 0x100;
...
EXPORT(signed short)
primitiveSQFileSetBlockingWithSessionIdentifierMetadata = 0x100;
EXPORT(signed short) primitiveSQFileSetNonBlockingMetadata = 0x100;
EXPORT(signed short)
primitiveSQFileSetNonBlockingWithSessionIdentifierMetadata = 0x100;
EXPORT(signed short) primitiveSQFileSetUnbufferedMetadata = 0x100;
EXPORT(signed short)
primitiveSQFileSetUnbufferedWithSessionIdentifierMetadata = 0x100;
