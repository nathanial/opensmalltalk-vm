/* Automatically generated by
        SmartSyntaxPluginCodeGenerator VMMaker.oscog-eem.3562 uuid:
   1c2af8bc-8d18-4b77-b707-0f607d037a18 (Compiler-eem.514) from BochsX64Plugin
   Cog-eem.472 uuid: 26e34df1-754e-4a01-a521-e336c752c003
 */
#if defined(__clang__) || defined(__GNUC__)
static const char __buildInfo[] __attribute__((unused)) =
    "BochsX64Plugin Cog-eem.472 uuid: "
    "26e34df1-754e-4a01-a521-e336c752c003 " __DATE__;
#else
...
static const char *moduleName = "BochsX64Plugin Cog-eem.472 " INT_EXT;
...
primitiveDisassembleAtInMemory(void) {
  ...
  if (!((cpu = ((void *)(/* startOfData: */
                         ((longAt((void *)(cpuAlien + BaseHeaderSize))) > 0
                              ...
                              : longAt((void *)((cpuAlien + BaseHeaderSize) +
                                                BytesPerOop)))))))) {
  ...
  instrLenOrErr = disassembleForAtInSizePrintAddress(
      cpu, address, memory - memoryOffset,
      (byteSizeOf(((sqInt)(sqIntptr_t)(memory)-BaseHeaderSize))) + memoryOffset,
      1);
  if (instrLenOrErr < 0) {
...
/*	<Integer> */
/*	<ByteArray|ShortArray|WordArray|DoubleWordArray> */
/*	Return an Array of the instruction length, its decompilation as a
   string, and its address for the instruction at address in memory. Don't
   prefix the disassembly with the instruction's address. */
...
/*	Answer an IntegerArray of the processor's integer register state, ending
        ...
        included in the register state, the pc and the flags (if the processor
   has flags). This primitive is unnecessary; it exists only to speed up single
        stepping.
 */
...
primitiveIntegerRegisterState(void) {
  ...
  registerStateVector =
      instantiateClassindexableSize(dataClass, NumIntegerRegisterStateFields);
  if (!registerStateVector) {
    return primitiveFailFor(PrimErrNoMemory);
  }
  if ((byteSizeOf(registerStateVector)) !=
      (NumIntegerRegisterStateFields *
       (sizeof(sqLong /* integerRegisterElementType */)))) {
    return primitiveFailFor(PrimErrInternalError);
  }
  storeIntegerRegisterStateOfinto(cpu,
                                  firstIndexableField(registerStateVector));
  if (!(failed())) {
...
/*	Get or set the memory offset. If non-zero the memory offset is one word
   (4 or 8 bytes), to marry with the SuprMemoryManager simulators discarding of
   the first word of memory as a null pointer trap. */
...
primitiveRunInMemoryMinAddressMaxAddressReadWrite(void) {
  ...
  /* Add forceStopOnInterrupt to the interrupt check chain.  It is our
     responsibility to chain calls, hence we remember any previous client in
  ...
  maybeErr = runCPUInSizeMinAddressReadWrite(
      cpu, memory - memoryOffset,
      (((memorySize + memoryOffset) < maxAddress) ? (memorySize + memoryOffset)
                                                  : maxAddress),
      minAddress, minWriteMaxExecAddress);
...
primitiveRunInMemoryMinimumAddressReadWrite(void) {
  ...
  if ((minAddress < memoryOffset) || (minWriteMaxExecAddress < memoryOffset)) {
    return primitiveFailFor(PrimErrBadArgument);
  }
  prevInterruptCheckChain = setInterruptCheckChain(forceStopOnInterrupt);
  if (prevInterruptCheckChain == forceStopOnInterrupt) {
...
primitiveRunInMemoryOffsetMinimumAddressReadWrite(void) {
  ...
  maybeErr = runCPUInSizeMinAddressReadWrite(
      cpu, ((((char *)memory)) - memoryOffset) + offset,
      ((byteSizeOf(((sqInt)(sqIntptr_t)(memory)-BaseHeaderSize))) +
       memoryOffset) -
          offset,
      minAddress, minWriteMaxExecAddress);
  setInterruptCheckChain(prevInterruptCheckChain);
...
primitiveSingleStepInMemoryMinAddressMaxAddressReadWrite(void) {
  ...
  memorySize = byteSizeOf(((sqInt)(sqIntptr_t)(memory)-BaseHeaderSize));
  maybeErr = singleStepCPUInSizeMinAddressReadWrite(
      cpu, memory - memoryOffset,
      (((memorySize + memoryOffset) < maxAddress) ? (memorySize + memoryOffset)
                                                  : maxAddress),
      minAddress, minWriteMaxExecAddress);
...
primitiveSingleStepInMemoryOffsetMinimumAddressReadWrite(void) {
  ...
  maybeErr = singleStepCPUInSizeMinAddressReadWrite(
      cpu, ((((char *)memory)) - memoryOffset) + offset,
      ((byteSizeOf(((sqInt)(sqIntptr_t)(memory)-BaseHeaderSize))) - offset) +
          memoryOffset,
      minAddress, minWriteMaxExecAddress);
  if (maybeErr) {
    return primitiveFailForOSError(maybeErr);
...
void *BochsX64Plugin_exports[][3] = {
    ...
    {(void *)_m, "primitiveDisassembleAtInMemory\000\001\000",
     (void *)primitiveDisassembleAtInMemory},
    {(void *)_m, "primitiveDisassembleWithAddressAtInMemory\000\001\000",
     (void *)primitiveDisassembleWithAddressAtInMemory},
    {(void *)_m, "primitiveDisassembleWithoutAddressAtInMemory\000\001\000",
     (void *)primitiveDisassembleWithoutAddressAtInMemory},
    {(void *)_m, "primitiveErrorAndLog\000\377\000",
     (void *)primitiveErrorAndLog},
    {(void *)_m, "primitiveFlushICacheFromTo\000\000\000",
     (void *)primitiveFlushICacheFromTo},
    {(void *)_m, "primitiveIntegerRegisterState\000\000\000",
     (void *)primitiveIntegerRegisterState},
    {(void *)_m, "primitiveMemoryOffset\000\000\001",
     (void *)primitiveMemoryOffset},
    {(void *)_m, "primitiveNewCPU\000\377\000", (void *)primitiveNewCPU},
    {(void *)_m, "primitivePrintInstructionBytes\000\000\001",
     (void *)primitivePrintInstructionBytes},
    {(void *)_m, "primitiveResetCPU\000\000\000", (void *)primitiveResetCPU},
    {(void *)_m,
     "primitiveRunInMemoryMinAddressMaxAddressReadWrite\000\001\000",
     (void *)primitiveRunInMemoryMinAddressMaxAddressReadWrite},
    {(void *)_m, "primitiveRunInMemoryMinimumAddressReadWrite\000\001\000",
     (void *)primitiveRunInMemoryMinimumAddressReadWrite},
    {(void *)_m,
     "primitiveRunInMemoryOffsetMinimumAddressReadWrite\000\001\000",
     (void *)primitiveRunInMemoryOffsetMinimumAddressReadWrite},
    {(void *)_m,
     "primitiveSingleStepInMemoryMinAddressMaxAddressReadWrite\000\001\000",
     (void *)primitiveSingleStepInMemoryMinAddressMaxAddressReadWrite},
    {(void *)_m,
     "primitiveSingleStepInMemoryMinimumAddressReadWrite\000\001\000",
     (void *)primitiveSingleStepInMemoryMinimumAddressReadWrite},
    {(void *)_m,
     "primitiveSingleStepInMemoryOffsetMinimumAddressReadWrite\000\001\000",
     (void *)primitiveSingleStepInMemoryOffsetMinimumAddressReadWrite},
...
EXPORT(signed short)
primitiveDisassembleWithoutAddressAtInMemoryMetadata = 0x100;
...
EXPORT(signed short)
primitiveRunInMemoryMinAddressMaxAddressReadWriteMetadata = 0x100;
EXPORT(signed short)
primitiveRunInMemoryMinimumAddressReadWriteMetadata = 0x100;
EXPORT(signed short)
primitiveRunInMemoryOffsetMinimumAddressReadWriteMetadata = 0x100;
EXPORT(signed short)
primitiveSingleStepInMemoryMinAddressMaxAddressReadWriteMetadata = 0x100;
EXPORT(signed short)
primitiveSingleStepInMemoryMinimumAddressReadWriteMetadata = 0x100;
EXPORT(signed short)
primitiveSingleStepInMemoryOffsetMinimumAddressReadWriteMetadata = 0x100;
#endif // SPURVM
