/* Automatically generated by
   ...
   d9ef6856-42d8-4074-bde7-82f1e1d41282 (Compiler-eem.512) from RePlugin
...
static const char __buildInfo[] __attribute__((unused)) =
    "RePlugin VMMaker.oscog-eem.3475 uuid: "
...
extern void rePluginFree(void *aPointer);
extern void *rePluginMalloc(size_t anInteger);
...
static const char *moduleName = "RePlugin VMMaker.oscog-eem.3475 " INT_EXT;
...
static sqInt allocateByteArrayAndSetRcvrExtraPtrFrom(sqInt anExtraPtr) {
  ...
  if (anExtraPtr) {
    extraObject = instantiateClassindexableSize(classByteArray(),
...
/*	<rcvr primPCRECompile>, where rcvr is an object with instance variables:
        ...
        successful, attempt to optimize the compiled expression. Store the
   results in <pcrePtr> and <extratr>, or fill errorStr with a meaningful
   errorString and errorOffset with an indicator where the error was found,
   applying compileFlags throughout. Answer nil with a clean compile (regardless
   of whether an optimization is possible, and answer with the string otherwise.
 */
...
primPCRECompile(void) {
  ...
  pcrePtr = (sqInt)pcre_compile(patternStrPtr, compileFlags, &errorStrBuffer,
                                &errorOffset, NULL);
  if (pcrePtr) {
    allocateByteArrayAndSetRcvrPCREPtrFromPCRE(pcrePtr);
    extraPtr =
        (sqInt)pcre_study((pcre *)pcrePtr, compileFlags, &errorStrBuffer);
...
/*	<rcvr primPCREExec: searchObject>, where rcvr is an object with instance
        ...
        'patternStr compileFlags pcrePtr extraPtr errorStr errorOffset
        matchFlags'
        Apply the regular expression (stored in <pcrePtr> and <extratr>,
   generated from calls to primPCRECompile), to smalltalk String searchObject
   using <matchOptions>. If there is no match, answer nil. Otherwise answer a
...
primPCREExec(void) {
  ...
  extraPtr =
      ((sqInt)((!(extraObj == (nilObject())) ? arrayValueOf(extraObj) : 0)));
  ...
  result = pcre_exec((pcre *)pcrePtr, (pcre_extra *)extraPtr, searchBuffer,
                     length, 0, matchFlags, matchSpacePtr, matchSpaceSize);
  pop(2);
  pushInteger(result);
...
/*	<rcvr primPCREExec: searchObject from: fromInteger to: toInteger>, where
        ...
        Apply the regular expression (stored in <pcrePtr> and <extratr>,
   generated from calls to primPCRECompile), to smalltalk String searchObject
   using <matchOptions>, beginning at offset <fromInteger> and continuing until
...
/*	Free a block of fixed memory allocated with rePluginMalloc. Instrumented
        ...
        OS-specific variations on malloc/free, such as with MacOS, are handled
   by adding a C macro to the header file redefining malloc/free -- see the
        class comment
 */
...
/*	Allocate a block of fixed memory using C calls to malloc(). Instrumented
        ...
        anInteger. OS-specific variations on malloc/free, such as with MacOS,
   are handled by adding a C macro to the header file redefining malloc/free --
...
void *RePlugin_exports[][3] = {
    ...
    {(void *)_m, "primPCRENumSubPatterns\000\000\000",
     (void *)primPCRENumSubPatterns},
