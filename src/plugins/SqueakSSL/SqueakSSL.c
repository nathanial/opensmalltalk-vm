/* Automatically generated by
   ...
   d9ef6856-42d8-4074-bde7-82f1e1d41282 (Compiler-eem.512) from SqueakSSLPlugin
...
static const char __buildInfo[] __attribute__((unused)) =
    "SqueakSSLPlugin VMMaker.oscog-eem.3475 uuid: "
...
static const char *moduleName = "SqueakSSL VMMaker.oscog-eem.3475 " INT_EXT;
...
/*	Primitive. Starts or continues a server handshake using the current
        session. Will eventually produce output to be sent to the client.
   Requires the host and cert name to be set for the session. Returns a code
   indicating the sate of the connection: > 0	 - Number of bytes to be sent to
   the client. 0	 - Success. The connection is established. -1 - More
   input is required. < -1 - Other errors.
         */
...
/*	Primitive. Starts or continues a client handshake using the provided
   data. Will eventually produce output to be sent to the server. Requires the
   host name to be set for the session. Returns: > 0	 - Number of bytes to be
   sent to the server 0	 - Success. The connection is established. -1 - More
   input is required. < -1 - Other errors.
         */
...
primitiveConnect(void) {
  ...
  srcPtr = (srcPtr + start) - 1;
  dstLen = byteSizeOf(dstOop);
  canDisown = (isPinned(srcOop)) && (isPinned(dstOop));
  if (canDisown) {
...
void *SqueakSSL_exports[][3] = {
    ...
    {(void *)_m, "primitiveGetStringProperty\000\000\000",
     (void *)primitiveGetStringProperty},
    {(void *)_m, "primitiveSetIntProperty\000\000\000",
     (void *)primitiveSetIntProperty},
    {(void *)_m, "primitiveSetStringProperty\000\001\000",
     (void *)primitiveSetStringProperty},
