/* Automatically generated by
        VMPluginCodeGenerator VMMaker.oscog-eem.3658 uuid:
   fe32aec6-a1d5-4e6a-814c-287d3f6467ab (Compiler-eem.520) from
        ThreadedARM64AppleFFIPlugin VMMaker.oscog-eem.3658 uuid:
   fe32aec6-a1d5-4e6a-814c-287d3f6467ab
 */
#if defined(__clang__) || defined(__GNUC__)
static const char __buildInfo[] __attribute__((unused)) =
    "ThreadedARM64AppleFFIPlugin VMMaker.oscog-eem.3658 uuid: "
    "fe32aec6-a1d5-4e6a-814c-287d3f6467ab " __DATE__;
#else
...
#define ThreadedFFIPlugin                                                      \
  1 /* to filter-out unwanted declarations from sqFFI.h */
 ...
 * less than or equal to eight bytes in length in registers. Linux never does
 * so.
 */
#if __linux__
...
/*
 * cygwin -mno-cygwin (MinGW) gcc 3.4.x's alloca is a library routine that
 * answers %esp + xx, so the outgoing stack is offset by one or more word if
 * uncorrected. Grab the actual stack pointer to correct.
 */
...
/* The dispatchOn:in:with:with: generates an unwanted call on error.  Just
 * squash it. */
#define error(foo) 0
...
/*** Function Prototypes ***/
static void *ffiAddressOfstartingAtsize(sqInt rcvr, sqInt byteOffset,
                                        sqInt byteSize);
...
static sqInt structIsHomogenousFloatArrayOfSizetypeSpecofLength(
    sqInt structSize, unsigned int *argSpec, sqInt argSpecSize);
static sqInt structIsHomogenousIntegerArrayOfSizetypeSpecofLength(
    sqInt structSize, unsigned int *argSpec, sqInt argSpecSize);
...
static void *(*ioLoadSymbolOfLengthFromModule)(sqInt functionNameIndex,
...
static sqInt (*primitiveFailForwithSecondary)(sqInt reasonCode,
...
extern void *ioLoadSymbolOfLengthFromModule(sqInt functionNameIndex,
                                            sqInt functionLength,
                                            void *moduleHandle);
...
static const char *moduleName =
    "ARM64AppleFFIPlugin VMMaker.oscog-eem.3658 " INT_EXT;
static sqInt nilObj;
...
#define dispatchFunctionPointerwithwithwithwith(aFunctionPointer, int1, int2,  \
                                                int3, int4)                    \
  (aFunctionPointer)(int1, int2, int3, int4)
#define dispatchFunctionPointerwithwithwithwithwithwith(                       \
    aFunctionPointer, int1, int2, int3, int4, int5, int6)                      \
  (aFunctionPointer)(int1, int2, int3, int4, int5, int6)
#define dispatchFunctionPointerwithwithwithwithwithwithwithwith(               \
    aFunctionPointer, int1, int2, int3, int4, int5, int6, int7, int8)          \
  (aFunctionPointer)(int1, int2, int3, int4, int5, int6, int7, int8)
#define ffiAlloc(bytes) (usqInt) malloc(bytes)
#define ffiFree(pointer) free((void *)(pointer))
#define isCalleePopsConvention(callType)                                       \
  (PLATFORM_API_USES_CALLEE_POPS_CONVENTION && (callType) == FFICallTypeApi)
...
/*	Answer the address of the byte at byteOffset in rcvr. Nominally intended
        for use
        with ExternalAddress objects, this code will work with pure bits arrays
   as well. Answer nil (0) on error. */
...
static void *ffiAddressOfstartingAtsize(sqInt rcvr, sqInt byteOffset,
  ...
  if (!((byteOffset > 0) && (isWordsOrBytes(rcvr)))) {
    return null;
  }
...
static sqInt ffiArgumentSpecClassin(sqInt oop, sqInt argSpec, sqInt argClass,
  ...
  if (!(argClass == nilObj)) {
    ...
    if (!((nilObj == oop) || (includesBehaviorThatOf(oopClass, argClass)))) {
      return FFIErrorCoercionFailed;
    }
  }

  /* Type check 1:
     Is the required class of the argument a general instance of
     ExternalStructure? Okay, we've passed the type check (so far) Determine
     valueOop (e.g., the actual oop to pass as argument). The default is the
     object itself */
  valueOop = oop;
  ...
  isStruct = 0;
  if ((oop != nilObj) && (isPointers(oop))) {
    if ((isStruct =
             includesBehaviorThatOf(oopClass, classExternalStructure()))) {
      valueOop = fetchPointerofObject(0, oop);
    } else {
      if ((argClass == nilObj) && ((slotSizeOf(oop)) >= 3)) {
    ...
    /* Special case for Callback or FFICallback objects.
       Callback: The first field must be a block closure, but we have no way to
       tell. The second field must be an Alien. FFICallback: The first field is
       an ExternalAddress. eem 7/16/2024 11:27 I would like to register the
       class for Callback or FFICallback (preferrably only one) via addGCRoot:
       and use includesBehavior:ThatOf:, but interpreterProxy does not provide
       findClassByName: etc. Note that VMMaker.oscog-eem.3403 includes the
       version that short-cuts directly here to ffiPushPointer:in:. Nicer might
       be to add a primitive activeCallbackClass: thunkIndex: to this plugin and
       have Callback install this at startUp:. But plugin unload/reload would
       break this convention. */
    if (!((argClass == nilObj) || (isStruct))) {
      return FFIErrorCoercionFailed;
    }
  }

  /* #isPointers: will fail if oop is immediate so don't even attempt to use it
     note: the test for #isPointers: above should speed up execution since no
     pointer type ST objects are allowed in external calls (except callbacks)
     and thus if #isPointers: is true then the arg must be ExternalStructure to
     work. If it isn't then the code fails anyways so speed isn't an issue. Do
     the actual preparation of the argument Note: Order is important since
     FFIFlagStructure + FFIFlagPointer is used to represent 'typedef void*
     VoidPointer' and VoidPointer really is *struct* not pointer. */
  if (((((calloutState->ffiArgHeader)) & FFIFlagStructure) != 0)) {
    ...
    if (ptrClass == (classExternalAddress())) {
      ...
      if (structIsHomogenousFloatArrayOfSizetypeSpecofLength(
              structSize, argSpec1, argSpecSize)) {
        availableRegisterSpace =
            (NumFloatRegArgs - ((calloutState->floatRegisterIndex))) *
            BytesPerWord;
        if (structSize <= availableRegisterSpace) {
          if ((((usqInt)(((argSpec1[1]) & FFIAtomicTypeMask))) >>
               FFIAtomicTypeShift) == FFITypeDoubleFloat) {
            memcpy(((void *)((&(((calloutState->floatRegisters))[(
                       calloutState->floatRegisterIndex)])))),
                   ptrAddress, structSize);

            /* Round structSize up and divide by 8 (NB: _not_ 4 !) */
            (calloutState->floatRegisterIndex =
                 ((calloutState->floatRegisterIndex)) +
                 ((((usqInt)((structSize + 7))) >> 3)));
          } else {
            for (i = 0; i < (structSize / (sizeof(float))); i += 1) {
              (*((float *)((
                   &(((calloutState->floatRegisters))
                         [((calloutState->floatRegisterIndex)) + i])))) =
                   (((float *)ptrAddress))[i]);
            }
            ...
            (calloutState->floatRegisterIndex =
                 ((calloutState->floatRegisterIndex)) +
                 ((((usqInt)((structSize + 3))) >> 2)));
          }

          /* doubles can be copied in one go
             floats are passed in the single-precision half of the double
             register values and so must be copied piece-by-piece */
          return 0;
        }
        ...
        availableRegisterSpace =
            (NumIntRegArgs - ((calloutState->integerRegisterIndex))) *
            BytesPerWord;
      }

      /* If it's small (16 bytes or less) and will fit in registers it is passed
         in registers, otherwise it is copied to memory. If it is a Homogenous
         Short Vector (HVA) (up to 32 bytes long) and will fit it is passed in
         registers. */
      if ((structSize <= availableRegisterSpace) &&
          ((structSize <= 16) ||
           (structIsHomogenousIntegerArrayOfSizetypeSpecofLength(
               structSize, argSpec1, argSpecSize)))) {
        if (structSize <= availableRegisterSpace) {
          memcpy(((void *)((&(((calloutState->integerRegisters))[(
                     calloutState->integerRegisterIndex)])))),
                 ptrAddress, structSize);
      ...
      /* all in integer registers; we have no way of getting to SIMD registers
         If small and won't fit in registers, copy to the stack.
         N.B. my (eem) reading of IHI0055B_aapcs64.pdf  is that unlike the
         32-bit PCS, aggregates are never split between memory and registers. */
      if (structSize <= 16) {
        roundedSize = (((structSize + 7) | 7) - 7);
        if ((((calloutState->currentArg)) + roundedSize) >
            ((calloutState->limit))) {
        ...
        misAlignedBytes =
            (((sqInt)((calloutState->currentArg)))) & (BytesPerWord - 1);
        if (misAlignedBytes) {
          (calloutState->currentArg =
               ((calloutState->currentArg)) + (BytesPerWord - misAlignedBytes));
        }
        memcpy((calloutState->currentArg), ptrAddress, structSize);
        (calloutState->currentArg = ((calloutState->currentArg)) + roundedSize);
      }

      /* If it is not small it is passed as a pointer. N.B. Spur guarantees only
         8-byte alignment. IHI0055B_aapcs64.pdf is vague on the memory's
         alignment. Arguably the memory should be pinned in case of a callback.
         Don't bother for now. eem 3/11/2023 */

      /* begin ffiPushPointer:in: */
      if ((((calloutState->integerRegisterIndex)) < NumIntRegArgs) &&
          ((!(((calloutState->ffiArgHeader)) & FFIFlagVariadic)))) {
        ((calloutState
              ->integerRegisters))[(calloutState->integerRegisterIndex)] =
            (((sqInt)ptrAddress));
        (calloutState->integerRegisterIndex =
             ((calloutState->integerRegisterIndex)) + 1);
      } else {
        if ((((calloutState->currentArg)) + BytesPerWord) >
            ((calloutState->limit))) {
          return FFIErrorCallFrameTooBig;
        }
    ...
    if (ptrClass == (classByteArray())) {
      if (!((byteSizeOf(valueOop)) ==
            (((calloutState->ffiArgHeader)) & FFIStructSizeMask))) {
      ...
      if (!(((((calloutState->ffiArgHeader)) & FFIFlagPointer) != 0))) {
        ...
        /* If it's small (16 bytes or less) and will fit in registers it is
           passed in registers, otherwise it is copied to memory. If it is a
           Homogenous Short Vector (HVA) (up to 32 bytes long) and will fit it
        ...
        if (structSize <= 16) {
          ...
          if (misAlignedBytes) {
            (calloutState->currentArg = ((calloutState->currentArg)) +
                                        (BytesPerWord - misAlignedBytes));
          }
          memcpy((calloutState->currentArg), ptrAddress, structSize);
          (calloutState->currentArg =
               ((calloutState->currentArg)) + roundedSize);
        }

        /* If it is not small it is passed as a pointer. N.B. Spur guarantees
           only 8-byte alignment. IHI0055B_aapcs64.pdf is vague on the memory's
           alignment. Arguably the memory should be pinned in case of a
           callback. Don't bother for now. eem 3/11/2023 */
      ...
      /* Since this involves passing the address of the first indexable field we
         need to fail the call if it is threaded and the object is young, since
         it may move during the call. If FFIFlagPointer + FFIFlagStructure is
         set use ffiPushPointer on the contents */
      if (!((((calloutState->ffiArgHeader)) & FFIStructSizeMask) ==
            BytesPerWord)) {
        return FFIErrorStructSize;
      }
      ptrAddress = ((void *)(fetchPointerofObject(0, valueOop)));
  ...
  if (((((calloutState->ffiArgHeader)) & FFIFlagPointer) != 0)) {
    ...
    if (((((calloutState->ffiArgHeader)) & FFIFlagAtomic) != 0)) {
      if (isStruct) {
        ...
        atomicType =
            ((usqInt)((((calloutState->ffiArgHeader)) & FFIAtomicTypeMask))) >>
            FFIAtomicTypeShift;
        ...
        if (atomicType) {
          ...
          if (!((isPointers(ptrType)) && ((slotSizeOf(ptrType)) >= 2))) {
            err = FFIErrorWrongType;
            goto l1;
          }
          specOop = fetchPointerofObject(0, ptrType);
          if (!((isWords(specOop)) && ((slotSizeOf(specOop)) > 0))) {
            err = FFIErrorWrongType;
            goto l1;
          }
          ...
          specType =
              ((usqInt)((spec & FFIAtomicTypeMask))) >> FFIAtomicTypeShift;
       ...
       ******* */
      oopClassSqInt = (valueOop == oop ? oopClass : fetchClassOf(valueOop));
      ...
      if ((isExternalAddress = oopClassSqInt == (classExternalAddress()))) {
        ...
        if ((isStringObj =
                 includesBehaviorThatOf(oopClassSqInt, classString()))) {
          isAlienObj = 0;
        } else {
          isAlienObj = includesBehaviorThatOf(oopClassSqInt, classAlien());
        }
      }
      if ((((usqInt)(atomicType)) >> 1) ==
          (((usqInt)(FFITypeSignedChar8)) >> 1)) {
        if (isStringObj) {
      ...
      /* Since all the following pass the address of the first indexable field
         we need to fail the call if it is threaded and the object is young,
         since it may move during the call. */
      if ((((usqInt)(atomicType)) >> 1) ==
          (((usqInt)(FFITypeSignedInt8)) >> 1)) {
        if (isStringObj || (oopClassSqInt == (classByteArray()))) {
        ...
        if (isExternalAddress) {
          pointer = ((void *)(longAt((void *)(valueOop + BaseHeaderSize))));
          ...
          pointer =
              ((char *)(/* startOfData: */
                        ((longAt((void *)(valueOop + BaseHeaderSize))) > 0
                             ...
                             : longAt((void *)((valueOop + BaseHeaderSize) +
                                               BytesPerOop)))));
      ...
      /* byte* -- see comment on string above
         ...
         I can push pointers to any type (take for instance calls who receive
         int* output arguments, etc.) but I need to store them into a ByteArray,
         ExternalAddress or Alien */
  ...
  if (((((calloutState->ffiArgHeader)) & FFIFlagAtomic) != 0)) {
    ...
    if (!((((usqInt)(atomicType)) >> 1) == 6)) {
      ...
      if (((valueOop & (BytesPerWord - 1)) != 0)) {
        ...
        /* In Squeak nil, true & false are always contiguous at the start of old
         * space... */
        if (oopisGreaterThanOrEqualToandLessThanOrEqualTo(valueOop, nilObj,
                                                          trueObject())) {
          intValue = ((sqLong)((valueOop == (trueObject()) ? 1 : 0)));
          goto l3;
        }
    ...
    l3:
      ...
      /* N.B. Slang inlines the functions within the dispatchOn:in:with:with:
       * here-in */
      switch (atomicType) {
      case 0:
      case 12:
      ...
      case 8: // ffiPushUnsigned64:in:
        if ((((calloutState->integerRegisterIndex)) < NumIntRegArgs) &&
          ...
          /* begin alignCurrentArgOf:to: */
          misAlignedBytes = (((sqInt)((calloutState->currentArg)))) & 7;
          if (misAlignedBytes) {
            (calloutState->currentArg =
                 ((calloutState->currentArg)) + (8 - misAlignedBytes));
          }
      ...
      case 14: // ffiPushUnsigned16:in:
        if ((((calloutState->integerRegisterIndex)) < NumIntRegArgs) &&
          ...
          /* begin alignCurrentArgOf:to: */
          misAlignedBytes = (((sqInt)((calloutState->currentArg)))) & 1;
          if (misAlignedBytes) {
            (calloutState->currentArg =
                 ((calloutState->currentArg)) + (2 - misAlignedBytes));
          }
      ...
      case 15: // ffiPushUnsigned32:in:
        if ((((calloutState->integerRegisterIndex)) < NumIntRegArgs) &&
          ...
          /* begin alignCurrentArgOf:to: */
          misAlignedBytes = (((sqInt)((calloutState->currentArg)))) & 3;
          if (misAlignedBytes) {
            (calloutState->currentArg =
                 ((calloutState->currentArg)) + (4 - misAlignedBytes));
          }
          longAtput((calloutState->currentArg), intValue);
  ...
  l2:
    ...
    if (atomicType == FFITypeSingleFloat) {
      ...
      if ((((calloutState->floatRegisterIndex)) < NumFloatRegArgs) &&
          ((!(((calloutState->ffiArgHeader)) & FFIFlagVariadic)))) {
        (*((float *)((&(((calloutState->floatRegisters))[(
             calloutState->floatRegisterIndex)])))) = valueFloat);
        (calloutState->floatRegisterIndex =
             ((calloutState->floatRegisterIndex)) + 1);
      } else {
        if ((((calloutState->currentArg)) + BytesPerWord) >
            ((calloutState->limit))) {
          return FFIErrorCallFrameTooBig;
      ...
      /* Note: this is a 'memcopy', so size is preserved. Casting to #double
       * changes the size */
      return 0;
    } else {
      ...
      if ((((calloutState->floatRegisterIndex)) < NumFloatRegArgs) &&
          ((!(((calloutState->ffiArgHeader)) & FFIFlagVariadic)))) {
        ((calloutState->floatRegisters))[(calloutState->floatRegisterIndex)] =
            floatValue;
        (calloutState->floatRegisterIndex =
             ((calloutState->floatRegisterIndex)) + 1);
      } else {
...
/*	Map the FFI error code into a primitive error code. If reason is
   negative it encodes one of the standard PrimErr... codes, negated to
   distinguish it from the FFIError codes. If it is an FFIError... code then add
   the size of the primitive error table + 2 to disambiguate it from the
   PrimErr... codes. For historic reasons the FFIError codes range from -1 on up
   hence adding size + 2 maps them to size of table + 1 on up. This OFFSET IS
   undone by ExternalFunction class>>externalCallFailedWith:. Thus we can
   communicate back both PrimErr.. and FFIError codes. Complex but necessary.
...
/*	Support for generic callout. Answer an integer value that is coerced as
   C would do.
 */
...
static sqLong ffiIntegerValueOf(sqInt oop) {
  if (((oop & (BytesPerWord - 1)) != 0)) {
    ...
    /* In Squeak nil, true & false are always contiguous at the start of old
     * space... */
    if (oopisGreaterThanOrEqualToandLessThanOrEqualTo(oop, nilObj,
                                                      trueObject())) {
      return (oop == (trueObject()) ? 1 : 0);
    }
...
static sqInt ffiLoadCalloutAddressFrom(sqInt oop) {
  ...
  address = ((sqInt)(ioLoadSymbolOfLengthFromModule(
      ((sqInt)(firstIndexableField(functionName))), byteSizeOf(functionName),
      ((void *)moduleHandle))));
  if ((failed()) || (address == 0)) {
...
static sqInt ffiLoadCalloutModule(sqInt module) {
  ...
  if (isBytes(module)) {
    ...
    moduleHandle = ((sqInt)(ioLoadModuleOfLength(
        ((sqInt)(firstIndexableField(ffiModuleName))), moduleLength)));
    if ((failed()) || (moduleHandle == 0)) {
...
/*	This is a special entry point exposed such that client code can enable
   and disable logging of FFI calls.
 */
...
static sqInt ffiPushPointerContentsOfin(sqInt oop, CalloutState *calloutState) {
  ...
  if (ptrClass == (classExternalAddress())) {
    ...
    if ((((calloutState->integerRegisterIndex)) < NumIntRegArgs) &&
        ((!(((calloutState->ffiArgHeader)) & FFIFlagVariadic)))) {
      ((calloutState->integerRegisters))[(calloutState->integerRegisterIndex)] =
          (((sqInt)ptrAddress));
      (calloutState->integerRegisterIndex =
           ((calloutState->integerRegisterIndex)) + 1);
    } else {
  ...
  /* Since this involves passing the address of the first indexable field we
     need to fail the call if it is threaded and the object is young, since it
     may move during the call. */
...
EXPORT(const char *)
getModuleName(void) { return "SqueakFFIPrims" PluginVersionInfo; }
...
initialiseModule(void) {
  ...
  /* Get the instSize of ExternalFunction to know whether it contains a cache of
     the stackSize, and what the offset of ExternalLibraryFunction's
     functionName and moduleName slots are. */
  externalFunctionInstSize = instanceSizeOf(classExternalFunction());
  initSurfacePluginFunctionPointers();
  return 1;
}

/*	Answer if oop is an Alien. We could ask if isWordsOrBytes: first, but
   that doesn't help. We still have to do the is:KindOf: walk. We're not
   interested in fast falsehood, but as fast as possible truth, and with the
   current API this is it. */

/* InterpreterPlugin>>#isAlien: */
static sqInt isAlien(sqInt oop) { return isKindOfClass(oop, classAlien()); }
...
primitiveCallout(void) {
  ...
  extern void callAndReturnWithStructAddr(void *structAddr, void *procAddr,
                                          void *arrayAddr);
  CalloutState *calloutState;
  sqInt classOop;
  sqInt err;
  ...
  sqInt i;
  usqLong intRet;
  extern void loadFloatRegs(double, double, double, double, double, double,
                            double, double);
  void *longLongRetPtr;
  usqLong mask;
  sqInt meth;
  ...
  /* This must come early for compatibility with the old FFIPlugin.  Image-level
     code may assume the function pointer is loaded eagerly.  Thanks to Nicolas
     Cellier. */
...
l1:
  ...
  if (!addressSqInt) {
    if (externalFunctionInstSize > ExternalFunctionStackSizeIndex) {
      storeIntegerofObjectwithValue(ExternalFunctionStackSizeIndex,
                                    externalFunction, -1);
    }
...
l2:
  ...
  argTypeArray =
      fetchPointerofObject(ExternalFunctionArgTypesIndex, externalFunction);
  ...
  if (((nTypes = slotSizeOf(argTypeArray))) !=
      ((variadicArgsIndex = nArgs + 1))) {
    if (!(nTypes == (nArgs + 2))) {
      ffiFail(FFIErrorBadArgs);
      goto l5;
    }
  ...
  /* Scan for a variadic arg start indication (FFIFlagVariadic). If one is not
     found, fail due to num types not matching num args + 1.  In this case there
     is an extra type so the total must be nArgs + 2. check if the calling
...
#else  // COGMTVM
  if (!(/* ffiSupportsCallingConvention: */
        (flags == FFICallTypeCDecl) || (flags == FFICallTypeApi))) {
    ffiFail(FFIErrorCallType);
    goto l5;
  }
#endif // COGMTVM

  /* not masking causes threaded calls to fail, which is as they should if the
   * plugin is not threaded. */
  requiredStackSize =
      (externalFunctionInstSize > ExternalFunctionStackSizeIndex
           ? fetchIntegerofObject(ExternalFunctionStackSizeIndex,
                                  externalFunction)
           : -1);
  if (failed()) {
    primitiveFailFor(PrimErrBadMethod);
  ...
  stackSize = (requiredStackSize < 0 ? DefaultMaxStackSize : requiredStackSize);
  calloutState = (&theCalloutState);
  ...
  if (!((isWords(argSpec)) && ((slotSizeOf(argSpec)) > 0))) {
    err = FFIErrorWrongType;
    goto l4;
  }
  ...
  if ((((calloutState->ffiRetHeader)) & (FFIFlagPointer | FFIFlagStructure)) ==
      FFIFlagStructure) {
    (calloutState->structReturnSize =
         ((calloutState->ffiRetHeader)) & FFIStructSizeMask);
...
l4:
  ...
  allocation = alloca((stackSize + ((calloutState->structReturnSize))) +
                      (cStackAlignment()));
  if (mustAlignStack()) {
    allocation =
        ((char *)((((((usqIntptr_t)allocation)) | ((cStackAlignment()) - 1)) -
                   ((cStackAlignment()) - 1))));
  }
  ...
  for (i = 1; i <= nArgs; i += 1) {
    argType = fetchPointerofObject((i >= variadicArgsIndex ? i + 1 : i),
                                   argTypeArray);
    argSpec = fetchPointerofObject(0, argType);
    argClass = fetchPointerofObject(1, argType);
    ...
    (calloutState->ffiArgSpecSize =
         (byteSizeOf(argSpec)) / (sizeof(unsigned int)));
    if (!((calloutState->ffiArgSpecSize))) {
      goto l5;
    }
    (calloutState->ffiArgSpec =
         ((unsigned int *)(firstIndexableField(argSpec))));
    (calloutState->ffiArgHeader =
         (i >= variadicArgsIndex
    ...
    if (err) {
      ...
      while (((calloutState->stringArgIndex)) > 0) {
        free(((calloutState
                   ->stringArgs))[(calloutState->stringArgIndex =
                                       ((calloutState->stringArgIndex)) - 1)]);
      }
#if COGMTVM
  ...
  vmHandle =
      disownVM(/* disownFlagsFor: */
               (((((calloutState->callFlags)) & FFICallFlagThreaded) != 0)
                    ? DisownVMForFFICall + DisownVMForThreading
                    : DisownVMForFFICall));
  if (((calloutState->floatRegisterIndex)) > 0) {
    loadFloatRegs(((calloutState->floatRegisters))[0],
                  ((calloutState->floatRegisters))[1],
                  ((calloutState->floatRegisters))[2],
                  ((calloutState->floatRegisters))[3],
                  ((calloutState->floatRegisters))[4],
                  ((calloutState->floatRegisters))[5],
                  ((calloutState->floatRegisters))[6],
                  ((calloutState->floatRegisters))[7]);
  }
  if ((allocaLiesSoSetSpBeforeCall()) || (mustAlignStack())) {
    setsp((calloutState->argVector));
  }
  atomicType =
      ((usqInt)((((calloutState->ffiRetHeader)) & FFIAtomicTypeMask))) >>
      FFIAtomicTypeShift;
  if (((((usqInt)(atomicType)) >> 1) ==
       (((usqInt)(FFITypeSingleFloat)) >> 1)) ||
      (((((calloutState->ffiRetHeader)) &
         (FFIFlagPointer + FFIFlagStructure)) == FFIFlagStructure) &&
       (structIsHomogenousFloatArrayOfSizetypeSpecofLength(
           ((calloutState->ffiRetHeader)) & FFIStructSizeMask,
           ((unsigned int *)(firstIndexableField((calloutState->ffiRetSpec)))),
           ((specSize = byteSizeOf((calloutState->ffiRetSpec)))) /
               (sizeof(unsigned int)))))) {
    (floatRet.d = dispatchFunctionPointerwithwithwithwithwithwithwithwith(
         ((struct dprr (*)(sqIntptr_t, sqIntptr_t, sqIntptr_t, sqIntptr_t,
                           sqIntptr_t, sqIntptr_t, sqIntptr_t,
                           sqIntptr_t))address),
         ((calloutState->integerRegisters))[0],
         ((calloutState->integerRegisters))[1],
         ((calloutState->integerRegisters))[2],
         ((calloutState->integerRegisters))[3],
         ((calloutState->integerRegisters))[4],
         ((calloutState->integerRegisters))[5],
         ((calloutState->integerRegisters))[6],
         ((calloutState->integerRegisters))[7]));

    /* undo any callee argument pops because it may confuse stack management
     * with the alloca. */
    ...
    ownVM(vmHandle);
    if (atomicType == FFITypeDoubleFloat) {
      result = floatObjectOf(((((floatRet.d)).doubles))[0]);
      goto l3;
    }
    if (atomicType == FFITypeSingleFloat) {
      result = floatObjectOf(((((floatRet.f)).floats))[0]);
      goto l3;
    }

    /* If the struct is a vector of floats then move float[2] to float[1],
       float[4] to float[2] and float[6] to float[3], to pack the float data in
       the double fields.  We can tell if the struct is composed of floats if
       its size is less than the spec size, since the spec size is (1 + n
       fields) * 4 bytes, and the struct size is n fields * 4 bytes for floats
       and n fields * 8 bytes for doubles.  We can't access the spec post call
...
#if SPURVM
    oopSqInt = instantiateClassindexableSize(classByteArray(),
                                             (calloutState->structReturnSize));
#else  /* SPURVM */
...
#endif /* SPURVM */
    memcpy(firstIndexableField(oopSqInt), (&floatRet),
           (calloutState->structReturnSize));
  ...
  /* Integer and Structure returns...
     If struct address used for return value, call is special; struct return
     pointer must be in x8 */
  if (((calloutState->structReturnSize)) > (2 * BytesPerWord)) {
    intRet = 0;
    structAddr = ((void *)(((sqLong)((calloutState->limit)))));
    procAddr1 = ((void *)(((sqLong)address)));
    arrayAddr = ((void *)(((sqLong)((&((calloutState->integerRegisters)))))));

    /* begin setReturnRegister:andCall:withArgsArray: */
    callAndReturnWithStructAddr(structAddr, procAddr1, arrayAddr);
  } else {
    structRet = dispatchFunctionPointerwithwithwithwithwithwithwithwith(
        ((SixteenByteReturnII (*)(sqIntptr_t, sqIntptr_t, sqIntptr_t,
                                  sqIntptr_t, sqIntptr_t, sqIntptr_t,
                                  sqIntptr_t, sqIntptr_t))address),
        ((calloutState->integerRegisters))[0],
        ((calloutState->integerRegisters))[1],
        ((calloutState->integerRegisters))[2],
        ((calloutState->integerRegisters))[3],
  ...
  if (((((calloutState->ffiRetHeader)) & (FFIFlagPointer + FFIFlagStructure)) !=
    ...
    if (((((calloutState->ffiRetHeader)) & FFIFlagPointer) != 0)) {
      ...
      retClass = fetchPointerofObject(1, returnType);
      if (retClass == nilObj) {
        atomicType1 =
            ((usqInt)((((calloutState->ffiRetHeader)) & FFIAtomicTypeMask))) >>
            FFIAtomicTypeShift;
        if ((((usqInt)(atomicType1)) >> 1) ==
            (((usqInt)(FFITypeSignedChar8)) >> 1)) {
          result = (!(stringForCString(((char *)intRet)))
                        ? fetchPointerofObject(PrimErrNoMemory + 1,
                                               primitiveErrorTable())
                        : stringForCString(((char *)intRet)));
          goto l3;
        }
...
#if SPURVM
        oopSqInt =
            instantiateClassindexableSize(classExternalAddress(), BytesPerWord);
...
#endif /* SPURVM */
        storePointerofObjectwithValue(1, retOop, returnType);
        result = retOop;
        goto l3;
      }
    ...
    longLongRetPtr =
        ((calloutState->structReturnType) ? ((char *)((&structRet)))
                                          : (calloutState->limit));
    ...
    retOop = popRemappableOop();
#endif /* SPURVM */
    memcpy(firstIndexableField(oopSqInt), longLongRetPtr,
           (calloutState->structReturnSize));
    storePointerofObjectwithValue(0, retOop, oopSqInt);
    result = retOop;
    goto l3;
  ...
  /* Note: Order is important here since FFIFlagPointer + FFIFlagStructure is
     used to represent 'typedef void* VoidPointer' and VoidPointer must be
  ...
  if (atomicType == FFITypeBool) {
    ...
    value =
        (byteSize == (sizeof(intRet))
             ? intRet
             : intRet & ((((((unsigned long long)1)) << (byteSize * 8))) - 1));
    result = (value ? trueObject() : falseObject());
    goto l3;
  }
  ...
  if (atomicType <= FFITypeSignedInt32) {
    ...
    result = (((atomicType & 1) != 0) ? signed64BitIntegerFor(intRet)
...
l3:
  ...
  /* begin cleanupCalloutState: */
  while (((calloutState->stringArgIndex)) > 0) {
    free(((calloutState->stringArgs))[(
        calloutState->stringArgIndex = ((calloutState->stringArgIndex)) - 1)]);
  }

  /* Can not safely use argumentCount (via e.g. methodReturnValue:) since it may
   * have been changed by a callback. */
  popthenPush(primNumArgs + 1, result);
  /* end ffiCall:ArgArrayOrNil:NumArgs: */
l5:
  return;
}
...
l2:
  ...
  requiredStackSize =
      (externalFunctionInstSize > ExternalFunctionStackSizeIndex
           ...
           : -1);
  if (failed()) {
    primitiveFailFor((!argArray ? PrimErrBadMethod : PrimErrBadReceiver));
    goto l5;
  }
  stackSize = (requiredStackSize < 0 ? DefaultMaxStackSize : requiredStackSize);
  calloutState = (&theCalloutState);
...
l4:
  ...
  for (i = 1; i <= nArgs; i += 1) {
    ...
    (calloutState->ffiArgHeader =
         (i >= variadicArgsIndex
              ? (((calloutState->ffiArgSpec))[0]) | FFIFlagVariadic
              : ((calloutState->ffiArgSpec))[0]));
    oop = (argArray ? fetchPointerofObject(i - 1, argArray)
                    : stackValue(nArgs - i));
    err = ffiArgumentSpecClassin(oop, argSpec, argClass, calloutState);
    if (err) {
      /* begin cleanupCalloutState: */
  ...
  if (((((usqInt)(atomicType)) >> 1) ==
    ...
    /* begin ffiReturnType: */
    specLiteral =
        (argArray ? stackValue(1) : literalofMethod(0, primitiveMethod()));
    argTypes = fetchPointerofObject(ExternalFunctionArgTypesIndex, specLiteral);
    ffiRetType = fetchPointerofObject(0, argTypes);
...
/*	Two forms of C Data Model infomation.
        ...
        LLP64, etc.
        WIth 1 argument, which must be a ByteArray of at least 9 elements,
   answer the sizes of char, short, int, long, long long, wchar_t, float,
   double, void *. */
...
primitiveCDataModel(void) {
  ...
  if ((methodArgumentCount()) == 1) {
    sizes = stackValue(0);
    if (!((isBytes(sizes)) && ((slotSizeOf(sizes)) == 9))) {
  ...
  /* Attempt to identify the programming model:
     ...
     char			 8		 8		  8 8 8 8 short
     16		16		16		16		16
     64 int				16		32		32
     32		64		64 long			32		32
     32 		64		64		64 long long
     64		64		64		64		64
     64 pointer			32		32		64
     64		64		64 */

  /* Set bit 0 if char is wrong, bit 1 if short is wrong, 2 for int, 3 for long,
   * 4 for long long, 5 for void * */
  errorCode = 0;
  if ((sizeof(char)) != 1) {
    errorCode += 1;
  }
  ...
  if ((sizeof(void *)) == 8) {
    ...
    if ((sizeof(int)) == 4) {
      ...
      /* LLP64 */
      if (((sizeof(long)) != 8) && ((sizeof(long)) != 4)) {
        errorCode += 8;
      }
    }

    /* LP64 or LLP64 */
    if (((sizeof(int)) != 8) && ((sizeof(int)) != 4)) {
      errorCode += 4;
    }
  }
...
primitiveExternalAddressAsInteger(void) {
  ...
  if (size == 8) {
    return methodReturnValue(
        positive64BitIntegerFor((*((usqLong *)(firstIndexableField(rcvr))))));
  }
  if (size == 4) {
    return methodReturnValue(positive32BitIntegerFor(
        (*((unsigned int *)(firstIndexableField(rcvr))))));
  }
...
primitiveExternalAddressFromInteger(void) {
  ...
  if ((address = instantiateClassindexableSize(classExternalAddress(),
                                               BytesPerWord))) {
...
primitiveFFIAllocate(void) {
  ...
  oop =
      instantiateClassindexableSize(classExternalAddress(), sizeof(sqIntptr_t));
...
primitiveFFIDoubleAt(void) {
  ...
  if ((addr = ffiAddressOfstartingAtsize(rcvr, integerValueOf(byteOffset),
                                         sizeof(double)))) {
...
/*	Primitive. Return the error code from a failed call to the foreign
        function interface.
        This is for backwards-compatibility. Thread-safe access to the error
   code is via the primitive error code. */
...
primitiveFFIIntegerAt(void) {
  ...
  if ((addr = ffiAddressOfstartingAtsize(rcvr, byteOffset, byteSize))) {
    ...
    if (byteSize < BytesPerWord) {
      ...
      /* sign extend value
         note: byte/short (&long if BytesPerWord=8) never exceed SmallInteger
         range */
    ...
    return primitiveFailFor(
        (isWordsOrBytes(rcvr) ? PrimErrBadIndex : PrimErrBadReceiver));
  }
}
    ...
    if (byteSize < 8) {
      if (isSigned) {
        ...
        if (!((((unsigned long long)value)) <
              (((((unsigned long long)1)) << (8 * byteSize))))) {
          return primitiveFail();
        }
      }
    }
...
l1:
  ...
  moduleHandle = ((
      sqInt)(ioLoadModuleOfLength(((sqInt)(firstIndexableField(ffiModuleName))),
                                  byteSizeOf(ffiModuleName))));
  if (failed()) {
    return ffiFail(FFIErrorModuleNotFound);
  }
  ptr = firstIndexableField(moduleHandlePtr);
...
primitiveLoadSymbolFromModule(void) {
  ...
  module = stackValue(0);
  symbol = stackValue(1);
  moduleHandle = (module != nilObj ? ffiLoadCalloutModule(module) : 0);
  if (failed()) {
    return primitiveFailFor(PrimErrNotFound);
  }
  address = ioLoadSymbolOfLengthFromModule(
      ((sqInt)(firstIndexableField(symbol))), byteSizeOf(symbol),
      ((void *)moduleHandle));
  if ((failed()) || (address == 0)) {
    return primitiveFailFor(PrimErrNotFound);
  }
...
primitiveLogCallsTo(void) {
  ...
  if (logFile == nilObj) {
    ...
    ok = ffiLogFileNameOfLength(firstIndexableField(logFile),
                                byteSizeOf(logFile));
    if (!ok) {
      return primitiveFail();
    }
...
EXPORT(sqInt)
primitivePluginVersion(void) { return methodReturnInteger(1); }
...
/*	Answer the signed 16-bit integer starting at the given byte offset
   (native endian).
 */
...
/*	Store the signed 16-bit integer starting at the given byte offset
   (native endian).
 */
...
primitiveSignedInt16AtPut(void) {
  ...
  if (!((isIntegerObject(valueArg)) &&
        ((((((value = integerValueOf(valueArg))) >= -32768) &&
           (value <= 0x7FFF))) &&
...
/*	Answer the signed 32-bit integer starting at the given byte offset
   (native endian).
 */
...
primitiveSignedInt32At(void) {
  ...
  if ((addr = ffiAddressOfstartingAtsize(rcvr, integerValueOf(byteOffset),
                                         sizeof(int)))) {
    return methodReturnValue(
        signed32BitIntegerFor(((signed int)(unalignedLong32At(addr)))));
  } else {
    return primitiveFailFor(PrimErrBadIndex);
  }
}

/*	Store the signed 32-bit integer starting at the given byte offset
   (native endian).
 */
...
primitiveSignedInt32AtPut(void) {
  ...
  if (!((!(failed())) && (isIntegerObject(byteOffset)))) {
    return primitiveFailFor(PrimErrBadArgument);
  }
  if ((addr = ffiAddressOfstartingAtsize(rcvr, integerValueOf(byteOffset),
                                         sizeof(int)))) {
...
/*	Answer the signed 64-bit integer starting at the given byte offset
   (native endian).
 */
...
primitiveSignedInt64At(void) {
  ...
  if ((addr = ffiAddressOfstartingAtsize(rcvr, integerValueOf(byteOffset),
                                         sizeof(sqLong)))) {
    return methodReturnValue(
        signed64BitIntegerFor(((sqLong)(unalignedLong64At(addr)))));
  } else {
    return primitiveFailFor(PrimErrBadIndex);
  }
}

/*	Store the signed 64-bit integer starting at the given byte offset
   (native endian).
 */
...
/*	Answer the signed 8-bit integer starting at the given byte offset
   (native endian).
 */
...
primitiveSignedInt8AtPut(void) {
  ...
  rcvr = stackValue(2);
  if (!((isIntegerObject(valueArg)) &&
        ((((((value = integerValueOf(valueArg))) >= -128) &&
           (value <= 0x7F))) &&
         (isIntegerObject(byteOffset))))) {
    return primitiveFailFor(PrimErrBadArgument);
  }
...
primitiveStructureElementAlignment(void) {
  ...
  if (!((isIntegerObject(typeCode)) &&
        ((((((typeCode = integerValueOf(typeCode))) >= FFITypeUnsignedInt8) &&
           (typeCode <= FFITypeDoubleFloat))) ||
...
primitiveUnsignedInt32At(void) {
  ...
  if ((addr = ffiAddressOfstartingAtsize(rcvr, integerValueOf(byteOffset),
                                         sizeof(int)))) {
    return methodReturnValue(
        positive32BitIntegerFor(((unsigned int)(unalignedLong32At(addr)))));
  } else {
    return primitiveFailFor(PrimErrBadIndex);
  }
}
...
primitiveUnsignedInt64At(void) {
  ...
  if ((addr = ffiAddressOfstartingAtsize(rcvr, integerValueOf(byteOffset),
                                         sizeof(usqLong)))) {
    return methodReturnValue(
        positive64BitIntegerFor(((usqLong)(unalignedLong64At(addr)))));
  } else {
    return primitiveFailFor(PrimErrBadIndex);
  }
}
...
setInterpreter(struct VirtualMachine *anInterpreter) {
  ...
  if (ok) {
    ...
    ioLoadSymbolOfLengthFromModule =
        interpreterProxy->ioLoadSymbolOfLengthFromModule;
    ...
    positiveMachineIntegerValueOf =
        interpreterProxy->positiveMachineIntegerValueOf;
    primitiveErrorTable = interpreterProxy->primitiveErrorTable;
    primitiveFail = interpreterProxy->primitiveFail;
    primitiveFailFor = interpreterProxy->primitiveFailFor;
#if VM_PROXY_MAJOR > 1 || (VM_PROXY_MAJOR == 1 && VM_PROXY_MINOR >= 14)
    primitiveFailForwithSecondary =
...
/*	See IHI0055B_aapcs64.pdf
   ...
   Aggregate with a Fundamental Data Type that is a Floating-Point type and at
   most four uniquely addressable members. */
...
static sqInt structIsHomogenousFloatArrayOfSizetypeSpecofLength(
  ...
  typeOfFirstField =
      ((usqInt)((((firstField = argSpec[1])) & FFIAtomicTypeMask))) >>
      FFIAtomicTypeShift;
...
/*	See IHI0055B_aapcs64.pdf
   ...
   of one fundamental integral or floating- point type. It may be 8 or 16 bytes
   in total size. A short vector has a base type that is the fundamental
   integral or floating-point type from which it is composed, but its alignment
   is always the same as its total size. The number of elements in the short
   vector is always such that the type is fully packed. For example, an 8-byte
   short vector may contain 8 unsigned byte elements, 4 unsigned half-word
        ...
        a single STR of the entire register; a short vector is loaded from
   memory using the corresponding LDR instruction. On a little-endian system
   this means that element 0 will always contain the lowest addressed element of
   a short vector; on a big-endian system element 0 will contain the
   highest-addressed element of a short vector.

        A language binding may define extended types that map directly onto
   short vectors. Short vectors are not otherwise created spontaneously (for
   example because a user has declared an aggregate consisting of eight
   consecutive byte-sized objects). */
...
static sqInt structIsHomogenousIntegerArrayOfSizetypeSpecofLength(
  ...
  typeOfFirstField =
      ((usqInt)((((firstField = argSpec[1])) & FFIAtomicTypeMask))) >>
      FFIAtomicTypeShift;
  if (!(((typeOfFirstField >= FFITypeUnsignedInt8) &&
         (typeOfFirstField <= FFITypeSignedInt64)))) {
    return 0;
  }
  ...
  /* {	FFITypeUnsignedInt8. FFITypeUnsignedInt16. FFITypeUnsignedInt32.
     FFITypeUnsignedInt64. FFITypeSignedInt8. FFITypeSignedInt16.
     FFITypeSignedInt32. FFITypeSignedInt64} collect:
...
void *ARM64AppleFFIPlugin_exports[][3] = {
    {(void *)_m, "ffiLogCallsTo", (void *)ffiLogCallsTo},
    {(void *)_m, "getModuleName", (void *)getModuleName},
    {(void *)_m, "initialiseModule", (void *)initialiseModule},
    {(void *)_m, "primitiveCallout\000\377\000", (void *)primitiveCallout},
    {(void *)_m, "primitiveCalloutWithArgs\000\004\000",
     (void *)primitiveCalloutWithArgs},
    {(void *)_m, "primitiveCDataModel\000\001\001",
     (void *)primitiveCDataModel},
    {(void *)_m, "primitiveCreateManualSurface\000\000\000",
     (void *)primitiveCreateManualSurface},
    {(void *)_m, "primitiveDestroyManualSurface\000\000\000",
     (void *)primitiveDestroyManualSurface},
    {(void *)_m, "primitiveExternalAddressAsInteger\000\001\001",
     (void *)primitiveExternalAddressAsInteger},
    {(void *)_m, "primitiveExternalAddressFromInteger\000\000\001",
     (void *)primitiveExternalAddressFromInteger},
    {(void *)_m, "primitiveFFIAllocate\000\000\000",
     (void *)primitiveFFIAllocate},
    {(void *)_m, "primitiveFFIDoubleAt\000\001\003",
     (void *)primitiveFFIDoubleAt},
    {(void *)_m, "primitiveFFIDoubleAtPut\000\001\003",
     (void *)primitiveFFIDoubleAtPut},
    {(void *)_m, "primitiveFFIFloatAt\000\001\003",
     (void *)primitiveFFIFloatAt},
    {(void *)_m, "primitiveFFIFloatAtPut\000\001\003",
     (void *)primitiveFFIFloatAtPut},
    {(void *)_m, "primitiveFFIFree\000\001\000", (void *)primitiveFFIFree},
    {(void *)_m, "primitiveFFIGetLastError\000\377\001",
     (void *)primitiveFFIGetLastError},
    {(void *)_m, "primitiveFFIIntegerAt\000\001\001",
     (void *)primitiveFFIIntegerAt},
    {(void *)_m, "primitiveFFIIntegerAtPut\000\001\001",
     (void *)primitiveFFIIntegerAtPut},
    {(void *)_m, "primitiveForceLoad\000\002\000", (void *)primitiveForceLoad},
    {(void *)_m, "primitiveLoadSymbolFromModule\000\002\000",
     (void *)primitiveLoadSymbolFromModule},
    {(void *)_m, "primitiveLogCallsTo\000\001\000",
     (void *)primitiveLogCallsTo},
    {(void *)_m, "primitivePluginVersion\000\377\001",
     (void *)primitivePluginVersion},
    {(void *)_m, "primitiveSetManualSurfacePointer\000\000\000",
     (void *)primitiveSetManualSurfacePointer},
    {(void *)_m, "primitiveSignedInt16At\000\001\001",
     (void *)primitiveSignedInt16At},
    {(void *)_m, "primitiveSignedInt16AtPut\000\001\001",
     (void *)primitiveSignedInt16AtPut},
    {(void *)_m, "primitiveSignedInt32At\000\001\001",
     (void *)primitiveSignedInt32At},
    {(void *)_m, "primitiveSignedInt32AtPut\000\001\001",
     (void *)primitiveSignedInt32AtPut},
    {(void *)_m, "primitiveSignedInt64At\000\001\001",
     (void *)primitiveSignedInt64At},
    {(void *)_m, "primitiveSignedInt64AtPut\000\001\001",
     (void *)primitiveSignedInt64AtPut},
    {(void *)_m, "primitiveSignedInt8At\000\001\001",
     (void *)primitiveSignedInt8At},
    {(void *)_m, "primitiveSignedInt8AtPut\000\001\001",
     (void *)primitiveSignedInt8AtPut},
    {(void *)_m, "primitiveStructureElementAlignment\000\000\001",
     (void *)primitiveStructureElementAlignment},
    {(void *)_m, "primitiveUnsignedInt16At\000\001\001",
     (void *)primitiveUnsignedInt16At},
    {(void *)_m, "primitiveUnsignedInt16AtPut\000\001\001",
     (void *)primitiveUnsignedInt16AtPut},
    {(void *)_m, "primitiveUnsignedInt32At\000\001\001",
     (void *)primitiveUnsignedInt32At},
    {(void *)_m, "primitiveUnsignedInt32AtPut\000\001\001",
     (void *)primitiveUnsignedInt32AtPut},
    {(void *)_m, "primitiveUnsignedInt64At\000\001\001",
     (void *)primitiveUnsignedInt64At},
    {(void *)_m, "primitiveUnsignedInt64AtPut\000\001\001",
     (void *)primitiveUnsignedInt64AtPut},
    {(void *)_m, "primitiveUnsignedInt8At\000\001\001",
     (void *)primitiveUnsignedInt8At},
    {(void *)_m, "primitiveUnsignedInt8AtPut\000\001\001",
     (void *)primitiveUnsignedInt8AtPut},
