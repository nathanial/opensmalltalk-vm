/* Automatically generated by
   ...
   1a32964f-d425-4ed5-b830-6e8ce3d98688
        (* Compiler-eem.518)
   from
        FilePlugin VMMaker.oscog-eem.3599 uuid:
   1a32964f-d425-4ed5-b830-6e8ce3d98688
 */
...
#define MaxPathLen 0x800

/*** Function Prototypes ***/
EXPORT(sqInt)
fileOpenNamesizewritesecure(char *nameIndex, sqInt nameSize, sqInt writeFlag,
                            sqInt secureFlag);
EXPORT(sqInt)
...
static sqInt makeDirEntryNamesizecreateDatemodDateisDirfileSize(
    char *entryName, sqInt entryNameSize, sqInt createDate, sqInt modifiedDate,
    sqInt dirFlag, squeakFileOffsetType fileSize);
...
EXPORT(sqInt) primitiveGetWorkingDirectory(void);
EXPORT(sqInt) primitiveHasFileAccess(void);
EXPORT(sqInt) primitiveSetWorkingDirectory(void);
...
static const char *moduleName = "FilePlugin VMMaker.oscog-eem.3599 " INT_EXT;
static void *sCCPfn;
static void *sCDFfn;
static void *sCDPfn;
static void *sCGFTfn;
...
l1:
  if (!(failed())) {
    if (secureFlag) {
      if (sCOFfn) {
        okToOpen = ((sqInt (*)(char *, sqInt, sqInt))sCOFfn)(
...
/*	Open the new named file, possibly checking security. Answer the file
 * oop. */
...
l1:
  if (!(failed())) {
    if (secureFlag) {
      if (sCOFfn) {
        okToOpen = ((sqInt (*)(char *, sqInt, sqInt))sCOFfn)(nameIndex,
                                                             nameSize, true);
        if (!okToOpen) {
          return primitiveFailFor(PrimErrOperationFailed);
        }
...
usqIntptr_t fileRecordSize(void) { return sizeof(SQFile); }

/*	Return a pointer to the first byte of of the file record within the
   given Smalltalk object, or nil if objectPointer is not a file record.
 */
...
static sqInt makeDirEntryNamesizecreateDatemodDateisDirfileSize(
  ...
  storePointerofObjectwithValue(3, results,
                                (dirFlag ? trueObject() : falseObject()));
...
/*	Connect to the existing file identified by fileDescriptor.
        ...
        writeFlag is aboolean indicating whether to open in read or write mode
   and must be compatible with the existing file access.
 */
...
primitiveConnectToFileDescriptor(void) {
  ...
  if (!((!(failed())) && (isIntegerObject(fdPointer)))) {
    return primitiveFailFor(PrimErrBadArgument);
  }
...
l1:
  if (!(failed())) {
    sqConnectToFileDescriptor(file, ((int)(integerValueOf(fdPointer))),
                              writeFlag);
  }
  if (!(failed())) {
    methodReturnValue(filePointer);
...
/*	Two arguments - directory path, and simple file name;
        ...
        sugar for the current platform (e.g., '.' or '..', or on Windows,
   forward slashes instead of backslashes). These conventions are only fully
        supported on Unix
   ...
   ... See the comment in sqWin32Directory.c for more details.
        ...
        Primitive fails if the outer path does not identify a readable
   directory. (This is a lookup-by-name variant of primitiveDirectoryLookup.) */
...
primitiveDirectoryEntry(void) {
  ...
  /* If the security plugin can be loaded, use it to check for permission.
     If not, assume it's ok */
  okToList =
      (sCLPfn ? ((sqInt (*)(char *, sqInt))sCLPfn)(pathNameIndex, pathNameSize)
              : 1);
  status = (okToList ? dir_EntryLookup(pathNameIndex, pathNameSize,
                                       reqNameIndex, reqNameSize, entryName,
  ...
  popthenPush(3, makeDirEntryNamesizecreateDatemodDateisDirfileSize(
...
primitiveDirectoryGetMacTypeAndCreator(void) {
  ...
  if (!(dir_GetMacFileTypeAndCreator(fileNameIndex, fileNameSize,
                                     typeStringIndex, creatorStringIndex))) {
    return primitiveFail();
  }
  pop(3);
  return 0;
...
/*	Two arguments - directory path, and an index to an item; returns an
   array (see primitiveDirectoryLookup) describing the file or directory, or nil
   if it does not exist.

        Note that in general, the directory path must not contain syntactic
   sugar for the current platform (e.g., '.' or '..', or on Windows, forward
        ...
        supported on Unix platforms; on Windows, they are only supported for
   short non-UNC file paths containing max 260 characters: DON'T:
   primitiveDirectoryLookup '<very long path>' 'foo\.' DON'T:
   primitiveDirectoryLookup '<very long path>' foo\..' DON'T (on Windows):
   primitiveDirectoryLookup '<very long path>' 'foo/bar' DO:
   primitiveDirectoryLookup '<very long path>' '.' See the comment in
   sqWin32Directory.c for more details.

        Primitive fails if the outer path does not identify a readable
   directory. (For a lookup-by-name variant, see primitiveDirectoryEntry.)
 */
...
primitiveDirectoryLookup(void) {
  ...
  okToList =
      (sCLPfn ? ((sqInt (*)(char *, sqInt))sCLPfn)(pathNameIndex, pathNameSize)
              : 1);
  status = (okToList ? dir_Lookup(pathNameIndex, pathNameSize, index, entryName,
                                  (&entryNameSize), (&createDate),
                                  (&modifiedDate), (&dirFlag), (&fileSize),
                                  (&posixPermissions), (&symlinkFlag))
                     : DirNoMoreEntries);
...
primitiveDirectorySetMacTypeAndCreator(void) {
  ...
  if (!((isBytes(creatorString)) &&
        ((isBytes(typeString)) &&
         ((isBytes(fileName)) && (((byteSizeOf(creatorString)) == 4) &&
  ...
  if (!(dir_SetMacFileTypeAndCreator(fileNameIndex, fileNameSize,
                                     typeStringIndex, creatorStringIndex))) {
    return primitiveFail();
  }
  pop(3);
...
/*	Allow to test if the standard input/output files are from a console or
   not Return values:
        * -1 - Error
...
l1:
  ...
  bytesRead = sqFileReadIntoAt(file, count * elementSize,
                               ((char *)(firstIndexableField(array))),
                               (startIndex - 1) * elementSize);
  if (vmHandle) {
    ownVM(vmHandle);
  }
  if (!(failed())) {
  ...
  /* end primitiveFileReadPinningAndDisowning */
l2:;
#else  // COGMTVM
...
l3:
  ...
  /* end primitiveFileReadWithPinning */
l4:;
#endif // COGMTVM
...
l5:
  ...
  /* end primitiveFileReadWithoutPinning */
l6:;
#endif // SPURVM
...
primitiveFileRename(void) {
  ...
  sqFileRenameOldSizeNewSize(oldNameIndex, oldNameSize, newNameIndex,
                             newNameSize);
  if (!(failed())) {
    pop(2);
  }
...
/*	Answer an Array of file handles for standard in, standard out and
   standard error, with nil in entries that are unvailable, e.g. because the
   platform does not provide standard error, etc. Fail if an error occurs
   determining the stdio handles, if the security plugin denies access or if
   memory runs out. */
...
primitiveFileStdioHandles(void) {
  ...
  for (index = 0; index <= 2; index += 1) {
    if (validMask & (1U << index)) {
      ...
      memcpy(firstIndexableField(result), (&(fileRecords[index])),
             sizeof(SQFile));
    }
  }

  /* In the non-Spur threaded VM ensure the handles are old, so that
     sqFileReadIntoAt is unaffected by incremental GCs.  See
     platforms/Cross/plugins/FilePlugin/sqFilePluginBasicPrims.c.  The Spur VM
     uses pinning, so it doesn't need the GC. */
#if COGMTVM
#if !SPURVM
  fullGC();
#endif
...
l1:
  ...
  /* Note: adjust startIndex for zero-origin byte indexing */
  bytesWritten = sqFileWriteFromAt(file, count * elementSize,
                                   ((char *)(firstIndexableField(array))),
                                   (startIndex - 1) * elementSize);
  if (!(failed())) {
...
/*	Answer the VM process's working directory as a UTF8 string. */

/* FilePlugin>>#primitiveGetWorkingDirectory */
EXPORT(sqInt)
primitiveGetWorkingDirectory(void) {
  char buffer[MaxPathLen + 1];
  sqInt nBytesOrError;

  nBytesOrError = dir_GetPathToWorkingDirAsUTF8(buffer, MaxPathLen);
  if (nBytesOrError <= 0) {
    return primitiveFailForOSError(-nBytesOrError);
  }
  buffer[nBytesOrError] = 0;
  methodReturnString(buffer);
  return 0;
}
...
/*	Set the VM process's working directory from a UTF8 string. */

/* FilePlugin>>#primitiveSetWorkingDirectory */
EXPORT(sqInt)
primitiveSetWorkingDirectory(void) {
  char *dstPtr;
  sqInt errorCode;
  sqInt obj;
  char *path;
...
l1:
  if (path) {
    errorCode = dir_SetPathToWorkingDirAsUTF8(path);
    if (!errorCode) {
...
/*	Exported entry point for the VM. Needed for image saving only and no-op
   on anything but Macs.
 */

/* FilePlugin>>#setMacFile:Type:AndCreator: */
EXPORT(sqInt)
setMacFileTypeAndCreator(char *fileName, char *typeString,
                         char *creatorString) {
  return dir_SetMacFileTypeAndCreator(fileName, strlen(fileName), typeString,
                                      creatorString);
}

/* FilePlugin>>#shutdownModule */
EXPORT(sqInt)
shutdownModule(void) { return sqFileShutdown(); }
...
void *FilePlugin_exports[][3] = {
    {(void *)_m, "fileOpenNamesizewritesecure",
     (void *)fileOpenNamesizewritesecure},
    {(void *)_m, "fileOpenNewNamesizesecure",
     (void *)fileOpenNewNamesizesecure},
    {(void *)_m, "getModuleName", (void *)getModuleName},
    {(void *)_m, "initialiseModule", (void *)initialiseModule},
    {(void *)_m, "moduleUnloaded", (void *)moduleUnloaded},
    {(void *)_m, "primitiveConnectToFile\000\001\000",
     (void *)primitiveConnectToFile},
    {(void *)_m, "primitiveConnectToFileDescriptor\000\000\000",
     (void *)primitiveConnectToFileDescriptor},
    {(void *)_m, "primitiveDirectoryCreate\000\001\000",
     (void *)primitiveDirectoryCreate},
    {(void *)_m, "primitiveDirectoryDelete\000\001\000",
     (void *)primitiveDirectoryDelete},
    {(void *)_m, "primitiveDirectoryDelimitor\000\377\000",
     (void *)primitiveDirectoryDelimitor},
    {(void *)_m, "primitiveDirectoryEntry\000\001\000",
     (void *)primitiveDirectoryEntry},
    {(void *)_m, "primitiveDirectoryGetMacTypeAndCreator\000\001\000",
     (void *)primitiveDirectoryGetMacTypeAndCreator},
    {(void *)_m, "primitiveDirectoryLookup\000\001\000",
     (void *)primitiveDirectoryLookup},
    {(void *)_m, "primitiveDirectorySetMacTypeAndCreator\000\001\000",
     (void *)primitiveDirectorySetMacTypeAndCreator},
    {(void *)_m, "primitiveDisableFileAccess\000\377\000",
     (void *)primitiveDisableFileAccess},
    ...
    {(void *)_m, "primitiveFileDelete\000\001\000",
     (void *)primitiveFileDelete},
    {(void *)_m, "primitiveFileDescriptorType\000\000\000",
     (void *)primitiveFileDescriptorType},
    {(void *)_m, "primitiveFileFlush\000\001\000", (void *)primitiveFileFlush},
    {(void *)_m, "primitiveFileGetPosition\000\001\000",
     (void *)primitiveFileGetPosition},
    {(void *)_m, "primitiveFileOpen\000\001\000", (void *)primitiveFileOpen},
    {(void *)_m, "primitiveFileOpenNew\000\001\000",
     (void *)primitiveFileOpenNew},
    {(void *)_m, "primitiveFileRead\000\377\000", (void *)primitiveFileRead},
    {(void *)_m, "primitiveFileRename\000\001\000",
     (void *)primitiveFileRename},
    {(void *)_m, "primitiveFileSetPosition\000\001\000",
     (void *)primitiveFileSetPosition},
    {(void *)_m, "primitiveFileSize\000\001\000", (void *)primitiveFileSize},
    {(void *)_m, "primitiveFileStdioHandles\000\377\000",
     (void *)primitiveFileStdioHandles},
    {(void *)_m, "primitiveFileSync\000\001\000", (void *)primitiveFileSync},
    {(void *)_m, "primitiveFileTruncate\000\001\000",
     (void *)primitiveFileTruncate},
    {(void *)_m, "primitiveFileWrite\000\001\000", (void *)primitiveFileWrite},
    {(void *)_m, "primitiveGetWorkingDirectory\000\377\000",
     (void *)primitiveGetWorkingDirectory},
    {(void *)_m, "primitiveHasFileAccess\000\377\000",
     (void *)primitiveHasFileAccess},
    {(void *)_m, "primitiveSetWorkingDirectory\000\000\000",
     (void *)primitiveSetWorkingDirectory},
...
EXPORT(signed short) primitiveSetWorkingDirectoryMetadata = 0;
#endif // SPURVM
