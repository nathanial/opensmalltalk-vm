/* Automatically generated by
   ...
   d9ef6856-42d8-4074-bde7-82f1e1d41282 (Compiler-eem.512) from
        SoundGenerationPlugin VMMaker.oscog-eem.3475 uuid:
   d9ef6856-42d8-4074-bde7-82f1e1d41282 AbstractSound * Sound-mt.97 uuid:
   122e8e23-da9c-9b48-b787-b6478bd54df7
 */
#if defined(__clang__) || defined(__GNUC__)
static const char __buildInfo[] __attribute__((unused)) =
    "SoundGenerationPlugin VMMaker.oscog-eem.3475 uuid: d9ef6856-42d8-4074-bde7-82f1e1d41282\n\
AbstractSound * Sound-mt.97 uuid: 122e8e23-da9c-9b48-b787-b6478bd54df7 " __DATE__;
#else
...
static const char *moduleName =
    "SoundGenerationPlugin VMMaker.oscog-eem.3475 " INT_EXT;
...
primitiveApplyReverb(void) {
  ...
  for (sliceIndex = startIndex; sliceIndex < (startIndex + n);
       sliceIndex += 1) {
...
primitiveMixFMSound(void) {
  ...
  for (sliceIndex = startIndex; sliceIndex <= lastIndex; sliceIndex += 1) {
    sample = (scaledVol * (waveTable[(scaledIndex / ScaleFactor) + 1])) /
             ScaleFactor;
    if (doingFM) {
      offset = normalizedModulation *
               (waveTable[(scaledOffsetIndex / ScaleFactor) + 1]);
      scaledOffsetIndex =
          (scaledOffsetIndex + scaledOffsetIndexIncr) % scaledWaveTableSize;
      if (scaledOffsetIndex < 0) {
        scaledOffsetIndex += scaledWaveTableSize;
      }
      scaledIndex =
          ((scaledIndex + scaledIndexIncr) + offset) % scaledWaveTableSize;
      if (scaledIndex < 0) {
    ...
    if (scaledVolIncr) {
      ...
      if (((scaledVolIncr > 0) && (scaledVol >= scaledVolLimit)) ||
          ((scaledVolIncr < 0) && (scaledVol <= scaledVolLimit))) {
...
/*	Play samples from a wave table by stepping a fixed amount through the
        ...
        specified, then the index is looped back when the loopEnd index is
   reached until count drops below releaseCount. This allows a short sampled
   sound to be sustained indefinitely.
 */
...
primitiveMixLoopedSampledSound(void) {
  ...
  for (sliceIndex = startIndex; sliceIndex <= lastIndex; sliceIndex += 1) {
    ...
    if (((nextSampleIndex = sampleIndex + 1)) > lastSample) {
      ...
      if (scaledLoopLength) {
        nextSampleIndex =
            ((scaledIndex - scaledLoopLength) / LoopIndexScaleFactor) + 1;
      } else {
    ...
    rightVal =
        (leftVal = (((leftSamples[sampleIndex]) * (LoopIndexScaleFactor - m)) +
                    ((leftSamples[nextSampleIndex]) * m)) /
                   LoopIndexScaleFactor);
    if (isInStereo) {
      rightVal = (((rightSamples[sampleIndex]) * (LoopIndexScaleFactor - m)) +
                  ((rightSamples[nextSampleIndex]) * m)) /
                 LoopIndexScaleFactor;
    }
    if (leftVol > 0) {
...
primitiveMixPluckedSound(void) {
  ...
  for (sliceIndex = startIndex; sliceIndex <= lastIndex; sliceIndex += 1) {
    ...
    average = ((ring[scaledThisIndex / ScaleFactor]) +
               (ring[scaledNextIndex / ScaleFactor])) /
              2;
...
primitiveMixSampledSound(void) {
  ...
  outIndex = startIndex;
  sampleIndex =
      indexHighBits + (((usqInt)(scaledIndex)) >> IncrementFractionBits);
  while ((sampleIndex <= samplesSize) && (outIndex <= lastIndex)) {
    ...
    if (leftVol > 0) {
      ...
      /* clip the result!! */
      aSoundBuffer[i] = ((s > 0x7FFF ? 0x7FFF : (s < -32767 ? -32767 : s)));
    }
    if (rightVol > 0) {
      i = 2 * outIndex;
      s = (aSoundBuffer[i]) + ((sample * rightVol) / ScaleFactor);
    ...
    if ((MaxSmallInteger <= 0x3FFFFFFF) &&
      ...
      scaledIndex -= (((usqInt)(overflow) << IncrementFractionBits));
    }
    sampleIndex =
        indexHighBits + (((usqInt)(scaledIndex)) >> IncrementFractionBits);
    outIndex += 1;
  ...
  /* But we still have to update indexHighBits and scaledIndex correctly outside
   * the loop... */
  if (MaxSmallInteger > 0x3FFFFFFF) {
    overflow = ((usqInt)(scaledIndex)) >> IncrementFractionBits;
    indexHighBits += overflow;
...
void *SoundGenerationPlugin_exports[][3] = {
    ...
    {(void *)_m, "primitiveApplyReverb\000\377\000",
     (void *)primitiveApplyReverb},
    {(void *)_m, "primitiveMixFMSound\000\377\000",
     (void *)primitiveMixFMSound},
    {(void *)_m, "primitiveMixLoopedSampledSound\000\377\000",
     (void *)primitiveMixLoopedSampledSound},
    {(void *)_m, "primitiveMixPluckedSound\000\377\000",
     (void *)primitiveMixPluckedSound},
    {(void *)_m, "primitiveMixSampledSound\000\377\000",
     (void *)primitiveMixSampledSound},
