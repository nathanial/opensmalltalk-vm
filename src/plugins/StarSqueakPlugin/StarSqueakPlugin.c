/* Automatically generated by
   ...
   d9ef6856-42d8-4074-bde7-82f1e1d41282 (Compiler-eem.512) from StarSqueakPlugin
...
static const char __buildInfo[] __attribute__((unused)) =
    "StarSqueakPlugin VMMaker.oscog-eem.3475 uuid: "
...
static const char *moduleName =
    "StarSqueakPlugin VMMaker.oscog-eem.3475 " INT_EXT;
...
/*	Evaporate the integer values of the source Bitmap at the given rate. The
        rate is an integer between 0 and 1024, where 1024 is a scale factor
   of 1.0 (i.e., no evaporation).
 */
...
primitiveEvaporateRate(void) {
  ...
  patchVar = firstIndexableField(patchVarOop);
  /* end checkedUnsignedIntPtrOf: */
l1:
...
l2:
  ...
  for (y = 0; y < (h / patchSize); y += 1) {
    for (x = 0; x < (w / patchSize); x += 1) {
      level = ((shiftAmount < 0)
                   ? (((usqInt)((src[(srcIndex += 1)]))) >> (-shiftAmount))
                   : ((((usqInt)((src[(srcIndex += 1)])) << shiftAmount))));
      if (level > 0xFF) {
        level = 0xFF;
      }
      ...
      for (rowStart = offset; rowStart <= (offset + ((patchSize - 1) * w));
           rowStart += w) {
        for (dstIndex = rowStart; dstIndex < (rowStart + patchSize);
             dstIndex += 1) {
          dst[dstIndex] = pixel;
        }
      }
...
void *StarSqueakPlugin_exports[][3] = {
    ...
    {(void *)_m, "primitiveDiffuseFromToWidthHeightDelta\000\001\000",
     (void *)primitiveDiffuseFromToWidthHeightDelta},
    {(void *)_m, "primitiveEvaporateRate\000\001\000",
     (void *)primitiveEvaporateRate},
    {(void *)_m,
     "primitiveMapFromToWidthHeightPatchSizeRgbFlagsShift\000\001\000",
...
#if SPURVM
EXPORT(signed short) primitiveDiffuseFromToWidthHeightDeltaMetadata = 0x100;
EXPORT(signed short) primitiveEvaporateRateMetadata = 0x100;
EXPORT(signed short)
primitiveMapFromToWidthHeightPatchSizeRgbFlagsShiftMetadata = 0x100;
#endif // SPURVM
