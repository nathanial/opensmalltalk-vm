/* Automatically generated by
   ...
   1a32964f-d425-4ed5-b830-6e8ce3d98688
        (* Compiler-eem.518)
   from
        SoundCodecPlugin VMMaker.oscog-eem.3599 uuid:
   1a32964f-d425-4ed5-b830-6e8ce3d98688
 */
...
static const char *moduleName =
    "SoundCodecPrims VMMaker.oscog-eem.3599 " INT_EXT;
...
primitiveGSMDecode(void) {
  ...
  state = stackValue(5);
  success((isWordsOrShorts(dst)) && ((isBytes(src)) && (isBytes(state))));
  if (failed()) {
    return null;
  }
  srcSize = byteSizeOf(src);
  dstSize = (byteSizeOf(dst)) / 2;
  gsmDecode(state + BaseHeaderSize, frameCount, src, srcIndex, srcSize, dst,
            dstIndex, dstSize, (&srcDelta), (&dstDelta));
  if (failed()) {
    return null;
  }
...
primitiveGSMEncode(void) {
  ...
  frameCount = stackIntegerValue(4);
  state = stackValue(5);
  success((isBytes(dst)) && ((isWordsOrShorts(src)) && (isBytes(state))));
  if (failed()) {
    return null;
  }
  srcSize = (byteSizeOf(src)) / 2;
  dstSize = byteSizeOf(dst);
  gsmEncode(state + BaseHeaderSize, frameCount, src, srcIndex, srcSize, dst,
            dstIndex, dstSize, (&srcDelta), (&dstDelta));
  if (failed()) {
    return null;
  }
...
primitiveGSMNewState(void) {
  ...
  if ((state =
           instantiateClassindexableSize(classByteArray(), gsmStateBytes()))) {
...
void *SoundCodecPrims_exports[][3] = {
    ...
    {(void *)_m, "primitiveGSMDecode\000\001\000", (void *)primitiveGSMDecode},
    {(void *)_m, "primitiveGSMEncode\000\001\000", (void *)primitiveGSMEncode},
    {(void *)_m, "primitiveGSMNewState\000\377\000",
     (void *)primitiveGSMNewState},
...
#if SPURVM
EXPORT(signed short) primitiveGSMDecodeMetadata = 0x100;
EXPORT(signed short) primitiveGSMEncodeMetadata = 0x100;
#endif // SPURVM
